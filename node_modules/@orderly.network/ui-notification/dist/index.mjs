...
import { useWS, useOrderlyContext, useMarketList, useLocalStorage, useMaintenanceStatus, useQuery, MaintenanceStatus } from '@orderly.network/hooks';
...
var extractSymbolsFromMessage = (message) => {
  if (!message) return [];
  const matches = message.matchAll(/\$([A-Z0-9]+)/gi);
  const tickers = [...matches].map((m) => m[1].toUpperCase());
  return [...new Set(tickers)];
};
var isSymbolInMarketList = (symbol, marketList) => {
  const prefix = `PERP_${symbol}_`;
  return marketList.some((m) => m.symbol?.startsWith(prefix));
};
var shouldShowListingDelistingAnnouncement = (item, marketList) => {
  if (item.type !== AnnouncementType.Listing && item.type !== AnnouncementType.Delisting) {
    return true;
  }
...
var useAnnouncementData = () => {
  ...
  useEffect(() => {
    ...
    list = list.filter(
      (item) => shouldShowListingDelistingAnnouncement(item, marketList)
    );
