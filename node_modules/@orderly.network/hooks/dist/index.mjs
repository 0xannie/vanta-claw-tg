...
var generateKeyFun = (args) => (pageIndex, previousPageData) => {
  ...
  const { path: path2, startDate, endDate, size = 100, page, sort } = args;
  ...
  return `${path2}?${search.toString()}`;
};
var useDistribution = (params) => {
  ...
  const flattenOrders = useMemo(() => {
    ...
    return ordersResponse.data?.map((item) => item.rows)?.flat().map((item) => {
      return {
        ...item,
        created_time: item.created_time - 864e5,
        updated_time: item.updated_time - 864e5
      };
    });
  }, [ordersResponse.data]);
  return [
...
var useDaily = (options) => {
  ...
  const startDate = options?.endDate || new Date(getTimestamp() - 864e5 * 30);
  const start_date = formatDate(startDate);
  const end_date = formatDate(endDate);
  const url = `${path2}?start_date=${min(start_date, end_date)}&end_date=${max(
    start_date,
    end_date
  )}`;
...
// src/referral/useRefereeRebateSummary.ts
var useRefereeRebateSummary = (params) => {
  const path2 = "/v1/referral/referee_rebate_summary";
  let url = path2;
...
// src/referral/useCheckReferralCode.ts
var useCheckReferralCode = (code) => {
...
// src/referral/useGetReferralCode.ts
...
function checkTPSLOrderTypeIsMarket(key, values) {
  ...
  return values[orderTypeKey] ? values[orderTypeKey] === OrderType.MARKET : true;
}
...
function priceInputHandle(inputs) {
  ...
  if (values.order_quantity) {
    ...
    newValue.order_quantity = new Decimal(values.total).div(price).todp(config.base_dp).toString();
  }
  return [
    // {
    //   ...values,
...
function quantityInputHandle(inputs) {
  ...
  } else if (values.order_type === OrderType.LIMIT || values.order_type === OrderType.STOP_LIMIT) {
    if (values.order_price) {
...
function tpslInputHandle(inputs) {
  const [values, input, value, markPrice, config] = inputs;
  const price = values.order_type === OrderType.MARKET || values.order_type === OrderType.STOP_MARKET ? markPrice : values.order_price;
...
function calculate(values, fieldName, value, markPrice, config) {
  ...
  remove(prevData) {
    ...
    return prevData.map((item, i) => {
      return {
        ...
        rows: index === i ? item.rows.filter((_, i2) => i2 !== index2) : item.rows
      };
    });
  }
...
// src/services/orderMerge/algoOrderMergeHandler.ts
var AlgoOrderMergeHandler = class _AlgoOrderMergeHandler extends BaseMergeHandler {
  ...
  getFieldChanges(prevData) {
    ...
    const fieldChanges = {};
    if (algo_type === AlgoOrderRootType.TRAILING_STOP && algo_status === OrderStatus.REPLACED && prevData?.length) {
      for (const item of prevData) {
        const algoOrders = item.rows;
...
// src/utils/swr.ts
var generateKeyFun2 = (url, args) => (pageIndex, previousPageData) => {
  if (previousPageData && !previousPageData.rows?.length) {
    return null;
  }
...
// src/referral/useReferralInfo.tsx
var useReferralInfo = () => {
  const { state } = useAccount();
...
var useOrderStream = (params, options) => {
  ...
  const flattenOrders = useMemo(() => {
    ...
    if (includes2.includes("ALL") && excludes.length > 0) {
      return orders2?.filter((item) => !excludes.includes(item.algo_type));
    }
    if (includes2.length > 0 && excludes.length === 0) {
      return orders2?.filter((item) => includes2.includes(item.algo_type));
    }
    if (includes2.length > 0 && excludes.length > 0) {
...
// src/orderly/useOrderStream/useSubAccountAlgoOrderStream.ts
var useSubAccountAlgoOrderStream = (params, options) => {
  const { status } = params;
  ...
  const [doCancelAlgolOrder, { isMutating: cancelAlgoMutating }] = useSubAccountMutation("/v1/algo/order", "DELETE", { accountId });
  ...
  const { data, mutate: mutate6, isLoading } = useSubAccountQuery(
  ...
  const cancelAlgoOrdersByTypes = (types, symbol) => {
    if (!types) {
      throw new SDKError("Types is required");
    }
    if (!Array.isArray(types)) {
  ...
  const cancelAllPendingOrders = useCallback((symbol) => {
...
var useMaxWithdrawal = (token) => {
  ...
  const maxAmount = useMemo(() => {
    ...
    let quantity = 0;
    if (token === "USDC") {
      quantity = maxWithdrawalUSDC2({
        USDCBalance: usdcBalance,
        freeCollateral,
        upnl: unsettledPnL ?? 0
      });
    } else {
      quantity = maxWithdrawalOtherCollateral2({
        USDCBalance: usdcBalance,
        collateralQty: holding?.holding ?? 0,
        freeCollateral,
        indexPrice: indexPriceRef.current,
    ...
    return new Decimal(quantity || 0).todp(tokenInfo.decimals, Decimal.ROUND_DOWN).toNumber();
  }, [
    token,
    tokenInfo?.decimals,
    usdcBalance,
...
var BracketLimitOrderCreator = class extends LimitOrderCreator {
  ...
  async validate(values, config) {
    ...
    const bracketData = await bracketOrderValidator(values, config);
...
function validatePrice(values, config) {
  const errors = {};
  ...
  create(values, config) {
    ...
    if (typeof values.tp_trigger_price !== "undefined") {
      ...
      const childOrderItem = {
        ...
        reduce_only: true,
        side,
        type: values.tp_order_price ? OrderType.LIMIT : OrderType.CLOSE_POSITION,
        trigger_price: tp_trigger_price,
        trigger_price_type: TriggerPriceType.MARK_PRICE,
        symbol: values.symbol,
        is_activated: !!values.tp_trigger_price
      ...
      if (values.tp_order_price) {
        ...
        childOrderItem.price = new Decimal(values.tp_order_price).todp(config.symbol.quote_dp).toNumber();
      }
    ...
    if (typeof values.sl_trigger_price !== "undefined") {
      ...
      const childOrderItem = {
        ...
        reduce_only: true,
        side,
        type: values.sl_order_price ? OrderType.LIMIT : OrderType.CLOSE_POSITION,
        trigger_price: sl_trigger_price,
        trigger_price_type: TriggerPriceType.MARK_PRICE,
        symbol: values.symbol,
        is_activated: !!values.sl_trigger_price
      ...
      if (values.sl_order_price) {
        childOrderItem.type = OrderType.LIMIT;
        childOrderItem.price = new Decimal(values.sl_order_price).todp(config.symbol.quote_dp).toNumber();
      }
      child_orders.push(childOrderItem);
    }
    return {
      algo_type: values.position_type === PositionType.FULL ? AlgoOrderRootType.POSITIONAL_TP_SL : AlgoOrderRootType.TP_SL,
      trigger_price_type: TriggerPriceType.MARK_PRICE,
      // reduce_only: true,
      symbol: values.symbol,
      child_orders
...
var TrailingStopOrderCreator = class extends BaseOrderCreator {
  constructor() {
    super(...arguments);
  ...
  create(values, config) {
    ...
    const order = {
      ...
      activated_price,
      callback_value,
      callback_rate: callback_rate ? new Decimal(callback_rate).div(100).toString() : void 0
    };
    return pick(
      [
        "symbol",
  ...
  async validate(values, config) {
    ...
    if (callback_rate) {
      ...
      if (callbackRate.lt(1e-3) || callbackRate.gt(0.05)) {
...
var getCreateOrderUrl = (order) => {
  const isAlgoOrder = order?.order_type === OrderType.STOP_LIMIT || order?.order_type === OrderType.STOP_MARKET || order?.order_type === OrderType.TRAILING_STOP || order?.order_type === OrderType.CLOSE_POSITION || order.algo_type && order.algo_type === AlgoOrderRootType.BRACKET || isBracketOrder(order);
...
// src/orderly/useTakeProfitAndStopLoss/index.ts
...
function useOrderEntry(symbolOrOrder, sideOrOptions, reduceOnly, options) {
  ...
  const formattedOrder = useMemo(() => {
    ...
    const values = calculate2(parsedData, item.key, item.value);
...
// src/services/painter/basePaint.ts
var BasePaint = class {
...
// src/services/painter/backgroundPaint.ts
...
// src/services/painter/dataPaint.ts
var DataPaint = class extends BasePaint {
  ...
  drawPositionTime(options) {
    ...
    this._drawText(
      !hasReferral ? options.data?.updateTime : `Share on ${options.data?.updateTime}   |`,
      {
        left,
        top: this._ratio(top),
        // top: 536,
...
// src/services/painter/layout.config.ts
      ...
      if (typeof res === "object" && "rows" in res && "current_epoch" in res) {
        ...
        if (Array.isArray(rows)) {
          ...
          if (epochOne && epochOne?.start_time > getTimestamp()) {
            ...
            setCurEpochInfo(
              curEpochIndex !== -1 ? list?.[curEpochIndex] : void 0
            );
          }
          return list;
        }
...
var useAccountRewardsHistory = (address) => {
  const [brokers] = useAllBrokers();
  const { data } = usePrivateQuery(
    `/v1/public/trading_rewards/account_rewards_history?address=${address ?? ""}`,
    {
      revalidateOnFocus: false,
      formatter: (res) => {
        const data2 = res?.rows?.map((e) => {
          e.broker = e.broker.map((item) => ({
            ...item,
            broker_name: item?.broker_id
          }));
          return {
...
// src/trading-rewards/abis/orderTokenAbi.json
...
// src/trading-rewards/abis/proxyLedgerAbi.json
...
// src/trading-rewards/abis/OmnichainLedgerV1ABI.json
...
// src/trading-rewards/useGetClaimed.ts
...
var useApiKeyManager = (queryParams) => {
  ...
  const resetOrderlyKeyIPRestriction = (orderlyKey, mode) => {
...
var useOrderEntity = (order, options) => {
  ...
  const validate = useMemoizedFn(async () => {
    return new Promise(
      async (resolve, reject) => {
        const creator = getOrderCreator(order);
        const errors2 = await creator?.validate(order, prepareData());
        const keys = Object.keys(errors2);
        if (keys.length > 0) {
          setErrors(errors2);
          reject(errors2);
