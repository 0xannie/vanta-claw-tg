...
function getDisplaySide(side) {
  if (side === "BUY") {
    return i18n.i18n.t("common.buy");
  } else if (side === "SELL") {
    return i18n.i18n.t("common.sell");
  }
  return utils.capitalizeString(side);
}
function getOrderExecutionReportMsg(data, symbolsInfo) {
  ...
  const displayQuantity = "algo_type" in data && data.algo_type === types.AlgoOrderRootType.POSITIONAL_TP_SL ? i18n.i18n.t("tpsl.entirePosition") : base_dp === void 0 ? quantity : ui.parseNumber(quantity, { dp: base_dp });
  let title = "";
  let msg = "";
  switch (status) {
    case types.OrderStatus.NEW:
      const isScaledOrder = client_order_id?.startsWith("scaled_");
      if (isScaledOrder) {
        title = i18n.i18n.t("orders.status.scaledSubOrderOpened.toast.title");
        msg = `${displaySide} ${displaySymbol} ${displayQuantity}`;
      } else {
        title = i18n.i18n.t("orders.status.opened.toast.title");
        msg = `${displaySide} ${displaySymbol} ${displayQuantity}`;
      }
      break;
    case types.OrderStatus.FILLED:
    case types.OrderStatus.PARTIAL_FILLED:
      const displayTotalExecutedQuantity = base_dp === void 0 ? total_executed_quantity : ui.parseNumber(total_executed_quantity, { dp: base_dp });
      title = i18n.i18n.t("orders.status.filled.toast.title");
      msg = `${displaySide} ${displaySymbol} ${displayTotalExecutedQuantity} / ${displayQuantity}`;
      break;
    case types.OrderStatus.CANCELLED:
      title = i18n.i18n.t("orders.status.canceled.toast.title");
      msg = `${displaySide} ${displaySymbol} ${displayQuantity}`;
      break;
    case types.OrderStatus.REJECTED:
      title = i18n.i18n.t("orders.status.rejected.toast.title");
      msg = `${displaySide} ${displaySymbol} ${displayQuantity}`;
      break;
    case types.OrderStatus.REPLACED:
      const { algo_type, activated_price } = data;
      if (algo_type === types.AlgoOrderRootType.TRAILING_STOP) {
        const fieldChange = fieldChanges?.[types.AlgoOrderRootType.TRAILING_STOP] || {};
        if (fieldChange.is_activated && fieldChange.extreme_price && activated_price) {
          title = i18n.i18n.t("orders.trailingStop.activated");
        ...
        title = i18n.i18n.t("orders.status.replaced.toast.title");
...
var useExecutionReport = () => {
  ...
  const [soundAutoPlay] = hooks.useLocalStorage(
    ORDERLY_ORDER_SOUND_ALERT_KEY,
    notification?.orderFilled?.defaultOpen ?? false
  );
  const [audioElement] = hooks.useAudioPlayer(src, {
    autoPlay: soundAutoPlay,
    volume: 1
  });
  const handler = hooks.useDebouncedCallback((data) => {
    const showToast = (data2) => {
      const { title, msg, status } = getOrderExecutionReportMsg(
        data2,
        symbolsInfoRef.current
      );
      const isFilled = status === types.OrderStatus.FILLED || status === types.OrderStatus.PARTIAL_FILLED;
...
function useUILocale() {
  ...
  return react.useMemo(() => {
    const calendarLocale = {
      [i18n.LocaleEnum.en]: locale.enUS,
      [i18n.LocaleEnum.zh]: locale.zhCN
      // [LocaleEnum.ja]: ja,
      // [LocaleEnum.es]: es,
      // [LocaleEnum.ko]: ko,
      // [LocaleEnum.vi]: vi,
...
var AppConfigProvider = (props) => {
  return /* @__PURE__ */ jsxRuntime.jsx(AppConfigContext.Provider, { value: props, children: props.children });
};
  ...
  react.useEffect(() => {
    const unsubscribe = ws.privateSubscribe(
      ...
      },
      {
        onMessage(data) {
          if (data.convertId) {
            ui.toast.success(t("transfer.convert.completed"));
          }
        }
      }
    );
...
function useLinkDevice() {
  const { connectedChain, disconnect } = hooks.useWalletConnector();
  const [_, setLinkDeviceStorage] = hooks.useLocalStorage(
    "orderly_link_device",
    {}
  );
  const { account } = hooks.useAccount();
  const { isMobile } = ui.useScreen();
  const configStore = hooks.useConfig();
...
function useSettleEvent() {
  const { t } = i18n.useTranslation();
  hooks.useSettleSubscription({
    onMessage: (data) => {
      const { status } = data;
      switch (status) {
        case "COMPLETED":
          ui.toast.success(t("settle.settlement.completed"));
          break;
        case "FAILED":
          ui.toast.error(t("settle.settlement.failed"));
          break;
      }
    }
  });
...
function useWalletEvent() {
  ...
  const [record, setRecord] = hooks.useSessionStorage(
  ...
  hooks.useWalletSubscription({
    onMessage: (data) => {
      ...
      if (transStatus === "COMPLETED" && showToast) {
        let msg = `${utils.capitalizeString(side)} completed`;
        if (side === "DEPOSIT") {
        ...
        let msg = `${utils.capitalizeString(side)} failed`;
        if (side === "DEPOSIT") {
...
var useWalletStateHandle = (options) => {
  ...
  if (typeof connect !== "function") {
    throw new types.SDKError("Please provide a wallet connector provider");
  }
  hooks.useEventEmitter();
  const isManualConnect = react.useRef(false);
  hooks.useConfig("brokerId");
  const { account, state: accountState } = hooks.useAccount();
  const keyStore = hooks.useKeyStore();
  const networkId = hooks.useConfig("networkId");
  const [chains, { checkChainSupport }] = hooks.useChains();
  const { storageChain, setStorageChain } = hooks.useStorageChain();
  const [unsupported, setUnsupported] = react.useState(false);
  const { track, setTrackUserId } = hooks.useTrack();
  const currentWalletAddress = react.useMemo(() => {
  ...
  const currentChain = react.useMemo(() => {
    ...
    return {
      id: utils.parseChainIdToNumber(id),
      namespace: namespace2
    };
  }, [connectedWallet]);
  react.useEffect(() => {
    if (accountState.status >= types.AccountStatusEnum.EnableTrading && account.accountId) {
  ...
  react.useEffect(() => {
    ...
    if (types.ABSTRACT_CHAIN_ID_MAP.has(parseInt(connectedChain.id)) && connectedWallet?.label !== "AGW") {
      isSupported = false;
    }
  ...
  react.useEffect(() => {
    utils.windowGuard(() => {
      ...
      if (connectedChain?.namespace === types.ChainNamespace.solana) {
        return;
      }
  ...
  react.useEffect(() => {
    if (connectedWallet === null && accountState.status > types.AccountStatusEnum.NotConnected && !accountState.validating) {
      account.disconnect();
      return;
    }
    ...
    if (!!currentWalletAddress && currentWalletAddress !== account.address && !linkData) {
      account.setAddress(currentWalletAddress, {
        ...
        chain: {
          id: utils.praseChainIdToNumber(currentChain.id),
      ...
      track(types.TrackerEventName.walletConnect, {
  ...
  const connectWallet = async () => {
    ...
    return connect({ chainId: options.currentChainId }).then(async (walletState) => {
      if (Array.isArray(walletState) && walletState.length > 0 && walletState[0] && walletState[0].accounts.length > 0) {
        ...
        const chainId = utils.praseChainIdToNumber(wallet.chains[0].id);
        ...
        if (accountState.status === types.AccountStatusEnum.EnableTradingWithoutConnected) {
        ...
        const status = await account.setAddress(wallet.accounts[0].address, {
          ...
          chain: {
            id: utils.praseChainIdToNumber(wallet.chains[0].id),
...
var AppStateProvider = (props) => {
  ...
  const restrictedInfo = hooks.useRestrictedInfo(props.restrictedInfo);
  ...
  const getMessage = (key, type, params) => {
    ...
    const map = {
      ...
      sl_trigger_price: {
        ...
        [hooks.ERROR_MSG_CODES.SL_PRICE_WARNING]: t(
          "tpsl.validate.slTriggerPrice.warning.closeToLiqPrice"
        ),
        [hooks.ERROR_MSG_CODES.SL_PRICE_ERROR]: t(
    ...
    if (this.state.hasError) {
      return /* @__PURE__ */ jsxRuntime.jsxs(
        ui.Flex,
        {
          ...
          itemAlign: "center",
          justify: "center",
          className: ui.cn("oui-h-screen oui-px-[10%]", this.props.className),
          children: [
            this.props.icon || /* @__PURE__ */ jsxRuntime.jsx(Icon, {}),
            /* @__PURE__ */ jsxRuntime.jsx(
              ui.Text,
              {
                className: ui.cn(
                ...
                intensity: 80,
                weight: "bold",
                children: this.props.title || i18n.i18n.t("errorBoundary.title")
              }
            ),
            /* @__PURE__ */ jsxRuntime.jsx(
              ui.Text,
              {
                ...
                weight: "semibold",
                intensity: 36,
                children: this.props.description || i18n.i18n.t("errorBoundary.description")
              }
            ),
            /* @__PURE__ */ jsxRuntime.jsxs(
              ui.Button,
              {
                ...
                size: "md",
                children: [
                  /* @__PURE__ */ jsxRuntime.jsx(RefreshIcon, { className: "oui-mr-1" }),
                  this.props.refreshButtonText || i18n.i18n.t("errorBoundary.refresh")
                ]
              }
            )
          ]
...
exports.OrderlyAppProvider = OrderlyAppProvider;
exports.useAppConfig = useAppConfig;
exports.useAppContext = useAppContext;
exports.useCanTrade = useCanTrade;
exports.useDataTap = useDataTap;
exports.useOrderEntryFormErrorMsg = useOrderEntryFormErrorMsg;
