...
// src/components/campaigns/pricePool/utils.ts
var getCurrentTierIndex = (tradingVolume, tieredPrizePools) => {
  if (!tieredPrizePools || tieredPrizePools.length === 0) {
    return -1;
  }
  const volumeLimits = tieredPrizePools.map((tier) => tier[0]?.volume_limit || 0).filter((limit) => limit >= 0).sort((a, b) => a - b);
...
var calculateProgressWidth = (tradingVolume, tieredPrizePools, offset = 0) => {
  if (!tieredPrizePools || tieredPrizePools.length === 0) {
    return 0;
  }
  const volumeLimits = tieredPrizePools.map((tier) => tier[0]?.volume_limit || 0).filter((limit) => limit >= 0).sort((a, b) => a - b);
  ...
  const progressInSegment = segmentRange > 0 ? volumeInSegment / segmentRange : 0;
...
var getFirstSegmentWidth = (totalTiers) => {
  return Math.floor(100 / (totalTiers * 2));
};
var getProgressLeft = (totalTiers, index) => {
  const firstSegmentWidth = getFirstSegmentWidth(totalTiers);
  if (totalTiers === 1) {
    return `${firstSegmentWidth}%`;
  }
  const left = firstSegmentWidth + index * (100 - firstSegmentWidth) / (totalTiers - 1);
  return `${Math.min(left, 100)}%`;
};
var TradingLeaderboardContext = react.createContext(
  {}
);
var TradingLeaderboardProvider = (props) => {
  const {
    campaignId,
    campaigns,
    backgroundSrc,
  ...
  const { data: generateCode, mutate: generateCodeMutate } = hooks.usePrivateQuery("/v1/referral/info", {
  ...
  const filteredCampaigns = react.useMemo(() => {
    return campaigns ? ramda.sortWith(
      [ramda.descend((campaign) => dateFns.parseISO(campaign.end_time))],
      campaigns
    ) : campaigns;
  }, [campaigns]);
  const memoCampaignChange = hooks.useMemoizedFn((id) => {
    onCampaignChange?.(id);
  });
  const symbols = Array.isArray(currentCampaign?.volume_scope) ? currentCampaign?.volume_scope.join(",") : currentCampaign?.volume_scope;
  ...
  const { data: stats } = hooks.useQuery(
    campaignId !== "general" ? `https://api.orderly.org/v1/public/campaign/stats?${new URLSearchParams(searchParams).toString()}` : null,
  ...
  const tieredIndex = react.useMemo(() => {
  ...
  const _currentCampaign = react.useMemo(() => {
  ...
  return /* @__PURE__ */ jsxRuntime.jsx(TradingLeaderboardContext.Provider, { value: memoizedValue, children });
};
var useTradingLeaderboardContext = () => {
  const ctx = react.useContext(TradingLeaderboardContext);
  return ctx;
};
    ...
    const columns = [
      ...
      },
      {
        ...
        render: (value) => {
          ...
          return /* @__PURE__ */ jsxRuntime.jsx(ui.Text.numeral, { prefix: "$", rule: "price", dp: 2, children: value });
        },
        width: 105
      },
      {
        ...
        render: (value) => {
          ...
          return /* @__PURE__ */ jsxRuntime.jsx(ui.Text.pnl, { prefix: "$", rule: "price", dp: 2, coloring: true, children: value });
        },
        width: 90
      },
      {
...
function useGeneralRankingScript(options) {
  ...
  const { data: top100Data } = hooks.useQuery(
    state.address ? getUrl({
      page: 1,
      pageSize: 100,
  ...
  const { data: userDataRes = [] } = hooks.useQuery(
    state.address ? getUrl({ page: 1, pageSize: 1, address: state.address, sort: null }) : null,
    {
      revalidateOnFocus: false
    }
  );
  const getAddressRank = react.useCallback(
    (address) => {
  ...
  const dataSource = react.useMemo(() => {
    ...
    if (campaignRankingList && campaignRankingList.length >= 0) {
      if (filteredCampaignData) {
        list = filteredCampaignData;
      } else {
        list = list.filter(
          (item) => campaignRankingList.includes(item.address)
        );
      }
    }
    if (page === 1) {
  ...
  const pagination = react.useMemo(
    () => parsePagination({
      total: campaignRankingList ? dataSource?.length : data?.meta?.total || 0,
  ...
  const onSort = react.useCallback(
    (sort2) => {
      if (sort2?.sortKey === "volume") {
        sort2.sortKey = "perp_volume";
      } else if (sort2?.sortKey === "pnl") {
        sort2.sortKey = "realized_pnl";
      }
      setSort((_sort) => {
        if (sort2) return sort2;
        return isMobile ? { sortKey: _sort?.sortKey || "", sort: "desc" } : initialSort;
      });
    },
...
var useCampaignRankingList = ({
  ...
  const { data: campaignData } = hooks.useQuery(
    getUrl("Week 2"),
    {
      revalidateOnFocus: false
    }
  ...
  const campaignRankingList = react.useMemo(() => {
    if (!dateRange?.label) {
      return void 0;
    }
    return campaignData?.map((item) => item.address) || [];
  }, [campaignData, dateRange]);
  const filteredCampaignData = react.useMemo(() => {
    if (campaignRankingList && dateRange?.label === "Week 1") {
      const list = data?.filter(
        (item) => campaignRankingList.includes(item.address) && (searchValue ? isSameAddress(item.address, searchValue) : true)
      ) || [];
...
function getUserMetricValue(userdata, metric) {
  return metric === "volume" ? userdata.volume : userdata.pnl;
}
function estimateUserRank(userdata, metric) {
  if (userdata.rank) {
    return Number(userdata.rank);
  }
  const userMetricValue = getUserMetricValue(userdata, metric);
  const totalParticipants = userdata.total_participants || 1e3;
  if (userMetricValue <= 0) return null;
  if (userMetricValue >= 1e5) return 1;
  if (userMetricValue >= 5e4) return Math.floor(totalParticipants * 0.05);
  if (userMetricValue >= 1e4) return Math.floor(totalParticipants * 0.2);
  if (userMetricValue >= 1e3) return Math.floor(totalParticipants * 0.5);
  return Math.floor(totalParticipants * 0.8);
}
function findRewardInPool(userdata, pool) {
...
function calculateEstimatedRewards(userdata, campaign) {
  ...
  return totalEstimatedReward > 0 ? { amount: totalEstimatedReward, currency: mainCurrency } : null;
}
function calculateEstimatedTickets(userdata, ticketRules) {
  const userMetricValue = ticketRules.metric === "volume" ? userdata.volume : userdata.pnl;
...
function calculateTieredTickets(metricValue, tiers) {
...
function calculateLinearTickets(metricValue, linearRule) {
...
function formatTicketCount(tickets) {
  return tickets.toLocaleString();
}
function calculateUserPoolReward(userdata, pool) {
  return findRewardInPool(userdata, pool);
}
function calculateTicketProgress(userdata, ticketRules) {
  ...
  if (ticketRules.mode === "tiered" && ticketRules.tiers) {
    ...
    if (!currentTier) {
      const progress2 = userMetricValue / nextTier.value * 100;
      return {
        percent: progress2,
        value: nextTier.value - userMetricValue
      };
    }
    const progress = (userMetricValue - currentTier.value) / (nextTier.value - currentTier.value) * 100;
    return {
      percent: progress,
      value: nextTier.value - userMetricValue
    };
...
// src/components/ranking/campaignRanking/campaignRanking.script.ts
function useCampaignRankingScript(options) {
  ...
  const [sort, setSort] = react.useState(initialSort);
  const { currentCampaign, setUserData, setUpdatedTime, dataAdapter } = useTradingLeaderboardContext();
  const { state } = hooks.useAccount();
  const brokerId = hooks.useConfig("brokerId");
  const { isMobile } = ui.useScreen();
  const { page, pageSize, setPage, parsePagination } = ui.usePagination({
  ...
  const { data: userDataRes } = hooks.useQuery(getUserUrl(), {
    revalidateOnFocus: false
  });
  const getAddressRank = react.useCallback(
    (address) => {
  ...
  const dataSource = react.useMemo(() => {
    if (customData) {
      return formatData2(customData?.dataSource, currentCampaign, sortKey);
    }
    let list = data?.rows || [];
    if (page === 1) {
      list = list.slice(0, pageSize);
    ...
    const rankList = addRankForList(list, total);
    const _data = page === 1 ? userData ? [userData, ...rankList] : rankList : rankList;
    return formatData2(_data, currentCampaign, sortKey);
  }, [
    data,
    page,
    pageSize,
  ...
  const dataList = react.useMemo(() => {
    if (customData) {
      return formatData2(customData?.dataList, currentCampaign, sortKey);
    }
    if (!infiniteData?.length) {
      return [];
    }
  ...
  const pagination = react.useMemo(
    () => parsePagination({
      ...
      current_page: customData?.meta?.current_page || data?.meta?.current_page || 1,
      records_per_page: pageSize
    }),
    [
      data?.meta?.total,
...
function formatData2(data, currentCampaign, metric) {
  const pool = currentCampaign?.prize_pools?.find(
    (item) => item.metric === metric
  );
  return data?.map((item) => {
    const rewards = pool ? calculateUserPoolReward(item, pool) : 0;
    return {
      ...item,
      rewards: {
        amount: rewards,
...
var CampaignRankingWidget = (props) => {
...
var LeaderboardFilter = (props) => {
  const { t } = i18n.useTranslation();
  const { isMobile } = ui.useScreen();
  const { useCampaignDateRange, weeklyRanges, setDateRange } = props;
  const weeklyView = /* @__PURE__ */ jsxRuntime.jsx(ui.Flex, { gap: 3, className: ui.cn(isMobile ? "oui-h-[24px]" : "oui-h-[53px]"), children: weeklyRanges.map((range) => /* @__PURE__ */ jsxRuntime.jsxs(
    "button",
    {
      className: "oui-relative oui-w-fit oui-whitespace-nowrap oui-px-2 oui-py-[2px] oui-text-sm",
      children: [
        /* @__PURE__ */ jsxRuntime.jsx("div", { className: "oui-z-10", children: /* @__PURE__ */ jsxRuntime.jsx(
          ui.Text.gradient,
          {
            color: props.dateRange?.label === range.label ? "brand" : void 0,
            className: props.dateRange?.label !== range.label ? "oui-text-base-contrast-54" : "",
  ...
  const input = /* @__PURE__ */ jsxRuntime.jsx(
    ui.Input,
    {
      ...
      suffix: props.searchValue && /* @__PURE__ */ jsxRuntime.jsx(ui.Box, { mr: 2, children: /* @__PURE__ */ jsxRuntime.jsx(
  ...
  const dateRangeView = props.filterItems.length > 0 && /* @__PURE__ */ jsxRuntime.jsx(
    ui.DataFilter,
    {
      ...
      "button",
      {
        ...
        children: [
          /* @__PURE__ */ jsxRuntime.jsx("div", { className: "oui-z-10", children: /* @__PURE__ */ jsxRuntime.jsx(
            ui.Text.gradient,
            {
              color: props.filterDay === value ? "brand" : void 0,
              className: props.filterDay !== value ? "oui-text-base-contrast-54" : "",
              children: `${value}D`
            }
          ) }),
          /* @__PURE__ */ jsxRuntime.jsx(
  ...
  if (isMobile) {
    return /* @__PURE__ */ jsxRuntime.jsxs(
      ui.Flex,
      {
        ...
        direction: "column",
        mt: 3,
        className: ui.cn("oui-mobile-trading-leaderboard-ranking-filter"),
        children: [
          input,
          useCampaignDateRange ? /* @__PURE__ */ jsxRuntime.jsx(ui.Flex, { gap: 3, className: "oui-w-full oui-py-3", children: /* @__PURE__ */ jsxRuntime.jsx(ui.ScrollIndicator, { className: "oui-w-full", children: weeklyView }) }) : /* @__PURE__ */ jsxRuntime.jsxs(ui.Flex, { gap: 3, className: "oui-w-full", children: [
            dateRangeView,
            /* @__PURE__ */ jsxRuntime.jsx(ui.ScrollIndicator, { className: "oui-w-full", children: /* @__PURE__ */ jsxRuntime.jsx(ui.Flex, { gap: 3, children: filterDayView }) })
          ] })
        ]
      }
    );
  ...
  return /* @__PURE__ */ jsxRuntime.jsxs(
    ui.Flex,
    {
      ...
      justify: "between",
      itemAlign: "center",
      className: ui.cn("oui-trading-leaderboard-ranking-filter"),
      children: [
        ...
        /* @__PURE__ */ jsxRuntime.jsx(ui.Box, { width: 240, children: input })
      ]
    }
  );
};
var SearchIcon = (props) => /* @__PURE__ */ jsxRuntime.jsx(
  "svg",
  {
    width: "14",
    height: "14",
    ...
    xmlns: "http://www.w3.org/2000/svg",
    ...props,
    children: /* @__PURE__ */ jsxRuntime.jsx("path", { d: "M5.841 1.14a4.667 4.667 0 0 0 0 9.333 4.74 4.74 0 0 0 2.875-.975l2.54 2.56a.6.6 0 0 0 .838 0 .6.6 0 0 0 0-.838L9.537 8.677a4.72 4.72 0 0 0 .971-2.871 4.667 4.667 0 0 0-4.667-4.667m0 1.166a3.5 3.5 0 1 1 0 7 3.5 3.5 0 0 1 0-7" })
  }
);
var LeaderboardTabs = (props) => {
  const { t } = i18n.useTranslation();
  ...
  const { showVolume, showPnl } = react.useMemo(() => {
    const metrics = currentCampaign?.prize_pools?.map((item) => item.metric);
    const isMobileGeneral = props.isMobile && !currentCampaign;
    const showVolume2 = isMobileGeneral ? true : metrics?.includes("volume" /* Volume */);
    const showPnl2 = isMobileGeneral ? true : metrics?.includes("pnl" /* Pnl */);
    return {
  ...
  react.useEffect(() => {
    if (showVolume && showPnl) {
      props.onTabChange("volume" /* Volume */);
    } else if (showVolume) {
      props.onTabChange("volume" /* Volume */);
    } else if (showPnl) {
      props.onTabChange("pnl" /* Pnl */);
    }
  ...
  return /* @__PURE__ */ jsxRuntime.jsxs(
    ui.Flex,
    {
      ...
      children: [
        renderTabs(),
        updateTime && /* @__PURE__ */ jsxRuntime.jsxs(
          ui.Flex,
          {
            itemAlign: "start",
            ...
            children: [
              /* @__PURE__ */ jsxRuntime.jsxs("span", { children: [
                t("tradingLeaderboard.lastUpdate"),
                ":"
              ] }),
              /* @__PURE__ */ jsxRuntime.jsx("span", { children: updateTime })
            ]
          }
        )
      ]
  ...
  if (isMobile) {
    return /* @__PURE__ */ jsxRuntime.jsxs(
      ui.Box,
      {
        ...
        style: props.style,
        children: [
          /* @__PURE__ */ jsxRuntime.jsx(LeaderboardFilter, { ...props }),
          /* @__PURE__ */ jsxRuntime.jsx(
            LeaderboardTabs,
            {
              isMobile,
...
function useCampaignLeaderboardScript() {
  const [activeTab, setActiveTab] = react.useState(
    "volume" /* Volume */
  );
  return {
...
var CampaignLeaderboard = (props) => {
  const { isMobile } = ui.useScreen();
  const sortKey = react.useMemo(() => {
    return props.activeTab === "volume" /* Volume */ ? "volume" : "pnl";
  }, [props.activeTab]);
  const fields = react.useMemo(() => {
    const allFields = [
      "rank",
...
var CampaignLeaderboardWidget = (props) => {
...
var LeaderboardBackground = (props) => {
  const { isMobile } = ui.useScreen();
  ...
  if (isVideo) {
    return /* @__PURE__ */ jsxRuntime.jsxs("div", { className: ui.cn("oui-absolute oui-left-0 oui-top-0", "oui-size-full"), children: [
      /* @__PURE__ */ jsxRuntime.jsx(
        "div",
        {
          style: {
          ...
          className: ui.cn("oui-absolute oui-left-0 oui-top-0", "oui-size-full")
        }
      ),
...
// src/components/campaigns/type.ts
var CampaignTagEnum = /* @__PURE__ */ ((CampaignTagEnum2) => {
  CampaignTagEnum2["ONGOING"] = "ongoing";
  CampaignTagEnum2["COMING"] = "coming";
  CampaignTagEnum2["ENDED"] = "ended";
  CampaignTagEnum2["EXCLUSIVE"] = "exclusive";
  return CampaignTagEnum2;
})(CampaignTagEnum || {});

// src/components/campaigns/utils.ts
var DEFAULT_TIMEZONE_DISPLAY = "UTC";
var getUserTimezoneDisplay = () => {
  try {
...
var getCampaignTag = (campaign, userReferralCode) => {
...
var formatCampaignDateRange = (startTime, endTime, showTimezone = true) => {
  try {
    ...
    const startFormatted = dateFns.format(startDate, "MMM d, yyyy");
    const endFormatted = dateFns.format(endDate, "MMM d, yyyy");
...
var getTradingVolume = (statistics) => {
  return statistics?.total_volume || 0;
};
var getTotalPrizePool = (campaign, tieredIndex) => {
  ...
  campaign.prize_pools.forEach((pool) => {
    currencyTotals[pool.currency] = (currencyTotals[pool.currency] || 0) + pool.total_prize;
  });
...
var getTicketPrizePool = (campaign) => {
...
var formatPrizeAmount = (amount, currency) => {
  return `${amount?.toLocaleString()} ${currency}`;
};
var formatTradingVolume = (volume, decimalPlaces = 1) => {
  if (volume >= 1e6) {
    return `$${(volume / 1e6).toFixed(decimalPlaces)}M`;
  } else if (volume >= 1e3) {
    return `$${(volume / 1e3).toFixed(decimalPlaces)}K`;
  } else {
    return `$${volume.toFixed(decimalPlaces)}`;
  }
};
var formatParticipantsCount = (count, decimalPlaces = 1) => {
  if (count >= 1e6) {
    return `${(count / 1e6).toFixed(decimalPlaces)}M`;
  } else if (count >= 1e3) {
    return `${(count / 1e3).toFixed(decimalPlaces)}K`;
  } else {
    return count.toLocaleString();
  }
};
var generateCampaignTimeline = (campaign) => {
  const currentTime = /* @__PURE__ */ new Date();
  const registerTime = campaign.register_time ? new Date(campaign.register_time) : null;
  const startTime = new Date(campaign.start_time);
  const endTime = new Date(campaign.end_time);
  const rewardTime = campaign.reward_distribution_time ? new Date(campaign.reward_distribution_time) : null;
  const timeline = [];
  const getTimelineType = (time, isNow = false) => {
    if (isNow) return "active";
    return currentTime >= time ? "past" : "future";
  };
  const formatTimeDisplay = (time) => {
    try {
      const formatted = dateFns.format(time, "yyyy-MM-dd HH:mm");
      const timezoneDisplay = getUserTimezoneDisplay();
  ...
  if (registerTime) {
    ...
    const isInRegisterTime = currentTime >= registerTime && currentTime <= startTime;
    if (isInRegisterTime) {
      timeline.push({
        title: i18n.i18n.t("chart.now"),
        type: "active",
  ...
  timeline.push({
    title: i18n.i18n.t("tradingLeaderboard.battleStarts"),
  ...
  timeline.push({
    title: i18n.i18n.t("tradingLeaderboard.battleEnds"),
    type: getTimelineType(endTime),
    time: formatTimeDisplay(endTime)
  });
  if (rewardTime) {
    timeline.push({
      title: i18n.i18n.t("tradingLeaderboard.rewardDistribution"),
...
var NormalPricePoolUI = ({
  ...
  const tooltipContent = react.useMemo(() => {
    ...
    return /* @__PURE__ */ jsxRuntime.jsx("div", { className: "oui-flex oui-min-w-[240px] oui-flex-col oui-gap-1", children: campaign?.prize_pools?.map((pool) => {
      return /* @__PURE__ */ jsxRuntime.jsxs(
        "div",
        {
  ...
  return /* @__PURE__ */ jsxRuntime.jsxs("div", { className: "oui-overflow-hidden oui-rounded-2xl oui-border oui-border-solid oui-border-base-contrast/[0.08]", children: [
    /* @__PURE__ */ jsxRuntime.jsxs(
      "div",
      {
        className: ui.cn([
          ...
          /* @__PURE__ */ jsxRuntime.jsxs("div", { className: "oui-flex oui-items-center oui-gap-1", children: [
            /* @__PURE__ */ jsxRuntime.jsx(
              ui.Text,
              {
                ...
                weight: "semibold",
                className: "oui-text-base-contrast-54",
                children: campaign?.user_account_label || t("tradingLeaderboard.participants")
              }
            ),
            /* @__PURE__ */ jsxRuntime.jsx(
              ui.Text,
              {
                ...
                children: statistics?.total_participants
              }
            )
          ] }),
          /* @__PURE__ */ jsxRuntime.jsxs("div", { className: "oui-flex oui-items-center oui-gap-1", children: [
            /* @__PURE__ */ jsxRuntime.jsx(
              ui.Text,
              {
                ...
                weight: "semibold",
                className: "oui-text-base-contrast-54",
                children: t("tradingLeaderboard.tradingVolume")
              }
            ),
            /* @__PURE__ */ jsxRuntime.jsx(
              ui.Text.numeral,
    ...
    /* @__PURE__ */ jsxRuntime.jsxs(
      "div",
      {
        ...
        children: [
          /* @__PURE__ */ jsxRuntime.jsxs("div", { className: "oui-flex oui-items-center oui-gap-6", children: [
                  ...
                  ui.Tooltip,
                  {
                    ...
                    children: /* @__PURE__ */ jsxRuntime.jsx(
                      "div",
                      {
                        ...
                        children: /* @__PURE__ */ jsxRuntime.jsx(ui.InfoCircleIcon, { className: "oui-cursor-pointer" })
                      }
                    )
                  }
                )
              ...
              /* @__PURE__ */ jsxRuntime.jsx("div", { children: /* @__PURE__ */ jsxRuntime.jsx(
                ui.Text.gradient,
                {
                  ...
                  className: ui.cn([
                    ...
                    isMobile ? "oui-text-[20px] oui-leading-[24px]" : "oui-text-[24px] oui-leading-[28px]"
                  ]),
                  children: totalPrizePool ? formatPrizeAmount(
            ...
            (ticketPrizePool || highlightPool) && /* @__PURE__ */ jsxRuntime.jsxs("div", { className: "oui-flex oui-flex-col", children: [
              /* @__PURE__ */ jsxRuntime.jsx("div", { className: "oui-flex oui-items-center oui-gap-1", children: /* @__PURE__ */ jsxRuntime.jsx(
                ui.Text,
                {
                  size: isMobile ? "2xs" : "xs",
                  weight: "semibold",
                  className: "oui-text-base-contrast-54",
                  children: highlightPool?.label || "Raffle prize"
                }
              ) }),
              /* @__PURE__ */ jsxRuntime.jsx("div", { children: /* @__PURE__ */ jsxRuntime.jsx(
                ui.Text.gradient,
                {
                  ...
                  children: formatPrizeAmount(
                    ticketPrizePool?.amount || highlightPool?.total_prize || 0,
                    ticketPrizePool?.currency || highlightPool?.currency || "USDC"
                  )
                }
              ) })
            ] })
          ] }),
          isMobile && campaign?.rule_url && /* @__PURE__ */ jsxRuntime.jsxs(
            "div",
            {
              ...
              children: [
                ...
                /* @__PURE__ */ jsxRuntime.jsx(ui.ChevronRightIcon, { size: 16, className: "oui-text-base-contrast-36" })
              ]
            }
          ),
          /* @__PURE__ */ jsxRuntime.jsxs(
            "div",
            {
              ...
              children: [
                !isMobile && campaign?.rule_url && /* @__PURE__ */ jsxRuntime.jsx(
                  ui.Button,
                  {
                    size: "md",
                    variant: "outlined",
                    ...
                    onClick: onLearnMore,
                    fullWidth: true,
                    children: t("tradingLeaderboard.viewRules")
                  }
                ),
                (shouldShowJoinButton || showTradeButton) && /* @__PURE__ */ jsxRuntime.jsx("div", { className: "oui-w-full", children: /* @__PURE__ */ jsxRuntime.jsxs(
                  uiConnector.AuthGuard,
                  {
                    ...
                    children: [
                      shouldShowJoinButton && /* @__PURE__ */ jsxRuntime.jsx(
                        ui.Button,
                        {
                          size: isMobile ? "sm" : "md",
                          variant: "gradient",
                          ...
                          fullWidth: true,
                          children: t("tradingLeaderboard.joinNow")
                        }
                      ),
                      showTradeButton && /* @__PURE__ */ jsxRuntime.jsx(
                        ui.Button,
                        {
                          size: isMobile ? "sm" : "md",
                          variant: "gradient",
                          ...
                          onClick: onTradeNow,
                          fullWidth: true,
                          children: campaign?.trading_config?.format || t("tradingLeaderboard.tradeNow")
                        }
                      )
                    ]
                  }
...
var PricePoolDesktopUI = ({
  ...
  const renderButton = () => {
    if (shouldShowJoinButton || showTradeButton) {
      return /* @__PURE__ */ jsxRuntime.jsx("div", { className: "oui-w-full", children: /* @__PURE__ */ jsxRuntime.jsxs(
        uiConnector.AuthGuard,
        {
          buttonProps: {
            size: "md",
            fullWidth: true,
            className: ui.cn(["oui-px-5", !isMobile && "oui-w-[590px]"])
          },
          children: [
            shouldShowJoinButton && /* @__PURE__ */ jsxRuntime.jsx(
              ui.Button,
    ...
    "div",
    {
      ...
      children: [
        /* @__PURE__ */ jsxRuntime.jsxs("div", { className: "oui-flex oui-items-center", children: [
          /* @__PURE__ */ jsxRuntime.jsxs("div", { className: "oui-flex oui-flex-1 oui-flex-col", children: [
            /* @__PURE__ */ jsxRuntime.jsx("div", { className: "oui-text-xs oui-font-semibold oui-text-base-contrast-54", children: "Current total prize pool" }),
            /* @__PURE__ */ jsxRuntime.jsx("div", { children: /* @__PURE__ */ jsxRuntime.jsx(
              ui.Text.gradient,
              {
                weight: "bold",
                ...
                className: ui.cn([
                  "oui-trading-leaderboard-title ",
                  isMobile ? "oui-text-[20px] oui-leading-[28px]" : "oui-text-[32px] oui-leading-[28px]"
                ]),
                children: totalPrizePool ? formatPrizeAmount(
                  totalPrizePool.amount,
                  totalPrizePool.currency
          ...
          (ticketPrizePool || highlightPool) && /* @__PURE__ */ jsxRuntime.jsxs("div", { className: "oui-flex oui-flex-1 oui-flex-col", children: [
            /* @__PURE__ */ jsxRuntime.jsx("div", { className: "oui-text-xs oui-font-semibold oui-text-base-contrast-54", children: highlightPool?.label || "Raffle prize" }),
            /* @__PURE__ */ jsxRuntime.jsx("div", { children: /* @__PURE__ */ jsxRuntime.jsx(
              ui.Text.gradient,
              {
                weight: "bold",
        ...
        /* @__PURE__ */ jsxRuntime.jsx("div", { className: "oui-mb-3 oui-mt-4", children: /* @__PURE__ */ jsxRuntime.jsx(
        ...
        /* @__PURE__ */ jsxRuntime.jsxs(
          "div",
          {
            ...
            children: [
              isMobile && campaign?.rule_url && /* @__PURE__ */ jsxRuntime.jsx("div", { className: "oui-w-full", children: /* @__PURE__ */ jsxRuntime.jsx(
                ui.Button,
                {
                  fullWidth: true,
                  size: "md",
...
var TradingVolumeAndParticipation = ({
  onLearnMore,
  tradingVolume,
  statistics
}) => {
  ...
  return /* @__PURE__ */ jsxRuntime.jsxs("div", { className: "oui-flex oui-items-center oui-text-2xs oui-font-semibold oui-text-base-contrast-36", children: [
    /* @__PURE__ */ jsxRuntime.jsx("div", { children: t("tradingLeaderboard.tradingVolume") }),
    /* @__PURE__ */ jsxRuntime.jsx("div", { className: "oui-ml-1 oui-mr-2", children: /* @__PURE__ */ jsxRuntime.jsx(ui.Text.gradient, { color: "brand", children: /* @__PURE__ */ jsxRuntime.jsx(ui.Text.numeral, { dp: 2, prefix: "$", children: tradingVolume }) }) }),
    /* @__PURE__ */ jsxRuntime.jsx("div", { children: t("tradingLeaderboard.participants") }),
    /* @__PURE__ */ jsxRuntime.jsx("div", { className: "oui-ml-1 oui-text-base-contrast-54", children: /* @__PURE__ */ jsxRuntime.jsx(ui.Text.numeral, { dp: 0, children: statistics?.total_participants }) }),
    /* @__PURE__ */ jsxRuntime.jsxs(
      "div",
      {
...
var PricePoolProgress = ({
  ...
  return /* @__PURE__ */ jsxRuntime.jsxs(
    "div",
    {
      ...
      children: [
        /* @__PURE__ */ jsxRuntime.jsx("div", { className: "oui-absolute oui-flex oui-h-6 oui-w-[calc(100%-64px)] oui-items-center", children: /* @__PURE__ */ jsxRuntime.jsxs(
          "div",
          {
            className: ui.cn([
                  ...
                  children: [
                    /* @__PURE__ */ jsxRuntime.jsx(
                      "div",
                      {
                        className: ui.cn(
                          ...
                          "-oui-translate-x-1/2",
                          isReached ? "oui-text-base-contrast" : "oui-text-base-contrast-36"
                        ),
                        style: { left },
                        children: `>${formatTradingVolume(prizePool[0].volume_limit || 0, 0)}`
                      }
                    ),
                    /* @__PURE__ */ jsxRuntime.jsx(
                      "div",
                    ...
                    /* @__PURE__ */ jsxRuntime.jsx(
                      "div",
                      {
                        ...
                        style: { left },
                        children: isCurrentTier ? /* @__PURE__ */ jsxRuntime.jsx(ui.Text.gradient, { color: "brand", weight: "bold", children: /* @__PURE__ */ jsxRuntime.jsx(ui.Text.numeral, { dp: 0, prefix: "$", children: prizePool[0].total_prize }) }) : /* @__PURE__ */ jsxRuntime.jsx(
                          ui.Text.numeral,
                          {
                            dp: 0,
                            prefix: "$",
                            ...
                            children: prizePool[0].total_prize
                          }
                        )
                      }
                    )
...
var usePricePoolScript = (campaign) => {
  const { state } = hooks.useAccount();
  const ticketPrizePool = react.useMemo(
    () => getTicketPrizePool(campaign),
    [campaign]
  );
  const highlightPool = react.useMemo(() => {
...
var PricePoolWidget = (props) => {
  const state = usePricePoolScript(props.campaign);
  return /* @__PURE__ */ jsxRuntime.jsx("div", { children: /* @__PURE__ */ jsxRuntime.jsx(PricePoolDesktopUI, { ...state, ...props }) });
};
var CampaignsContentDesktopUI = ({
  campaign,
  statistics,
  onLearnMore,
  onTradeNow,
  ...
  const showTradeButton = react.useMemo(() => {
    return !shouldShowJoinButton && (/* @__PURE__ */ new Date()).toISOString() < campaign.end_time;
  }, [shouldShowJoinButton, campaign.end_time]);
  const isCampaignStarted = react.useMemo(() => {
    return campaign.start_time < (/* @__PURE__ */ new Date()).toISOString();
  }, [campaign]);
  const highlightPool = react.useMemo(() => {
    if (!campaign?.highlight_pool_id) {
      return null;
  ...
  const showTieredPricePool = react.useMemo(() => {
    return campaign?.tiered_prize_pools && campaign?.tiered_prize_pools.length > 0;
  }, [campaign]);
  const renderContent = () => {
    return /* @__PURE__ */ jsxRuntime.jsxs(jsxRuntime.Fragment, { children: [
      /* @__PURE__ */ jsxRuntime.jsx(
  ...
  return /* @__PURE__ */ jsxRuntime.jsxs(
    "div",
    {
      ...
      children: [
            ...
            children: [
              /* @__PURE__ */ jsxRuntime.jsx(
                ui.Text,
                {
                  size: "sm",
                  weight: "semibold",
                  className: ui.cn(["oui-text-base-contrast-54", classNames?.time]),
                  children: dateRange
                }
              ),
              renderContent()
        ...
        showTieredPricePool ? /* @__PURE__ */ jsxRuntime.jsx(
          PricePoolWidget,
          {
            onLearnMore,
            onTradeNow,
...
var useCampaignsScript = () => {
  ...
  const isCampaignEnded = react.useMemo(() => {
    return currentCampaign?.end_time && currentCampaign?.end_time < (/* @__PURE__ */ new Date()).toISOString();
  }, [currentCampaign]);
  const { state } = hooks.useAccount();
  const { data: userCampaigns, mutate: refreshUserCampaigns } = hooks.useQuery(
    currentCampaignId !== "general" && state.address ? `https://api.orderly.org/v1/public/campaigns?address=${state.address}` : null,
    { revalidateOnFocus: false }
  );
  const isParticipated = react.useMemo(() => {
    const target = userCampaigns?.find((item) => item.id == currentCampaignId);
    return !!target;
  }, [userCampaigns, currentCampaignId]);
  const shouldShowJoinButton = react.useMemo(() => {
    return !!state.address && !isCampaignEnded && !isParticipated;
  }, [state.address, isCampaignEnded, isParticipated]);
  const [doJoinCampaign, { isMutating: isJoining, error: joinError }] = hooks.useMutation(`/v1/client/campaign/sign_up`, "POST");
  const joinCampaign = react.useCallback(
    async (data) => {
      try {
        if (state.status < types.AccountStatusEnum.EnableTrading) {
          ui.toast.error("Please enable trading to proceed.");
          return;
        }
        ...
        if (result?.success !== false) {
          ...
          ui.toast.success(result?.message || "Campaign joined successfully");
          return result;
        } else {
          ui.toast.error(result?.message || "Failed to join campaign");
        }
      } catch (error) {
        throw error;
      }
  ...
  const onLearnMore = () => {
    if (currentCampaign?.rule_url) {
      if (currentCampaign.rule_config?.action === "scroll") {
        document.getElementById(currentCampaign.rule_url)?.scrollIntoView({ behavior: "smooth" });
      } else {
  ...
  const canTrade = react.useMemo(() => {
    ...
    return currentCampaign?.start_time < (/* @__PURE__ */ new Date()).toISOString() && currentCampaign?.end_time > (/* @__PURE__ */ new Date()).toISOString() && state.status >= types.AccountStatusEnum.EnableTrading;
  ...
  const totalPrizePool = react.useMemo(() => {
    return getTotalPrizePool(currentCampaign, tieredIndex ?? void 0);
  }, [currentCampaign, tieredIndex]);
  return {
    campaigns,
    currentCampaignId,
...
var CampaignsAxis = ({ points }) => {
  if (!points || points.length === 0) {
    return null;
  }
  const activeIndex = points.findIndex((point) => point.type === "active");
  if (points.length === 1) {
    return /* @__PURE__ */ jsxRuntime.jsx("div", { className: "oui-flex oui-w-full oui-justify-center", children: /* @__PURE__ */ jsxRuntime.jsxs("div", { className: "oui-flex oui-flex-col oui-items-center", children: [
      /* @__PURE__ */ jsxRuntime.jsx(AxisPoint, { type: points[0].type }),
      /* @__PURE__ */ jsxRuntime.jsxs("div", { className: "oui-mt-4 oui-flex oui-flex-col oui-items-center oui-text-center", children: [
        /* @__PURE__ */ jsxRuntime.jsx("div", { className: "oui-trading-leaderboard-title oui-mb-1 oui-whitespace-nowrap oui-text-sm oui-font-medium oui-text-base-contrast-54", children: points[0].title }),
        /* @__PURE__ */ jsxRuntime.jsx("div", { className: "oui-whitespace-nowrap oui-text-xs oui-text-base-contrast-36", children: points[0].time })
      ] })
    ] }) });
  }
  const getLineBackgroundClass = (segmentIndex) => {
  ...
  return /* @__PURE__ */ jsxRuntime.jsx("div", { className: "oui-flex oui-w-full", style: { width: widthPercentage }, children: points.map((point, index) => {
    ...
    return /* @__PURE__ */ jsxRuntime.jsxs(
      "div",
      {
        ...
        children: [
          /* @__PURE__ */ jsxRuntime.jsxs("div", { className: "oui-relative oui-flex oui-w-full oui-items-center", children: [
            /* @__PURE__ */ jsxRuntime.jsx(
              "div",
              {
                className: ui.cn([
            ...
            /* @__PURE__ */ jsxRuntime.jsx("div", { className: "oui-z-10 oui-shrink-0", children: /* @__PURE__ */ jsxRuntime.jsx(AxisPoint, { type: point.type }) }),
            /* @__PURE__ */ jsxRuntime.jsx(
              "div",
              {
                className: ui.cn([
          ...
          /* @__PURE__ */ jsxRuntime.jsxs("div", { className: "oui-mt-4 oui-flex oui-flex-col oui-items-center oui-text-center", children: [
            /* @__PURE__ */ jsxRuntime.jsx("div", { className: "oui-trading-leaderboard-title oui-mb-1 oui-whitespace-nowrap oui-text-sm oui-font-medium oui-text-base-contrast-54", children: point.title }),
            /* @__PURE__ */ jsxRuntime.jsx("div", { className: "oui-whitespace-nowrap oui-text-xs oui-text-base-contrast-36", children: point.type !== "active" && point.time })
          ] })
        ]
      },
      index
...
var CampaignsAxisMobile = ({ points }) => {
  if (!points || points.length === 0) {
    return null;
  }
  const activeIndex = points.findIndex((point) => point.type === "active");
  ...
  return /* @__PURE__ */ jsxRuntime.jsx("div", { className: "oui-flex oui-w-full oui-flex-col oui-items-center oui-gap-10", children: points.map((point, index) => {
    const isFirst = index === 0;
    const isLast = index === points.length - 1;
    return /* @__PURE__ */ jsxRuntime.jsx("div", { className: ui.cn(["oui-relative"]), children: /* @__PURE__ */ jsxRuntime.jsxs("div", { className: "oui-flex oui-h-10 oui-items-start oui-gap-4", children: [
      /* @__PURE__ */ jsxRuntime.jsxs(
        "div",
        {
          className: ui.cn([
          ...
          children: [
            /* @__PURE__ */ jsxRuntime.jsx(AxisPoint, { type: point.type }),
      ...
      /* @__PURE__ */ jsxRuntime.jsxs("div", { className: "oui-flex oui-flex-col oui-justify-start", children: [
        /* @__PURE__ */ jsxRuntime.jsx("div", { className: "oui-trading-leaderboard-title oui-mb-1 oui-text-sm oui-font-medium oui-text-base-contrast-54", children: point.title }),
        /* @__PURE__ */ jsxRuntime.jsx("div", { className: "oui-min-w-[160px] oui-text-xs oui-text-base-contrast-36", children: point.type !== "active" && point.time })
      ] })
    ] }) }, index);
  }) });
};
      ...
      /* @__PURE__ */ jsxRuntime.jsx(
        "div",
        {
          className: ui.cn([
            ...
            isMobile ? "oui-text-[20px] oui-leading-[24px] oui-h-[24px]" : "oui-text-[36px] oui-leading-[44px] oui-h-[44px]"
          ]),
      ...
      /* @__PURE__ */ jsxRuntime.jsx(
        "div",
        {
          className: ui.cn([
            ...
            isMobile ? "oui-text-[10px] oui-leading-[14px] oui-h-[14px]" : "oui-text-sm oui-leading-[20px] oui-h-[20px]"
          ]),
          children: label
        }
      )
...
var CampaignsCountdown = ({ campaign, isMobile }) => {
  const { t } = i18n.useTranslation();
  ...
  react.useEffect(() => {
    const calculateTimeLeft = () => {
      ...
      const endTime2 = new Date(campaign.end_time).getTime();
      const hasStarted = currentTime2 >= startTime;
      setIsStarted(hasStarted);
      const targetTime = hasStarted ? endTime2 : startTime;
      const difference = targetTime - currentTime2;
      if (difference > 0) {
  ...
  if (currentTime > endTime) {
    return /* @__PURE__ */ jsxRuntime.jsxs("div", { className: "oui-w-full oui-flex oui-items-center oui-justify-center oui-gap-4", children: [
      /* @__PURE__ */ jsxRuntime.jsx(
        "div",
        {
      ...
      /* @__PURE__ */ jsxRuntime.jsx(
        "div",
        {
          className: ui.cn([
            ...
            isMobile ? "oui-text-[14px] oui-leading-[20px] oui-h-[20px] oui-whitespace-nowrap" : "oui-p-5 oui-text-[18px] oui-leading-[26px] oui-h-[26px]"
          ]),
          children: t("tradingLeaderboard.batteleHasEnded")
        }
      ),
      /* @__PURE__ */ jsxRuntime.jsx(
        "div",
  ...
  const titleText = isStarted ? t("tradingLeaderboard.battleEndsIn") : t("tradingLeaderboard.battleStartsIn");
  const timeUnits = [
        ...
        children: [
          /* @__PURE__ */ jsxRuntime.jsx(
            "div",
            {
              className: ui.cn([
                ...
                isMobile ? "oui-text-[14px] oui-leading-[20px] oui-h-[20px]" : "oui-text-[18px] oui-leading-[26px] oui-h-[26px]"
              ]),
              children: titleText
            }
          ),
          /* @__PURE__ */ jsxRuntime.jsx(
            "div",
            {
              ...
              children: timeUnits.map((unit, index) => /* @__PURE__ */ jsxRuntime.jsxs(
                "div",
                {
                  className: ui.cn([
                    "oui-flex oui-items-center",
                  ...
                  children: [
                    ...
                    index < timeUnits.length - 1 && /* @__PURE__ */ jsxRuntime.jsx(Circle, {})
                  ]
                },
                unit.label
              ))
...
var CampaignsTimeDesktopUI = ({ campaign, isMobile }) => {
  ...
  return /* @__PURE__ */ jsxRuntime.jsxs(
    "div",
    {
      ...
      children: [
        /* @__PURE__ */ jsxRuntime.jsx(CampaignsCountdown, { campaign, isMobile }),
        isMobile ? /* @__PURE__ */ jsxRuntime.jsx(CampaignsAxisMobile, { points: timelineData }) : /* @__PURE__ */ jsxRuntime.jsx(CampaignsAxis, { points: timelineData })
      ]
    }
  );
};
var DefaultCampaign = ({ currentCampaignId, onCampaignChange, style, className }) => {
  const { t } = i18n.useTranslation();
  return /* @__PURE__ */ jsxRuntime.jsx(
    "div",
    {
      className: ui.cn([
        ...
        currentCampaignId === "general" ? "oui-border-[rgba(var(--oui-gradient-brand-start))]" : "oui-border-transparent",
        className
      ]),
      style,
      onClick: () => onCampaignChange("general"),
      children: t("tradingLeaderboard.generalLeaderboard")
    }
  );
};
var CampaignItemUI = ({
  ...
  return /* @__PURE__ */ jsxRuntime.jsxs(
    "div",
    {
      ...
      children: [
        /* @__PURE__ */ jsxRuntime.jsx(CampaignTag, { tag, active, classNames: classNames?.tag }),
        /* @__PURE__ */ jsxRuntime.jsx(PriceTag, { price: formattedPrice, active }),
        /* @__PURE__ */ jsxRuntime.jsx(
          "div",
          {
            className: ui.cn([
        ...
        /* @__PURE__ */ jsxRuntime.jsxs(
          "div",
          {
            ...
            children: [
              ...
              campaign.referral_codes && /* @__PURE__ */ jsxRuntime.jsx(
                ui.Text,
                {
                  className: ui.cn([
                    "oui-text-base-contrast-54",
                  ...
                  children: Array.isArray(campaign.referral_codes) ? campaign.referral_codes.join(",") : campaign.referral_codes
                }
              )
            ]
          }
...
var CampaignTag = ({ tag, active, classNames }) => {
  ...
  return /* @__PURE__ */ jsxRuntime.jsx(
    "div",
    {
      ...
      children: tag === "ended" /* ENDED */ ? /* @__PURE__ */ jsxRuntime.jsx(
        ui.Text,
        {
          size: "2xs",
          weight: "semibold",
          className: ui.cn(["oui-text-base-contrast-54", classNames?.text]),
          children: tagText
        }
      ) : /* @__PURE__ */ jsxRuntime.jsx(
        ui.Text.gradient,
...
var PriceTag = ({ price, active }) => {
  return /* @__PURE__ */ jsxRuntime.jsxs(
    "div",
    {
      ...
      children: [
        /* @__PURE__ */ jsxRuntime.jsx(LeaderboardTotalPrice, {}),
        /* @__PURE__ */ jsxRuntime.jsx(
          ui.Text,
          {
            size: "2xs",
...
var CampaignsHeaderUI = ({ campaigns, currentCampaignId, onCampaignChange, backgroundSrc }) => {
  ...
  const updateScrollAvailability = react.useCallback(() => {
  ...
  const slideStyle = isLargeScreen ? { flexBasis: "calc((100% - 2rem) / 4)", width: "calc((100% - 2rem) / 4)" } : {
    flexBasis: "calc((100% - 1rem) / 3)",
    width: "calc((100% - 1rem) / 3)"
  };
  const shouldHideScrollButtons = isLargeScreen ? campaigns.length <= 4 : campaigns.length <= 3;
  return /* @__PURE__ */ jsxRuntime.jsxs("div", { className: "oui-flex oui-w-full oui-items-center oui-gap-2 oui-px-6", children: [
    /* @__PURE__ */ jsxRuntime.jsx("div", { className: "oui-shrink-0", children: /* @__PURE__ */ jsxRuntime.jsx(
      DefaultCampaign,
      {
        currentCampaignId,
        onCampaignChange,
    ...
    /* @__PURE__ */ jsxRuntime.jsx("div", { className: "oui-min-w-0 oui-flex-1 oui-overflow-hidden", children: /* @__PURE__ */ jsxRuntime.jsx("div", { ref: emblaRef, children: /* @__PURE__ */ jsxRuntime.jsx("div", { className: "oui-flex oui-gap-2", children: campaigns?.map((campaign) => /* @__PURE__ */ jsxRuntime.jsx(
      "div",
      {
...
var CampaignsHeaderMobileUI = react.memo(({ campaigns, currentCampaignId, onCampaignChange, backgroundSrc }) => {
  const [canScrollPrev, setCanScrollPrev] = react.useState(false);
  const [canScrollNext, setCanScrollNext] = react.useState(false);
  ...
  const updateScrollAvailability = react.useCallback(() => {
    ...
    if (selectedCampaign) {
      const campaignId = selectedCampaign.isDefault ? "general" : String(selectedCampaign.campaign_id);
  ...
  const scrollToCurrentCampaign = react.useCallback(() => {
    if (!emblaApi || !campaigns?.length) {
      return;
    }
  ...
  return /* @__PURE__ */ jsxRuntime.jsxs("div", { className: "oui-flex oui-w-full oui-items-center oui-gap-2 oui-px-3", children: [
    /* @__PURE__ */ jsxRuntime.jsx(
      "button",
      {
        onClick: scrollPrev,
    ...
    /* @__PURE__ */ jsxRuntime.jsx("div", { className: "oui-min-w-0 oui-flex-1 oui-overflow-hidden", children: /* @__PURE__ */ jsxRuntime.jsx("div", { ref: emblaRef, children: /* @__PURE__ */ jsxRuntime.jsxs("div", { className: "oui-flex oui-gap-2", children: [
      /* @__PURE__ */ jsxRuntime.jsx(
        DefaultCampaign,
        {
          currentCampaignId,
      ...
      campaigns?.map((campaign) => /* @__PURE__ */ jsxRuntime.jsx(
        "div",
        {
          className: "oui-shrink-0",
          style: slideStyle,
...
var CampaignsHeaderWidget = (props) => {
  const { isMobile } = ui.useScreen();
  return isMobile ? /* @__PURE__ */ jsxRuntime.jsx(CampaignsHeaderMobileUI, { ...props }) : /* @__PURE__ */ jsxRuntime.jsx(CampaignsHeaderUI, { ...props });
};
var CampaignsWidget = (props) => {
  ...
  const contentClassNames = react.useMemo(() => {
    ...
    return {
      container: ui.cn("oui-h-[400px] oui-gap-5"),
      time: "oui-text-sm oui-h-5",
      title: "oui-text-[24px] oui-leading-[32px]",
      description: "oui-text-2xs oui-leading-[15px]",
      topContainer: ui.cn("oui-w-[284px] oui-gap-1")
    };
  }, [isMobile, canTrade]);
  return /* @__PURE__ */ jsxRuntime.jsxs(
    "div",
    {
      className: ui.cn(["oui-relative oui-z-[1] oui-overflow-hidden"]),
      style: props.style,
      children: [
        !props.hideCampaignsBanner && /* @__PURE__ */ jsxRuntime.jsx(
        ...
        state.currentCampaign && /* @__PURE__ */ jsxRuntime.jsx(
...
var RewardsDesktopUI = ({
  ...
  const estimatedRewards = campaign && currentUserData ? calculateEstimatedRewards(currentUserData, campaign) : null;
  const estimatedTickets = campaign?.ticket_rules && currentUserData ? calculateEstimatedTickets(currentUserData, campaign.ticket_rules) : 0;
  estimatedRewards ? formatRewardAmount(estimatedRewards.amount, estimatedRewards.currency) : "0 USDC";
  ...
  const ticketTooltipContent = react.useMemo(() => {
    ...
    if (ticketRules.mode === "linear") {
      return /* @__PURE__ */ jsxRuntime.jsx("div", { children: t("tradingLeaderboard.earnTickets", {
        ticket: ticketRules?.linear?.tickets,
        amount: utils.commify(ticketRules?.linear?.every || 0)
      }) });
    }
    return /* @__PURE__ */ jsxRuntime.jsx("div", { className: "oui-flex oui-min-w-[240px] oui-flex-col oui-gap-1", children: ticketRules?.tiers?.map((tier) => {
      return /* @__PURE__ */ jsxRuntime.jsxs(
        "div",
        {
          ...
          children: [
            ...
            /* @__PURE__ */ jsxRuntime.jsxs("div", { className: "oui-text-base-contrast", children: [
              tier.tickets,
              " tickets"
            ] })
          ]
      ...
      children: [
        ...
        /* @__PURE__ */ jsxRuntime.jsxs(
          "div",
          {
            ...
            children: [
              campaign?.rule_url && /* @__PURE__ */ jsxRuntime.jsx(
                ui.Button,
                {
                  size: isMobile ? "md" : "lg",
                  variant: "outlined",
              ...
              shouldShowJoinButton && /* @__PURE__ */ jsxRuntime.jsx(
                ui.Button,
                {
                  ...
                  loading: isJoining,
                  disabled: isJoining,
                  className: ui.cn([isMobile ? "oui-flex-1" : "oui-w-[140px]"]),
                  onClick: () => joinCampaign?.({ campaign_id: Number(campaign?.campaign_id) }),
                  children: t("tradingLeaderboard.joinNow")
                }
              ),
              !shouldShowJoinButton && canTrade && /* @__PURE__ */ jsxRuntime.jsx(
                ui.Button,
                {
                  size: isMobile ? "md" : "lg",
                  variant: "gradient",
                  ...
                  className: ui.cn([isMobile ? "oui-flex-1" : "oui-w-[140px]"]),
                  onClick: onTradeNow,
                  children: t("tradingLeaderboard.tradeNow")
                }
              )
            ]
          }
...
var RewardItem = (props) => {
  ...
  return /* @__PURE__ */ jsxRuntime.jsxs("div", { className: "oui-flex oui-flex-1 oui-flex-col oui-items-center oui-justify-center oui-rounded-2xl oui-bg-base-9 oui-px-5 oui-py-4", children: [
    /* @__PURE__ */ jsxRuntime.jsx(
      "div",
      {
        className: ui.cn([
          "oui-text-base-contrast-54",
          props.isMobile ? "oui-h-[15px] oui-text-2xs oui-leading-[15px]" : "oui-text-sm oui-font-semibold"
        ]),
    ...
    /* @__PURE__ */ jsxRuntime.jsxs("div", { className: "oui-flex oui-items-center oui-gap-2", children: [
      /* @__PURE__ */ jsxRuntime.jsx(
        ui.Text.gradient,
        {
          ...
          className: ui.cn([
            "oui-trading-leaderboard-title",
            props.isMobile ? "oui-h-[16px] oui-text-base oui-leading-[16px]" : "oui-h-10 oui-text-[32px] oui-leading-10"
          ]),
          children: props.value
        }
      ),
      props.showTooltip && /* @__PURE__ */ jsxRuntime.jsx(ui.Tooltip, { content: props.tooltip, ...tooltipProps, children: /* @__PURE__ */ jsxRuntime.jsx(
        "div",
        {
          className: "oui-flex oui-size-4 oui-items-center oui-justify-center",
    ...
    props.showExtraInfo && /* @__PURE__ */ jsxRuntime.jsx(
      "div",
      {
        className: ui.cn([
        ...
        children: /* @__PURE__ */ jsxRuntime.jsxs("div", { className: "oui-flex oui-flex-col oui-items-center oui-gap-1 oui-text-2xs oui-font-semibold oui-text-base-contrast-36", children: [
          /* @__PURE__ */ jsxRuntime.jsx(
            "div",
            {
              className: ui.cn([
          ...
          /* @__PURE__ */ jsxRuntime.jsx(
            "div",
            {
              ...
              children: props.extraInfo.atMax ? /* @__PURE__ */ jsxRuntime.jsx(i18n.Trans, { i18nKey: "tradingLeaderboard.maxTicketsAchieved" }) : /* @__PURE__ */ jsxRuntime.jsx(
                i18n.Trans,
                {
                  i18nKey: "tradingLeaderboard.tradeForMoreTickets",
                  components: [
                    /* @__PURE__ */ jsxRuntime.jsxs("span", { className: "oui-text-base-contrast", children: [
                      "$",
...
var getListStyleClass = (style) => {
  const styleMap = {
    disc: "oui-list-disc",
    decimal: "oui-list-decimal",
    none: "oui-list-none",
    circle: "oui-list-disc",
    // Tailwind doesn't have circle, fallback to disc
    square: "oui-list-disc",
    // Tailwind doesn't have square, fallback to disc
    "decimal-leading-zero": "oui-list-decimal",
    "lower-alpha": "oui-list-decimal",
    // Fallback
    "upper-alpha": "oui-list-decimal",
    // Fallback
    "lower-roman": "oui-list-decimal",
    // Fallback
    "upper-roman": "oui-list-decimal"
    // Fallback
  };
  return styleMap[style] || "oui-list-disc";
};
var parseRichText = (text) => {
  const parts = [];
  let currentIndex = 0;
  let key = 0;
  const boldRegex = /\*\*(.*?)\*\*/g;
  ...
  for (const { type, match: match2 } of allMatches) {
    if (match2.index !== void 0 && match2.index > currentIndex) {
      const beforeText = text.slice(currentIndex, match2.index);
      if (beforeText) {
        parts.push(/* @__PURE__ */ jsxRuntime.jsx("span", { children: beforeText }, key++));
      }
    }
    switch (type) {
      case "bold":
    ...
    currentIndex = (match2.index ?? 0) + match2[0].length;
  }
  if (currentIndex < text.length) {
    const remainingText = text.slice(currentIndex);
    if (remainingText) {
      parts.push(/* @__PURE__ */ jsxRuntime.jsx("span", { children: remainingText }, key++));
    }
  }
  return parts.length > 0 ? parts : [/* @__PURE__ */ jsxRuntime.jsx("span", { children: text }, 0)];
};
var DescriptionContent = ({ description, config }) => {
  const renderContent = (contents, level = 0) => {
    ...
    if (!hasListItems && contents.length === 1) {
      ...
      return /* @__PURE__ */ jsxRuntime.jsx("div", { className: ui.cn("oui-mb-2", content.className), children: renderContentItem(content) });
    }
    return /* @__PURE__ */ jsxRuntime.jsx(
      "ul",
      {
          ...
          return /* @__PURE__ */ jsxRuntime.jsxs(
            "li",
            {
              ...
              children: [
                ...
                content?.children?.length && /* @__PURE__ */ jsxRuntime.jsx("div", { className: "oui-mt-2 oui-text-sm oui-text-base-contrast-36", children: renderContent(content.children, level + 1) })
              ]
            },
  ...
  const renderContentItem = (content) => {
    ...
    return /* @__PURE__ */ jsxRuntime.jsx("div", { className: "rich-text-content inline", children: parseRichText(content.content) });
  };
  return /* @__PURE__ */ jsxRuntime.jsx(
    "div",
    {
      ...
      children: renderContent(description)
    }
  );
};
var CampaignRuleUI = ({
  ...
  return /* @__PURE__ */ jsxRuntime.jsxs(
    "div",
    {
      className: ui.cn("oui-mx-auto oui-max-w-[992px] oui-py-10", className),
      id,
      children: [
        /* @__PURE__ */ jsxRuntime.jsx(
          LeaderboardTitle,
        ...
        /* @__PURE__ */ jsxRuntime.jsx(DescriptionContent, { description: rules || [], config: ruleConfig })
      ]
    }
  );
};
var CampaignTermsUI = ({
  ...
  const { t } = i18n.useTranslation();
  return /* @__PURE__ */ jsxRuntime.jsxs("div", { className: ui.cn("oui-mx-auto oui-max-w-[992px] oui-py-10", className), children: [
    /* @__PURE__ */ jsxRuntime.jsx(
      LeaderboardTitle,
      {
        isMobile,
    ...
    /* @__PURE__ */ jsxRuntime.jsx(DescriptionContent, { description: termsConfig || types.EMPTY_LIST })
  ] });
};
var RuleWidget = () => {
  const state = useCampaignsScript();
  ...
  if (state.currentCampaignId === "general" || !state.currentCampaign?.rule_config) {
    return null;
  }
  return /* @__PURE__ */ jsxRuntime.jsxs(jsxRuntime.Fragment, { children: [
    /* @__PURE__ */ jsxRuntime.jsx(
...
var LeaderboardPage = (props) => {
  const { hideCampaignsBanner, ...rest } = props;
  return /* @__PURE__ */ jsxRuntime.jsx(TradingLeaderboardProvider, { ...rest, children: /* @__PURE__ */ jsxRuntime.jsxs(
    "div",
    {
      style: {
      ...
      children: [
        ...
        /* @__PURE__ */ jsxRuntime.jsx(RewardsWidget, {}),
        /* @__PURE__ */ jsxRuntime.jsx(LeaderboardSection, { ...props }),
        /* @__PURE__ */ jsxRuntime.jsx(RuleWidget, {})
      ]
    }
  ) });
};
var LeaderboardSection = (props) => {
  const { t } = i18n.useTranslation();
  const { isMobile } = ui.useScreen();
  ...
  if (currentCampaign && currentCampaignId != "general") {
    return /* @__PURE__ */ jsxRuntime.jsxs(ui.Box, { px: 3, children: [
      ...
      /* @__PURE__ */ jsxRuntime.jsx(CampaignLeaderboardWidget, { ...props, campaignId: currentCampaignId })
    ] });
  }
  return null;
};
var LeaderboardTitle = (props) => {
  return /* @__PURE__ */ jsxRuntime.jsxs(
    ui.Flex,
    {
        ...
        /* @__PURE__ */ jsxRuntime.jsx(
          ui.Text,
          {
            className: ui.cn(
              ...
              props.isMobile ? "oui-text-[20px] oui-leading-[24px]" : "oui-text-[32px] oui-leading-[44px]"
            ),
            children: props.title
          }
        ),
...
exports.CampaignLeaderboardWidget = CampaignLeaderboardWidget;
exports.CampaignRankingWidget = CampaignRankingWidget;
exports.CampaignTagEnum = CampaignTagEnum;
exports.CampaignsHeaderWidget = CampaignsHeaderWidget;
exports.GeneralLeaderboard = GeneralLeaderboard;
exports.GeneralLeaderboardWidget = GeneralLeaderboardWidget;
exports.GeneralRankingWidget = GeneralRankingWidget;
exports.LeaderboardBackground = LeaderboardBackground;
exports.LeaderboardPage = LeaderboardPage;
exports.LeaderboardSection = LeaderboardSection;
exports.LeaderboardTitle = LeaderboardTitle;
exports.TradingLeaderboardContext = TradingLeaderboardContext;
exports.TradingLeaderboardProvider = TradingLeaderboardProvider;
exports.useCampaignLeaderboardScript = useCampaignLeaderboardScript;
exports.useCampaignRankingScript = useCampaignRankingScript;
exports.useGeneralLeaderboardScript = useGeneralLeaderboardScript;
exports.useGeneralRankingScript = useGeneralRankingScript;
exports.useTradingLeaderboardContext = useTradingLeaderboardContext;
