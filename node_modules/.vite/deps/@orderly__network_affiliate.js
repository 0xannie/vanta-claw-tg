import __buffer_polyfill from 'vite-plugin-node-polyfills/shims/buffer'
globalThis.Buffer = globalThis.Buffer || __buffer_polyfill
import __global_polyfill from 'vite-plugin-node-polyfills/shims/global'
globalThis.global = globalThis.global || __global_polyfill
import __process_polyfill from 'vite-plugin-node-polyfills/shims/process'
globalThis.process = globalThis.process || __process_polyfill

import {
  VolBarChart
} from "./chunk-2HWDSNVI.js";
import "./chunk-XKDBZXIP.js";
import {
  Scaffold,
  SideBar,
  useScaffoldContext
} from "./chunk-J6DTPFJE.js";
import {
  AuthGuard,
  AuthGuardDataTable,
  AuthStatusEnum,
  useAuthStatus
} from "./chunk-24DGMYBJ.js";
import {
  useAppContext
} from "./chunk-FQFTCGNL.js";
import {
  cleanStringStyle,
  noCacheConfig,
  useAccount,
  useCheckReferralCode,
  useDaily,
  useLocalStorage,
  useMediaQuery,
  useMemoizedFn,
  useMutation,
  usePrivateQuery,
  useRefereeInfo,
  useRefereeRebateSummary,
  useReferralRebateSummary
} from "./chunk-LKGTCLMW.js";
import "./chunk-IRK6YIG5.js";
import "./chunk-XOXN5XQJ.js";
import "./chunk-XPCMXA3P.js";
import "./chunk-DVJCRTEZ.js";
import "./chunk-E4THU5TJ.js";
import "./chunk-MV2QDQSH.js";
import "./chunk-6FN6GMXX.js";
import "./chunk-AFMN6TAI.js";
import "./chunk-5NXOGS6W.js";
import "./chunk-US3ISA6J.js";
import "./chunk-56IKAB2G.js";
import "./chunk-SR5T5QTJ.js";
import "./chunk-VTIMQKLI.js";
import "./chunk-XIZJ7GIP.js";
import "./chunk-25DLSVN2.js";
import "./chunk-ZBGMNZ4G.js";
import "./chunk-QEPTV7FP.js";
import "./chunk-UQ76QKZV.js";
import "./chunk-L2YYEYH6.js";
import "./chunk-KVMB3J3O.js";
import "./chunk-WSIE5WWB.js";
import "./chunk-VSFRA4CO.js";
import "./chunk-Y3BGTO6U.js";
import "./chunk-MTN4OBPL.js";
import "./chunk-P2NSRJ6F.js";
import "./chunk-DYEGVO7E.js";
import "./chunk-FDCKJXHS.js";
import "./chunk-UI66QAET.js";
import "./chunk-TFQH4EAC.js";
import "./chunk-4E5CUZQE.js";
import "./chunk-YOPLTIGC.js";
import {
  Box,
  Button,
  Collapsible,
  CollapsibleContent2,
  CopyIcon,
  DataTable,
  DatePicker2,
  Dialog,
  DialogBody,
  DialogContent,
  DialogHeader,
  DialogTitle,
  DialogTrigger,
  Divider,
  Flex,
  Grid2,
  Input2,
  ListView,
  M,
  Select2,
  SimpleDialog,
  Slider,
  Spinner,
  Statistic,
  TabPanel,
  Tabs,
  TabsBase,
  TabsContent,
  TabsList,
  TabsTrigger,
  Text2,
  TextField,
  Tooltip,
  Vt,
  WarningIcon,
  format,
  formatAddress,
  formatter_exports,
  modal,
  parseNumber,
  registerSimpleDialog,
  registerSimpleSheet,
  subDays,
  toDate,
  useModal,
  usePagination,
  useScreen
} from "./chunk-PFNLW6VD.js";
import {
  AccountStatusEnum,
  EMPTY_LIST,
  commify,
  commifyOptional,
  decimal_default2 as decimal_default
} from "./chunk-665UVHU6.js";
import "./chunk-TQ2QBXKN.js";
import "./chunk-SKTD7REL.js";
import "./chunk-5Q5JT3TT.js";
import {
  Trans2 as Trans,
  i18n_default,
  useTranslation
} from "./chunk-HNOAOT7S.js";
import {
  require_jsx_runtime
} from "./chunk-VNIJ3NDK.js";
import {
  require_react
} from "./chunk-K46N5KBP.js";
import {
  __toESM,
  require_dist,
  require_dist2,
  require_dist3
} from "./chunk-XJ3ZXRPO.js";

// node_modules/@orderly.network/affiliate/dist/index.mjs
var import_dist = __toESM(require_dist(), 1);
var import_dist2 = __toESM(require_dist2(), 1);
var import_dist3 = __toESM(require_dist3(), 1);
var import_react = __toESM(require_react(), 1);
var import_jsx_runtime = __toESM(require_jsx_runtime(), 1);
var __defProp = Object.defineProperty;
var __export = (target, all) => {
  for (var name in all)
    __defProp(target, name, { get: all[name], enumerable: true });
};
var dashboard_exports = {};
__export(dashboard_exports, {
  AffiliatePage: () => AffiliatePage,
  DashboardPage: () => DashboardPage,
  HomePage: () => HomePage,
  TraderPage: () => TraderPage
});
var TabTypes = ((TabTypes2) => {
  TabTypes2["affiliate"] = "affiliate";
  TabTypes2["trader"] = "trader";
  return TabTypes2;
})(TabTypes || {});
var ReferralContext = (0, import_react.createContext)(
  {}
);
var useReferralContext = () => {
  return (0, import_react.useContext)(ReferralContext);
};
var useVolumePrerequisite = () => {
  return usePrivateQuery(
    "/v1/referral/multi_level/volume_prerequisite",
    {
      revalidateOnFocus: false,
      shouldRetryOnError: false
    }
  );
};
var useMaxRebateRate = () => {
  return usePrivateQuery(
    "/v1/referral/multi_level/max_rebate_rate",
    {
      revalidateOnFocus: false,
      shouldRetryOnError: false
    }
  );
};
var useMultiLevelRebateInfo = () => {
  return usePrivateQuery(
    "/v1/referral/multi_level/rebate_info",
    {
      revalidateOnFocus: false,
      shouldRetryOnError: false
    }
  );
};
var useMultiLevelStatistics = (time_range) => {
  return usePrivateQuery(
    `/v1/referral/multi_level/statistics?time_range=${time_range}`,
    {
      revalidateOnFocus: false,
      shouldRetryOnError: false
    }
  );
};
var useMultiLevelReferralData = () => {
  const { data: volumePrerequisite, isLoading: volumePrerequisiteLoading } = useVolumePrerequisite();
  const { data: maxRebateRateRes, isLoading: maxRebateRateLoading } = useMaxRebateRate();
  const {
    data: multiLevelRebateInfoRes,
    mutate: multiLevelRebateInfoMutate,
    isLoading: multiLevelRebateInfoLoading
  } = useMultiLevelRebateInfo();
  const isMultiLevelReferralUnlocked = volumePrerequisite && volumePrerequisite.current_volume >= volumePrerequisite.required_volume;
  const isMultiLevelEnabled = !!maxRebateRateRes;
  const maxRebateRate = maxRebateRateRes?.max_rebate_rate;
  const multiLevelRebateInfo = (0, import_react.useMemo)(() => {
    if (!multiLevelRebateInfoRes) return;
    const {
      default_referee_rebate_rate: referee_rebate_rate,
      max_rebate_rate
    } = multiLevelRebateInfoRes;
    const referrer_rebate_rate = new decimal_default(max_rebate_rate || 0).sub(referee_rebate_rate || 0).toNumber();
    return {
      ...multiLevelRebateInfoRes,
      referee_rebate_rate,
      referrer_rebate_rate
    };
  }, [multiLevelRebateInfoRes, maxRebateRate]);
  const isLoading = volumePrerequisiteLoading || maxRebateRateLoading || multiLevelRebateInfoLoading;
  return {
    volumePrerequisite,
    maxRebateRate,
    multiLevelRebateInfo,
    isMultiLevelEnabled,
    isMultiLevelReferralUnlocked,
    multiLevelRebateInfoMutate,
    isLoading
  };
};
var ReferralProvider = (props) => {
  const {
    becomeAnAffiliateUrl = "https://orderly.network/",
    learnAffiliateUrl = "https://orderly.network/",
    referralLinkUrl,
    chartConfig,
    overwrite,
    children,
    splashPage,
    onBecomeAnAffiliate,
    bindReferralCodeState,
    onLearnAffiliate,
    showReferralPage
  } = props;
  const { state } = useAccount();
  const {
    data,
    mutate: referralInfoMutate,
    isLoading
  } = usePrivateQuery("/v1/referral/info", {
    revalidateOnFocus: false,
    errorRetryCount: 3,
    ...noCacheConfig
  });
  const { data: generateCode, mutate: generateCodeMutate } = usePrivateQuery(
    "/v1/referral/auto_referral/progress",
    {
      revalidateOnFocus: false,
      errorRetryCount: 0,
      formatter: (data2) => {
        return {
          code: data2.auto_referral_code,
          requireVolume: data2.required_volume,
          completedVolume: data2.completed_volume
        };
      }
    }
  );
  const {
    volumePrerequisite,
    multiLevelRebateInfo,
    isMultiLevelEnabled,
    isMultiLevelReferralUnlocked,
    multiLevelRebateInfoMutate,
    maxRebateRate,
    isLoading: isMultiLevelLoading
  } = useMultiLevelReferralData();
  const [showHome, setShowHome] = (0, import_react.useState)(isLoading);
  (0, import_react.useEffect)(() => {
    setShowHome(true);
  }, [isLoading]);
  const { data: dailyVolume, mutate: dailyVolumeMutate } = useDaily({
    startDate: subDays(/* @__PURE__ */ new Date(), 1),
    endDate: subDays(/* @__PURE__ */ new Date(), 90)
  });
  const { data: volumeStatistics, mutate: volumeStatisticsMutate } = usePrivateQuery("/v1/volume/user/stats", {
    revalidateOnFocus: false
  });
  const isAffiliate = (0, import_react.useMemo)(() => {
    return (data?.referrer_info?.referral_codes?.length || 0) > 0;
  }, [data?.referrer_info]);
  const isTrader = (0, import_react.useMemo)(() => {
    return (data?.referee_info?.referer_code?.length || 0) > 0;
  }, [data?.referee_info]);
  const userVolume = (0, import_react.useMemo)(() => {
    const volume = {};
    if (dailyVolume && dailyVolume.length > 0) {
      const now = format(/* @__PURE__ */ new Date(), "yyyy-MM-dd");
      const index = dailyVolume.findIndex((item) => item.date === now);
      let oneDayVolume = 0;
      if (index !== -1) {
        oneDayVolume = dailyVolume[index].perp_volume;
      }
      volume["1d_volume"] = oneDayVolume;
    }
    if (volumeStatistics) {
      volume["7d_volume"] = volumeStatistics.perp_volume_last_7_days;
      volume["30d_volume"] = volumeStatistics.perp_volume_last_30_days;
      volume["all_volume"] = volumeStatistics.perp_volume_ltd;
    }
    return volume;
  }, [dailyVolume, volumeStatistics]);
  (0, import_react.useEffect)(() => {
    if (isAffiliate || isTrader) {
      setShowHome(false);
    }
  }, [isAffiliate, isTrader]);
  const memoMutate = useMemoizedFn(() => {
    volumeStatisticsMutate();
    dailyVolumeMutate();
    referralInfoMutate();
    generateCodeMutate();
  });
  (0, import_react.useEffect)(() => {
    const searchParams = new URLSearchParams(window.location.search);
    const refCode = searchParams.get("ref");
    if (refCode) {
      localStorage.setItem("referral_code", refCode);
    }
  }, []);
  const [tab, setTab] = (0, import_react.useState)(() => {
    const savedTab = localStorage.getItem("orderly_affiliate_dashboard_tab");
    return savedTab || "affiliate";
  });
  (0, import_react.useEffect)(() => {
    localStorage.setItem("orderly_affiliate_dashboard_tab", tab);
  }, [tab]);
  const { wrongNetwork, disabledConnect } = useAppContext();
  const lastStete = (0, import_react.useRef)(AccountStatusEnum.NotConnected);
  const timerRef = (0, import_react.useRef)(null);
  (0, import_react.useEffect)(() => {
    if (lastStete.current !== state.status) {
      lastStete.current = state.status;
      timerRef.current = setTimeout(() => {
        memoMutate();
      }, 1e3);
    }
    return () => {
      if (timerRef.current) {
        clearTimeout(timerRef.current);
      }
    };
  }, [memoMutate, state.status]);
  const memoizedValue = (0, import_react.useMemo)(() => {
    return {
      generateCode,
      showHome,
      referralInfo: data,
      isAffiliate,
      isTrader,
      tab,
      becomeAnAffiliateUrl,
      learnAffiliateUrl,
      referralLinkUrl,
      userVolume,
      dailyVolume,
      chartConfig,
      overwrite,
      isLoading: isLoading || isMultiLevelLoading,
      wrongNetwork,
      disabledConnect,
      setShowHome,
      setTab,
      mutate: memoMutate,
      onBecomeAnAffiliate,
      bindReferralCodeState,
      onLearnAffiliate,
      showReferralPage,
      splashPage,
      volumePrerequisite,
      multiLevelRebateInfo,
      isMultiLevelEnabled,
      isMultiLevelReferralUnlocked,
      multiLevelRebateInfoMutate,
      maxRebateRate
    };
  }, [
    becomeAnAffiliateUrl,
    chartConfig,
    dailyVolume,
    data,
    disabledConnect,
    generateCode,
    isAffiliate,
    isLoading,
    isTrader,
    learnAffiliateUrl,
    overwrite,
    referralLinkUrl,
    showHome,
    tab,
    userVolume,
    wrongNetwork,
    onBecomeAnAffiliate,
    bindReferralCodeState,
    onLearnAffiliate,
    showReferralPage,
    splashPage,
    memoMutate,
    volumePrerequisite,
    multiLevelRebateInfo,
    isMultiLevelEnabled,
    isMultiLevelReferralUnlocked,
    multiLevelRebateInfoMutate,
    maxRebateRate,
    isMultiLevelLoading
  ]);
  return (0, import_jsx_runtime.jsx)(ReferralContext.Provider, { value: memoizedValue, children });
};
var Faq = () => {
  const { t } = useTranslation();
  const faqItemsData = (0, import_react.useMemo)(() => {
    return [
      {
        id: "faq-1",
        question: t("affiliate.faq.question1"),
        answer: t("affiliate.faq.answer1")
      },
      {
        id: "faq-2",
        question: t("affiliate.faq.question2"),
        answer: t("affiliate.faq.answer2")
      },
      {
        id: "faq-3",
        question: t("affiliate.faq.question3"),
        answer: t("affiliate.faq.answer3")
      },
      {
        id: "faq-4",
        question: t("affiliate.faq.question4"),
        answer: t("affiliate.faq.answer4")
      },
      {
        id: "faq-5",
        question: t("affiliate.faq.question5"),
        answer: t("affiliate.faq.answer5")
      }
    ];
  }, [t]);
  const [expandedItems, setExpandedItems] = (0, import_react.useState)(/* @__PURE__ */ new Set());
  const toggleItem = (0, import_react.useCallback)((id) => {
    setExpandedItems((prev) => {
      const newSet = new Set(prev);
      if (newSet.has(id)) {
        newSet.delete(id);
      } else {
        newSet.add(id);
      }
      return newSet;
    });
  }, []);
  const faqItems = (0, import_react.useMemo)(() => {
    return faqItemsData.map((item) => ({
      ...item,
      expanded: expandedItems.has(item.id)
    }));
  }, [faqItemsData, expandedItems]);
  return (0, import_jsx_runtime.jsxs)(Box, { children: [
    (0, import_jsx_runtime.jsx)(Text2, { className: "oui-text-[32px]", children: t("affiliate.faq.title") }),
    (0, import_jsx_runtime.jsx)(Flex, { direction: "column", itemAlign: "stretch", gap: 0, children: faqItems.map((item, index) => (0, import_jsx_runtime.jsxs)(import_react.Fragment, { children: [
      (0, import_jsx_runtime.jsx)(
        Collapsible,
        {
          open: item.expanded,
          onOpenChange: () => toggleItem(item.id),
          children: (0, import_jsx_runtime.jsxs)(
            Flex,
            {
              direction: "column",
              className: "oui-cursor-pointer oui-py-4",
              itemAlign: "stretch",
              onClick: () => toggleItem(item.id),
              children: [
                (0, import_jsx_runtime.jsxs)(Flex, { justify: "between", itemAlign: "center", gap: 4, children: [
                  (0, import_jsx_runtime.jsx)(Flex, { gap: 2, itemAlign: "center", className: "oui-flex-1", children: (0, import_jsx_runtime.jsx)(Text2, { size: "2xl", weight: "semibold", children: `${index + 1}. ${item.question}` }) }),
                  (0, import_jsx_runtime.jsx)(ChevronIcon, { expanded: item.expanded })
                ] }),
                (0, import_jsx_runtime.jsx)(CollapsibleContent2, { children: (0, import_jsx_runtime.jsx)(
                  Box,
                  {
                    className: "oui-pt-3 oui-text-base-contrast-54",
                    dangerouslySetInnerHTML: { __html: item.answer }
                  }
                ) })
              ]
            }
          )
        }
      ),
      index < faqItems.length - 1 && (0, import_jsx_runtime.jsx)("div", { className: "oui-my-4 oui-h-px oui-bg-line-6" })
    ] }, item.id)) })
  ] });
};
var ChevronIcon = ({ expanded }) => (0, import_jsx_runtime.jsx)(
  "svg",
  {
    width: "24",
    height: "24",
    viewBox: "0 0 16 16",
    fill: "none",
    xmlns: "http://www.w3.org/2000/svg",
    className: `oui-transition-transform oui-duration-200 ${expanded ? "oui-rotate-180" : ""}`,
    children: (0, import_jsx_runtime.jsx)(
      "path",
      {
        d: "M4 6L8 10L12 6",
        stroke: "currentColor",
        strokeWidth: "2",
        strokeLinecap: "round",
        strokeLinejoin: "round",
        className: "oui-text-base-contrast-54"
      }
    )
  }
);
var useReferralCode = () => {
  const [doCreateReferralCode, { isMutating: isCreateMutating }] = useMutation(
    "/v1/referral/multi_level/claim_code"
  );
  const [doEditReferralCode, { isMutating: isEditMutating }] = useMutation(
    "/v1/referral/edit_referral_code"
  );
  const [doUpdateRebateRate, { isMutating: isUpdateRebateRateMutating }] = useMutation("/v1/referral/multi_level/rebate_rate/update");
  const [doResetRebateRate, { isMutating: isResetRebateRateMutating }] = useMutation("/v1/referral/multi_level/rebate_rate/set_default");
  const createReferralCode = async (params) => {
    return doCreateReferralCode(params);
  };
  const editReferralCode = async (params) => {
    return doEditReferralCode(params);
  };
  const updateRebateRate = async (params) => {
    return doUpdateRebateRate(params);
  };
  const resetRebateRate = async (params) => {
    return doResetRebateRate(params);
  };
  const isMutating = isCreateMutating || isEditMutating || isUpdateRebateRateMutating || isResetRebateRateMutating;
  return {
    createReferralCode,
    editReferralCode,
    updateRebateRate,
    resetRebateRate,
    isMutating
  };
};
var useReferralCodeFormScript = (options) => {
  const { type, referralCode, maxRebateRate, referrerRebateRate, accountId } = options;
  const { t } = useTranslation();
  const [newCode, setNewCode] = (0, import_react.useState)(referralCode || "");
  const [isReview, setIsReview] = (0, import_react.useState)(false);
  const maxRebatePercentage = (0, import_react.useMemo)(() => {
    return new decimal_default(maxRebateRate).mul(100).toNumber();
  }, [maxRebateRate]);
  const [referrerRebatePercentage, setReferrerRebatePercentage] = (0, import_react.useState)(
    () => {
      if (type === "create") {
        return Math.ceil(maxRebatePercentage / 2);
      }
      if (referrerRebateRate) {
        return new decimal_default(referrerRebateRate).mul(100).toNumber();
      }
      return 0;
    }
  );
  const {
    createReferralCode,
    editReferralCode,
    updateRebateRate,
    resetRebateRate,
    isMutating
  } = useReferralCode();
  const refereeRebatePercentage = (0, import_react.useMemo)(() => {
    return Math.max(
      0,
      new decimal_default(maxRebatePercentage).sub(referrerRebatePercentage).toNumber()
    );
  }, [referrerRebatePercentage]);
  const codeChanged = (0, import_react.useMemo)(() => {
    return newCode !== referralCode;
  }, [newCode, referralCode]);
  const rateChanged = (0, import_react.useMemo)(() => {
    return new decimal_default(referrerRebatePercentage).toNumber() !== new decimal_default(referrerRebateRate || 0).mul(100).toNumber();
  }, [referrerRebatePercentage, referrerRebateRate, newCode]);
  const handleError = (err) => {
    Vt.error(err?.message || t("common.somethingWentWrong"));
  };
  const handleResult = (res) => {
    if (res.success) {
      options.onSuccess?.();
      Vt.success(t("affiliate.confirmChanges.success"));
      options.close?.();
    } else {
      Vt.error(res.message);
    }
  };
  const onEdit = async () => {
    const editReferralCodeParams = {
      current_referral_code: referralCode,
      new_referral_code: newCode
    };
    const updateRebateRateParams = {
      referee_rebate_rate: new decimal_default(refereeRebatePercentage).div(100).toNumber(),
      account_ids: accountId ? [accountId] : void 0
    };
    try {
      if (codeChanged && rateChanged) {
        const codeRes = await editReferralCode(editReferralCodeParams);
        if (!codeRes.success) {
          Vt.error(codeRes.message);
          return;
        }
        const rateRes = await updateRebateRate(updateRebateRateParams);
        handleResult(rateRes);
      } else if (codeChanged) {
        const res = await editReferralCode(editReferralCodeParams);
        handleResult(res);
      } else if (rateChanged) {
        const res = await updateRebateRate(updateRebateRateParams);
        handleResult(res);
      }
    } catch (err) {
      handleError(err);
    }
  };
  const onCreate = async () => {
    try {
      const referee_rebate_rate = new decimal_default(refereeRebatePercentage).div(100).toNumber();
      const res = await createReferralCode({ referee_rebate_rate });
      if (res.success) {
        Vt.success(t("affiliate.referralCode.create.success"));
        options.onSuccess?.();
        options.close?.();
      } else {
        Vt.error(res.message);
      }
    } catch (err) {
      handleError(err);
    }
  };
  const onReset = async () => {
    try {
      const res = await resetRebateRate({ account_ids: [accountId] });
      handleResult(res);
    } catch (err) {
      handleError(err);
    }
  };
  const onClick = () => {
    switch (type) {
      case "create":
        onCreate();
        break;
      case "edit":
        if (isReview) {
          onEdit();
        } else {
          setIsReview(true);
        }
        break;
      case "reset":
        onReset();
        break;
    }
  };
  const buttonDisabled = type === "edit" && !codeChanged && !rateChanged;
  return {
    onClick,
    maxRebatePercentage,
    referrerRebatePercentage,
    setReferrerRebatePercentage,
    refereeRebatePercentage,
    isMutating,
    newCode,
    setNewCode,
    isReview,
    buttonDisabled,
    onReset
  };
};
var GiftIcon = (props) => {
  const { size = 16, className, ...rest } = props;
  return (0, import_jsx_runtime.jsx)(
    "svg",
    {
      width: size,
      height: size,
      viewBox: "0 0 16 16",
      fill: "none",
      xmlns: "http://www.w3.org/2000/svg",
      className,
      ...rest,
      children: (0, import_jsx_runtime.jsx)(
        "path",
        {
          d: "M5.99902 1.99377C4.52636 1.99377 3.33236 3.18777 3.33236 4.66044H2.66569C2.29769 4.66044 1.99902 4.9591 1.99902 5.3271C1.99902 5.66044 1.99902 7.66043 1.99902 7.99377C1.99902 8.36177 2.29769 8.66043 2.66569 8.66043V11.3271C2.66569 12.7998 3.85969 13.9938 5.33236 13.9938H10.6657C12.1384 13.9938 13.3324 12.7998 13.3324 11.3271V8.66043C13.7004 8.66043 13.999 8.36177 13.999 7.99377V5.3271C13.999 4.9591 13.7004 4.66044 13.3324 4.66044H12.6657C12.6657 3.18777 11.4717 1.99377 9.99902 1.99377C9.19169 1.99377 8.48836 2.36444 7.99902 2.93111C7.50969 2.36444 6.80636 1.99377 5.99902 1.99377ZM5.99902 3.32711C6.73569 3.32711 7.33236 3.92377 7.33236 4.66044H4.66569C4.66569 3.92377 5.26236 3.32711 5.99902 3.32711ZM9.99902 3.32711C10.7357 3.32711 11.3324 3.92377 11.3324 4.66044H8.66569C8.66569 3.92377 9.26236 3.32711 9.99902 3.32711ZM3.33236 5.99377H12.6657V7.3271H3.33236C3.33236 6.84777 3.33236 6.4731 3.33236 5.99377ZM3.99902 8.66043H7.33236V12.6604H5.33236C4.59569 12.6604 3.99902 12.0638 3.99902 11.3271V8.66043ZM8.66569 8.66043H11.999V11.3271C11.999 12.0638 11.4024 12.6604 10.6657 12.6604H8.66569V8.66043Z",
          fill: "currentColor"
        }
      )
    }
  );
};
var ReferralCodeForm = (props) => {
  const { type, isReview } = props;
  const { t } = useTranslation();
  const isReset = type === "reset";
  const hasBoundReferee = !!props.directInvites && props.directInvites > 0;
  const isEditingRefereeRebateRate = !!props.accountId;
  const noCommissionAvailable = props.maxRebateRate === 0;
  const { title, description, buttonText } = (0, import_react.useMemo)(() => {
    switch (type) {
      case "create":
        return {
          title: t("affiliate.referralCode.create.modal.title"),
          description: t("affiliate.referralCode.create.warning"),
          buttonText: t("affiliate.confirmAndGenerate")
        };
      case "edit":
        return {
          title: isEditingRefereeRebateRate ? t("affiliate.refereeRebateRate.modal.title", {
            accountId: formatAddress(props.accountId)
          }) : t("affiliate.referralCode.edit.modal.title"),
          description: isEditingRefereeRebateRate ? isReview ? t("affiliate.refereeRebateRate.review.warning") : t("affiliate.refereeRebateRate.edit.warning") : isReview ? t("affiliate.rebateRate.review.warning") : t("affiliate.rebateRate.edit.warning"),
          buttonText: isReview ? t("affiliate.confirmChanges") : t("affiliate.review")
        };
      case "reset":
        return {
          title: t("affiliate.resetRebateRate.modal.title"),
          description: (0, import_jsx_runtime.jsx)(Text2, { size: "2xs", intensity: 54, children: t("affiliate.resetRebateRate.modal.description", {
            accountId: formatAddress(props.accountId)
          }) }),
          buttonText: t("common.reset")
        };
      default:
        return {
          title: "",
          description: "",
          buttonText: ""
        };
    }
  }, [t, type, isEditingRefereeRebateRate, isReview, props.accountId]);
  const titleView = (0, import_jsx_runtime.jsxs)(Flex, { width: "100%", direction: "column", itemAlign: "start", gap: 3, children: [
    (0, import_jsx_runtime.jsx)(Text2, { size: "base", intensity: 98, children: title }),
    (0, import_jsx_runtime.jsx)(Divider, { intensity: 8, className: "oui-w-full" })
  ] });
  const descriptionView = (0, import_jsx_runtime.jsx)(WarningBox, { children: description });
  const referralCodeInput = (0, import_jsx_runtime.jsx)(
    ReferralCodeInput,
    {
      value: props.newCode,
      onChange: props.setNewCode,
      autoFocus: props.focusField === "referralCode",
      disabled: isReview || hasBoundReferee
    }
  );
  const commissionConfiguration = (0, import_jsx_runtime.jsxs)(import_jsx_runtime.Fragment, { children: [
    (0, import_jsx_runtime.jsx)(Text2, { size: "2xs", intensity: 54, children: t("affiliate.commissionConfiguration") }),
    !isReview && (0, import_jsx_runtime.jsxs)(Text2, { size: "2xs", intensity: 54, children: [
      t("affiliate.totalCommissionAvailable"),
      ":",
      " ",
      (0, import_jsx_runtime.jsxs)(Text2, { className: "oui-text-warning", children: [
        props.maxRebatePercentage,
        "%"
      ] })
    ] })
  ] });
  const rebateRateSlider = (0, import_jsx_runtime.jsx)(
    RebateRateSlider,
    {
      value: props.referrerRebatePercentage,
      restValue: props.refereeRebatePercentage,
      onChange: props.setReferrerRebatePercentage,
      max: props.maxRebatePercentage,
      disabled: noCommissionAvailable || isReview || isReset,
      disabledIncrease: hasBoundReferee,
      directBonusRebateRate: props.directBonusRebateRate,
      noCommissionAvailable
    }
  );
  const refereeInfo = (0, import_jsx_runtime.jsxs)(Flex, { width: "100%", justify: "between", gap: 2, children: [
    (0, import_jsx_runtime.jsx)(Text2, { size: "2xs", intensity: 54, children: t("affiliate.referees") }),
    (0, import_jsx_runtime.jsx)(Text2.formatted, { rule: "address", size: "2xs", intensity: 98, children: props.accountId })
  ] });
  const noCommissionAvailableWarning = noCommissionAvailable && (0, import_jsx_runtime.jsx)(Text2, { size: "2xs", className: "oui-text-warning", children: isEditingRefereeRebateRate ? t("affiliate.rebateRate.noCommissionRate") : t("affiliate.rebateRate.noCommissionAvailable") });
  const resetRebateRateLabel = (0, import_jsx_runtime.jsxs)(Text2, { size: "2xs", intensity: 98, className: "oui-text-start", children: [
    t("affiliate.resetRebateRate.rateAfterReset"),
    ":"
  ] });
  const buttons = (0, import_jsx_runtime.jsxs)(Flex, { direction: "row", gap: 2, width: "100%", mt: 0, pt: 5, children: [
    (0, import_jsx_runtime.jsx)(
      Button,
      {
        variant: "contained",
        color: "gray",
        fullWidth: true,
        onClick: props.close,
        size: "md",
        children: t("common.cancel")
      }
    ),
    (0, import_jsx_runtime.jsx)(
      Button,
      {
        fullWidth: true,
        onClick: props.onClick,
        disabled: props.buttonDisabled || props.isMutating,
        loading: props.isMutating,
        size: "md",
        children: buttonText
      }
    )
  ] });
  const renderContent = () => {
    switch (type) {
      case "create":
        return (0, import_jsx_runtime.jsxs)(Flex, { width: "100%", direction: "column", itemAlign: "start", gap: 2, children: [
          commissionConfiguration,
          rebateRateSlider,
          noCommissionAvailableWarning,
          buttons
        ] });
      case "edit":
        return (0, import_jsx_runtime.jsxs)(import_jsx_runtime.Fragment, { children: [
          isEditingRefereeRebateRate ? refereeInfo : referralCodeInput,
          (0, import_jsx_runtime.jsxs)(Flex, { width: "100%", direction: "column", itemAlign: "start", gap: 2, children: [
            commissionConfiguration,
            rebateRateSlider,
            noCommissionAvailableWarning,
            buttons
          ] })
        ] });
      case "reset":
        return (0, import_jsx_runtime.jsxs)(Flex, { width: "100%", direction: "column", itemAlign: "start", gap: 2, children: [
          resetRebateRateLabel,
          rebateRateSlider,
          buttons
        ] });
      default:
        return null;
    }
  };
  return (0, import_jsx_runtime.jsxs)(
    Flex,
    {
      direction: "column",
      itemAlign: "start",
      gap: 6,
      className: "oui-font-semibold",
      children: [
        titleView,
        descriptionView,
        renderContent()
      ]
    }
  );
};
var NoCommissionCard = (props) => {
  const { t } = useTranslation();
  const amount = props.directBonusRebateRate;
  return (0, import_jsx_runtime.jsxs)(
    Flex,
    {
      direction: "column",
      gap: 2,
      mt: 2,
      width: "100%",
      p: 4,
      r: "lg",
      className: "oui-border oui-border-base-contrast/[0.08] oui-tracking-[0.03em]",
      children: [
        (0, import_jsx_runtime.jsxs)(Flex, { justify: "between", width: "100%", itemAlign: "center", children: [
          (0, import_jsx_runtime.jsxs)(Flex, { direction: "column", itemAlign: "start", gap: 1, children: [
            (0, import_jsx_runtime.jsx)(Text2, { size: "2xs", intensity: 54, children: t("affiliate.noCommissionCard.title") }),
            (0, import_jsx_runtime.jsxs)(Text2, { size: "lg", className: "oui-text-primary-light oui-font-semibold", children: [
              "+ ",
              amount,
              "%"
            ] })
          ] }),
          (0, import_jsx_runtime.jsx)(GiftIcon, { size: 24, className: "oui-shrink-0 oui-text-primary-light" })
        ] }),
        (0, import_jsx_runtime.jsx)(Text2, { size: "2xs", intensity: 98, className: "oui-leading-[15px]", children: t("affiliate.noCommissionCard.content", { amount }) })
      ]
    }
  );
};
var RebateRateSlider = (props) => {
  const { t } = useTranslation();
  const [maxValue] = (0, import_react.useState)(props.value);
  return (0, import_jsx_runtime.jsxs)(import_jsx_runtime.Fragment, { children: [
    !props.disabled && (0, import_jsx_runtime.jsx)(Box, { width: "100%", my: 2, children: (0, import_jsx_runtime.jsx)(
      Slider,
      {
        min: 0,
        max: props.max,
        step: 1,
        value: [props.value],
        onValueChange: (value) => {
          const newValue = value[0];
          props.onChange(
            props.disabledIncrease ? Math.min(newValue, maxValue) : newValue
          );
        },
        classNames: {
          range: "oui-bg-success-darken oui-h-2 oui-top-[0px]",
          trackInner: "oui-bg-success-darken/30 oui-h-2 oui-top-[0px]",
          thumb: "oui-border-[#d9d9d9] oui-bg-[#d9d9d9] oui-size-4"
        }
      }
    ) }),
    (0, import_jsx_runtime.jsxs)("div", { className: "oui-w-full", children: [
      (0, import_jsx_runtime.jsxs)(Flex, { justify: "between", width: "100%", children: [
        (0, import_jsx_runtime.jsx)(Text2, { size: "sm", intensity: 54, children: t("affiliate.youKeep") }),
        (0, import_jsx_runtime.jsx)(Text2, { size: "sm", intensity: 54, children: t("affiliate.refereesGet") })
      ] }),
      (0, import_jsx_runtime.jsxs)(Flex, { justify: "between", width: "100%", children: [
        (0, import_jsx_runtime.jsxs)(Text2.formatted, { size: "lg", className: "oui-text-success-darken", children: [
          props.value,
          "%"
        ] }),
        (0, import_jsx_runtime.jsxs)(Text2.formatted, { size: "lg", className: "oui-text-success-darken/50", children: [
          props.restValue,
          "%"
        ] })
      ] }),
      props.noCommissionAvailable ? (0, import_jsx_runtime.jsx)(
        NoCommissionCard,
        {
          directBonusRebateRate: props.directBonusRebateRate
        }
      ) : props.directBonusRebateRate != null && props.directBonusRebateRate > 0 && (0, import_jsx_runtime.jsxs)(Flex, { gap: 2, mt: 2, width: "100%", children: [
        (0, import_jsx_runtime.jsx)(
          GiftIcon,
          {
            size: 16,
            className: "oui-text-base-contrast oui-mt-[1px]"
          }
        ),
        (0, import_jsx_runtime.jsx)(
          Text2,
          {
            size: "base",
            intensity: 54,
            as: "span",
            className: "oui-inline-flex oui-items-center oui-gap-1 oui-tracking-[0.03em]",
            children: (0, import_jsx_runtime.jsx)(
              Trans,
              {
                i18nKey: "affiliate.extraBonusOnDirectReferrals",
                values: { amount: props.directBonusRebateRate },
                components: [(0, import_jsx_runtime.jsx)(Text2, { as: "span", color: "primaryLight" }, "0")]
              }
            )
          }
        )
      ] })
    ] })
  ] });
};
var ReferralCodeInput = (props) => {
  const { t } = useTranslation();
  const hasSetCursorToEnd = (0, import_react.useRef)(false);
  return (0, import_jsx_runtime.jsx)(
    TextField,
    {
      type: "text",
      fullWidth: true,
      label: t("affiliate.referralCode.editCodeModal.label"),
      value: props.value,
      onChange: (e) => {
        props.onChange(e.target.value);
      },
      onFocus: (e) => {
        if (props.autoFocus && !hasSetCursorToEnd.current) {
          hasSetCursorToEnd.current = true;
          const input = e.target;
          const len = input.value.length;
          requestAnimationFrame(() => {
            input.setSelectionRange(len, len);
          });
        }
      },
      formatters: [
        formatter_exports.createRegexInputFormatter((value) => {
          return String(value).replace(
            /[a-z]/g,
            (char) => char.toUpperCase()
          );
        }),
        formatter_exports.createRegexInputFormatter(/[^A-Z0-9]/g)
      ],
      className: "oui-w-full",
      classNames: {
        label: "oui-text-base-contrast-54 oui-text-xs",
        input: "placeholder:oui-text-base-contrast-20 placeholder:oui-text-sm"
      },
      maxLength: 10,
      minLength: 4,
      autoComplete: "off",
      disabled: props.disabled,
      autoFocus: props.autoFocus
    }
  );
};
var WarningBox = (props) => {
  const { children } = props;
  if (typeof children === "string") {
    return (0, import_jsx_runtime.jsxs)(
      Flex,
      {
        className: "oui-bg-warning/10",
        justify: "start",
        itemAlign: "start",
        gap: 1,
        r: "lg",
        p: 3,
        children: [
          (0, import_jsx_runtime.jsx)(WarningIcon, { className: "oui-shrink-0 oui-text-warning" }),
          (0, import_jsx_runtime.jsx)(Text2, { size: "2xs", intensity: 54, className: "oui-text-warning", children })
        ]
      }
    );
  }
  return children;
};
var ReferralCodeFormWidget = (props) => {
  const state = useReferralCodeFormScript(props);
  return (0, import_jsx_runtime.jsx)(ReferralCodeForm, { ...state, ...props });
};
var ReferralCodeFormDialogId = "ReferralCodeFormDialogId";
var ReferralCodeFormSheetId = "ReferralCodeFormSheetId";
registerSimpleDialog(ReferralCodeFormDialogId, ReferralCodeFormWidget, {
  size: "sm",
  classNames: {
    content: "oui-border oui-border-line-6",
    body: "!oui-pt-3"
  }
});
registerSimpleSheet(ReferralCodeFormSheetId, ReferralCodeFormWidget);
var TradingVolumeProgress = (props) => {
  const { t } = useTranslation();
  const { classNames } = props;
  const { volumePrerequisite } = useReferralContext();
  const { routerAdapter } = useScaffoldContext();
  const { current_volume = 0, required_volume = 0 } = volumePrerequisite || {};
  const progressPercentage = (0, import_react.useMemo)(() => {
    if (required_volume === 0) return 0;
    const percentage = new decimal_default(current_volume).div(required_volume).mul(100).toNumber();
    return Math.min(percentage, 100);
  }, [current_volume, required_volume]);
  const formatRequiredVolume = (0, import_react.useMemo)(() => {
    return parseNumber(required_volume, {
      rule: "price",
      dp: 0
    });
  }, [required_volume]);
  const gotoTrade = () => {
    routerAdapter?.onRouteChange?.({
      href: "/perp",
      name: "Perp"
    });
  };
  return (0, import_jsx_runtime.jsxs)(Flex, { width: "100%", direction: "column", gap: 4, className: classNames?.root, children: [
    (0, import_jsx_runtime.jsx)(
      Text2,
      {
        size: "sm",
        intensity: 54,
        className: M("oui-text-center", classNames?.description),
        children: t("affiliate.newReferralProgram.tradeUnlock.description", {
          volume: formatRequiredVolume
        })
      }
    ),
    (0, import_jsx_runtime.jsxs)(Flex, { width: "100%", direction: "column", gap: 2, children: [
      (0, import_jsx_runtime.jsxs)(
        Flex,
        {
          width: "100%",
          justify: "between",
          className: "oui-text-2xs oui-text-base-contrast",
          children: [
            (0, import_jsx_runtime.jsx)(Text2, { children: t("common.current") }),
            (0, import_jsx_runtime.jsxs)(Flex, { gap: 1, children: [
              (0, import_jsx_runtime.jsx)(Text2.numeral, { rule: "price", dp: 0, children: current_volume }),
              (0, import_jsx_runtime.jsx)(
                Text2.numeral,
                {
                  intensity: 54,
                  rule: "price",
                  dp: 0,
                  prefix: "/ ",
                  suffix: " USDC",
                  children: required_volume
                }
              )
            ] })
          ]
        }
      ),
      (0, import_jsx_runtime.jsx)("div", { className: "oui-h-[8px] oui-w-full oui-rounded-full oui-bg-base-contrast-4", children: (0, import_jsx_runtime.jsx)(
        "div",
        {
          style: {
            width: `${progressPercentage}%`
          },
          className: M(
            "oui-h-full oui-rounded-l-full oui-bg-primary-light",
            progressPercentage === 100 && "oui-rounded-r-full"
          )
        }
      ) })
    ] }),
    (0, import_jsx_runtime.jsx)(Button, { size: "md", onClick: gotoTrade, ...props.buttonProps, children: t("affiliate.newReferralProgram.tradeUnlock") })
  ] });
};
var Hero = () => {
  const { t } = useTranslation();
  const { state } = useAccount();
  const { wrongNetwork } = useAppContext();
  const status = state.status;
  const {
    isMultiLevelReferralUnlocked,
    isMultiLevelEnabled,
    multiLevelRebateInfo,
    multiLevelRebateInfoMutate,
    maxRebateRate
  } = useReferralContext();
  const onCreateReferralCode = () => {
    modal.show(ReferralCodeFormDialogId, {
      type: "create",
      maxRebateRate,
      directBonusRebateRate: 10,
      // Hardcoded value before API is ready
      onSuccess: () => {
        multiLevelRebateInfoMutate();
      }
    });
  };
  const description = (0, import_react.useMemo)(() => {
    if (wrongNetwork) {
      return t("affiliate.wrongNetwork.description");
    }
    if (status === AccountStatusEnum.NotConnected) {
      return t("affiliate.newReferralProgram.description");
    }
    if (status > AccountStatusEnum.Connected && status < AccountStatusEnum.EnableTrading) {
      return t("affiliate.setUpAccount.description");
    }
    if (!isMultiLevelReferralUnlocked) {
      return "";
    }
    return t("affiliate.newReferralProgram.description");
  }, [t, wrongNetwork, status]);
  const renderContent = () => {
    if (!isMultiLevelReferralUnlocked) {
      return (0, import_jsx_runtime.jsx)(
        TradingVolumeProgress,
        {
          classNames: {
            root: "oui-items-start",
            description: "!oui-text-start"
          },
          buttonProps: {
            size: "xl"
          }
        }
      );
    }
    if (isMultiLevelEnabled && !multiLevelRebateInfo?.referral_code) {
      return (0, import_jsx_runtime.jsx)(Button, { size: "lg", className: "oui-px-4", onClick: onCreateReferralCode, children: t("affiliate.referralCode.create") });
    }
    return "";
  };
  return (0, import_jsx_runtime.jsxs)(
    Flex,
    {
      gap: 8,
      className: "oui-flex-col-reverse md:oui-flex-row",
      id: "oui-affiliate-landing-hero",
      itemAlign: "center",
      children: [
        (0, import_jsx_runtime.jsxs)(Flex, { direction: "column", itemAlign: "start", gap: 6, className: "oui-flex-1", children: [
          (0, import_jsx_runtime.jsx)(
            Flex,
            {
              direction: "column",
              gap: 3,
              itemAlign: "start",
              id: "oui-affiliate-landing-hero-title",
              children: (0, import_jsx_runtime.jsx)(
                Text2,
                {
                  weight: "semibold",
                  className: "oui-text-[32px] oui-leading-tight md:oui-text-[48px]",
                  children: t("affiliate.landing.title")
                }
              )
            }
          ),
          description && (0, import_jsx_runtime.jsx)(Text2, { size: "sm", intensity: 54, children: description }),
          (0, import_jsx_runtime.jsx)(
            AuthGuard,
            {
              labels: {
                connectWallet: t("affiliate.connectWallet"),
                signin: t("affiliate.setUpAccount"),
                enableTrading: t("affiliate.setUpAccount")
              },
              children: renderContent()
            }
          )
        ] }),
        (0, import_jsx_runtime.jsx)(
          "img",
          {
            src: "https://oss.orderly.network/static/sdk/referral/network-diagram.png",
            alt: "Network diagram",
            className: "oui-size-[335px] oui-object-cover md:oui-size-[480px]",
            id: "oui-affiliate-landing-network-diagram"
          }
        )
      ]
    }
  );
};
var StepCard = (props) => {
  const { step, icon, title, description } = props;
  return (0, import_jsx_runtime.jsxs)(
    Flex,
    {
      direction: "column",
      gap: 3,
      r: "2xl",
      p: 5,
      itemAlign: "start",
      className: "oui-bg-base-9",
      children: [
        (0, import_jsx_runtime.jsx)(
          Flex,
          {
            justify: "center",
            className: "oui-rounded-full oui-bg-base-7 lg:oui-justify-start",
            p: 3,
            width: 48,
            height: 48,
            itemAlign: "center",
            children: renderIcon(icon)
          }
        ),
        (0, import_jsx_runtime.jsx)(Text2, { size: "xl", weight: "semibold", children: `${step}. ${title}` }),
        (0, import_jsx_runtime.jsx)(Text2, { intensity: 54, size: "sm", children: description })
      ]
    }
  );
};
var renderIcon = (iconType) => {
  switch (iconType) {
    case "wallet":
      return (0, import_jsx_runtime.jsx)(WalletIcon, {});
    case "settings":
      return (0, import_jsx_runtime.jsx)(SettingsIcon, {});
    case "rocket":
      return (0, import_jsx_runtime.jsx)(RocketIcon, {});
    default:
      return null;
  }
};
var WalletIcon = () => (0, import_jsx_runtime.jsx)(
  "svg",
  {
    width: "24",
    height: "24",
    viewBox: "0 0 24 24",
    fill: "none",
    xmlns: "http://www.w3.org/2000/svg",
    children: (0, import_jsx_runtime.jsx)(
      "path",
      {
        d: "M7.006 2.93a4 4 0 0 0-4 4v10a4 4 0 0 0 4 4h10c1.552 0 3-1.448 3-3l-.004-1.163a3.025 3.025 0 0 0 2.004-2.837v-4c0-1.268-.822-2.428-1.994-2.815l-.006-1.185c0-1.553-1.448-3-3-3zm0 2h10c.448 0 1 .552 1 1v1h-3a3 3 0 0 0-3 3v4a3 3 0 0 0 3 3h3v1c0 .447-.552 1-1 1h-10a2 2 0 0 1-2-2v-10a2 2 0 0 1 2-2m8 4h4a1 1 0 0 1 1 1v4a1 1 0 0 1-1 1h-4a1 1 0 0 1-1-1v-4a1 1 0 0 1 1-1m2 2a1 1 0 1 0 0 2 1 1 0 0 0 0-2",
        fill: "#fff",
        fillOpacity: ".54"
      }
    )
  }
);
var SettingsIcon = () => (0, import_jsx_runtime.jsx)(
  "svg",
  {
    width: "24",
    height: "24",
    viewBox: "0 0 24 24",
    fill: "none",
    xmlns: "http://www.w3.org/2000/svg",
    children: (0, import_jsx_runtime.jsx)(
      "path",
      {
        d: "M14.99 2.93c-1.267 0-2.45.832-2.818 2.002L3.99 4.93a1 1 0 0 0 0 2l8.18-.001c.449 1.225 1.552 2 2.82 2s2.382-.777 2.838-2.008l2.162.009a1 1 0 0 0 0-2h-2.17c-.482-1.22-1.562-2-2.83-2m0 2a1 1 0 1 1 0 2 1 1 0 0 1 0-2m-6 4c-1.317 0-2.42.82-2.823 2.002-.14.009-2.176-.002-2.176-.002a1 1 0 0 0 0 2s2.052-.021 2.18-.008a2.95 2.95 0 0 0 2.82 2.008c1.268 0 2.354-.777 2.83-1.997l8.17-.003a1 1 0 0 0 0-2l-8.176-.01A3.01 3.01 0 0 0 8.99 8.93m0 2a1 1 0 1 1 0 2 1 1 0 0 1 0-2m6 4c-1.267 0-2.427.835-2.819 1.99l-8.18.01a1 1 0 0 0 0 2h8.165a3.02 3.02 0 0 0 2.835 2c1.268 0 2.36-.793 2.842-2h2.158a1 1 0 0 0 0-2l-2.167-.006a3.03 3.03 0 0 0-2.833-1.994m0 2a1 1 0 1 1 0 2 1 1 0 0 1 0-2",
        fill: "#fff",
        fillOpacity: ".54"
      }
    )
  }
);
var RocketIcon = () => (0, import_jsx_runtime.jsx)(
  "svg",
  {
    width: "24",
    height: "24",
    viewBox: "0 0 24 24",
    fill: "none",
    xmlns: "http://www.w3.org/2000/svg",
    children: (0, import_jsx_runtime.jsx)(
      "path",
      {
        d: "M18.527 13.88c-.423.424-.884.808-1.345 1.154a.63.63 0 0 0-.269.423 9.2 9.2 0 0 1-2.459 4.346 10 10 0 0 1-1.883 1.462c-.576.346-1.268-.231-1.037-.885.384-1 .538-2.076.538-3.192-.077 0-.192.039-.27.039a39 39 0 0 1-2.612-2.423 51 51 0 0 1-2.42-2.615c0-.077.038-.193.038-.27-1.115 0-2.19.193-3.19.539-.614.23-1.23-.462-.883-1.039a10 10 0 0 1 1.46-1.884 9.2 9.2 0 0 1 4.342-2.461c.153-.039.307-.154.422-.27.346-.5.73-.923 1.153-1.346 2.997-3 7.185-4.038 11.066-3.153a.76.76 0 0 1 .538.538c.845 3.846-.192 8.038-3.189 11.038m-1.69-6.73a2.41 2.41 0 0 0-3.382 0 2.416 2.416 0 0 0 0 3.385 2.41 2.41 0 0 0 3.381 0c.96-.962.96-2.462 0-3.385M8.266 18.92c-.73.73-5.225 3.807-6.109 2.923-.884-.885 2.19-5.385 2.92-6.115s2.037-.616 2.92.269c.884.885 1 2.192.27 2.923",
        fill: "#fff",
        fillOpacity: ".54"
      }
    )
  }
);
var HowItWorks = () => {
  const { t } = useTranslation();
  const steps = (0, import_react.useMemo)(() => {
    return [
      {
        step: 1,
        icon: "wallet",
        title: t("affiliate.howItWorks.step1.title"),
        description: t("affiliate.howItWorks.step1.description")
      },
      {
        step: 2,
        icon: "settings",
        title: t("affiliate.howItWorks.step2.title"),
        description: t("affiliate.howItWorks.step2.description")
      },
      {
        step: 3,
        icon: "rocket",
        title: t("affiliate.howItWorks.step3.title"),
        description: t("affiliate.howItWorks.step3.description")
      }
    ];
  }, [t]);
  return (0, import_jsx_runtime.jsxs)(Box, { id: "oui-affiliate-landing-how-it-works", children: [
    (0, import_jsx_runtime.jsxs)(Flex, { direction: "column", itemAlign: "start", children: [
      (0, import_jsx_runtime.jsx)(Text2, { weight: "semibold", className: "oui-text-[32px]", children: t("affiliate.howItWorks.title") }),
      (0, import_jsx_runtime.jsx)(Box, { py: 4, children: (0, import_jsx_runtime.jsx)(Text2, { intensity: 54, children: t("affiliate.howItWorks.description") }) })
    ] }),
    (0, import_jsx_runtime.jsx)(Grid2, { className: "oui-grid-cols-1 md:oui-grid-cols-3", gap: 6, children: steps.map((step) => (0, import_jsx_runtime.jsx)(
      StepCard,
      {
        step: step.step,
        icon: step.icon,
        title: step.title,
        description: step.description
      },
      step.step
    )) })
  ] });
};
var LandingPage = () => {
  return (0, import_jsx_runtime.jsxs)(
    Flex,
    {
      id: "oui-affiliate-landing-page",
      direction: "column",
      gap: 10,
      itemAlign: "stretch",
      className: "oui-mx-auto oui-w-full oui-p-5 md:oui-w-[1040px] md:oui-py-12",
      children: [
        (0, import_jsx_runtime.jsx)(Hero, {}),
        (0, import_jsx_runtime.jsx)(HowItWorks, {}),
        (0, import_jsx_runtime.jsx)(Faq, {})
      ]
    }
  );
};
var CommissionChart = (props) => {
  const { t } = useTranslation();
  return (0, import_jsx_runtime.jsxs)(
    Flex,
    {
      id: "oui-affiliate-affiliate-titleStatistic",
      r: "2xl",
      p: 6,
      width: "100%",
      gap: 4,
      direction: "column",
      className: "oui-border oui-border-line-6 oui-bg-base-9",
      children: [
        (0, import_jsx_runtime.jsxs)(Flex, { justify: "between", width: "100%", children: [
          (0, import_jsx_runtime.jsx)(Text2, { size: "lg", children: `${t("affiliate.commission")} (USDC)` }),
          (0, import_jsx_runtime.jsx)("div", { children: (0, import_jsx_runtime.jsx)(
            Select2.options,
            {
              size: "xs",
              value: props.period,
              onValueChange: props.onPeriodChange,
              options: props.periodTypes
            }
          ) })
        ] }),
        (0, import_jsx_runtime.jsx)(Flex, { className: "oui-flex oui-h-[170px] oui-w-full oui-flex-row oui-items-stretch 2xl:oui-h-[196px]", children: (0, import_jsx_runtime.jsx)(
          VolBarChart,
          {
            data: props.dataSource,
            colors: { fill: "rgba(0, 180, 158, 1)" },
            className: "oui-w-full oui-flex-1",
            tooltip: {
              rm: decimal_default.ROUND_DOWN,
              dp: 2
            }
          }
        ) })
      ]
    }
  );
};
function fillData(days, origin) {
  const now = Date();
  const result = new Array(days).fill(0).map((_, index) => {
    return {
      date: format(subDays(now, index + 1), "yyyy-MM-dd"),
      volume: 0,
      opacity: 0
    };
  }).reverse();
  const dataObject = origin?.reduce(
    (acc, curr) => {
      acc[curr.date] = curr;
      return acc;
    },
    {}
  );
  for (let index = 0; index < result.length; index++) {
    const element = result[index];
    const originData = dataObject?.[element.date];
    if (originData) {
      result[index] = { ...originData, opacity: originData.volume > 0 ? 1 : 0 };
    }
  }
  return result;
}
var useCommissionChartScript = () => {
  const { t } = useTranslation();
  const [period, setPeriod] = (0, import_react.useState)("90");
  const periodTypes = (0, import_react.useMemo)(() => {
    return [
      { label: t("common.select.7d"), value: "7" },
      { label: t("common.select.30d"), value: "30" },
      { label: t("common.select.90d"), value: "90" }
    ];
  }, [t]);
  const onPeriodChange = (item) => {
    setPeriod(item);
  };
  const dateRange = (0, import_react.useMemo)(() => {
    const days = Number(period) || 7;
    const now = /* @__PURE__ */ new Date();
    return {
      startDate: subDays(now, days),
      endDate: subDays(now, 1)
    };
  }, [period]);
  const [rebateSummary] = useReferralRebateSummary({
    startDate: format(dateRange.startDate, "yyyy-MM-dd"),
    endDate: format(dateRange.endDate, "yyyy-MM-dd"),
    size: Number(period)
  });
  const dataSource = (0, import_react.useMemo)(() => {
    const data = (rebateSummary || [])?.map(
      (item) => ({
        date: item.date,
        volume: item.direct_rebate + item.indirect_rebate + item.direct_bonus_rebate
      })
    ) || [];
    data.reverse();
    return fillData(Number(period), data);
  }, [rebateSummary, period]);
  return {
    period,
    periodTypes,
    onPeriodChange,
    dataSource
  };
};
var CommissionChartWidget = () => {
  const state = useCommissionChartScript();
  return (0, import_jsx_runtime.jsx)(CommissionChart, { ...state });
};
var MultiLevelReferral = (props) => {
  const { isMultiLevelReferralUnlocked } = props;
  const { t } = useTranslation();
  return (0, import_jsx_runtime.jsxs)(
    Flex,
    {
      r: "2xl",
      width: "100%",
      height: "100%",
      gap: 4,
      p: 5,
      direction: "column",
      intensity: 900,
      className: "oui-border oui-border-line-6",
      children: [
        (0, import_jsx_runtime.jsx)(Text2, { size: "lg", className: "oui-w-full oui-text-start", children: t("affiliate.referral") }),
        (0, import_jsx_runtime.jsxs)(
          Flex,
          {
            width: "100%",
            height: "100%",
            direction: "column",
            itemAlign: "center",
            justify: "center",
            gap: 4,
            children: [
              (0, import_jsx_runtime.jsx)(MultiLevelReferralIcon, { className: "oui-text-primary-light" }),
              (0, import_jsx_runtime.jsx)(Text2, { size: "base", intensity: 98, children: t("affiliate.newReferralProgram.title") }),
              isMultiLevelReferralUnlocked ? (0, import_jsx_runtime.jsxs)(import_jsx_runtime.Fragment, { children: [
                (0, import_jsx_runtime.jsx)(Text2, { size: "sm", intensity: 54, className: "oui-text-center", children: t("affiliate.newReferralProgram.description") }),
                (0, import_jsx_runtime.jsx)(
                  Button,
                  {
                    size: "md",
                    className: "oui-px-16",
                    onClick: props.createReferralCode,
                    children: t("affiliate.referralCode.create")
                  }
                )
              ] }) : (0, import_jsx_runtime.jsx)(
                TradingVolumeProgress,
                {
                  buttonProps: {
                    className: "oui-px-16"
                  }
                }
              )
            ]
          }
        )
      ]
    }
  );
};
var MultiLevelReferralIcon = (props) => (0, import_jsx_runtime.jsxs)(
  "svg",
  {
    width: 64,
    height: 64,
    viewBox: "0 0 64 64",
    fill: "currentColor",
    xmlns: "http://www.w3.org/2000/svg",
    ...props,
    children: [
      (0, import_jsx_runtime.jsx)("rect", { width: 64, height: 64, rx: 32, fill: "rgb(var(--oui-color-base-8))" }),
      (0, import_jsx_runtime.jsx)(
        "path",
        {
          fillRule: "evenodd",
          clipRule: "evenodd",
          d: "M28.822 23.24a.86.86 0 0 1 .607-.24h5.142c.228 0 .446.086.607.24a.8.8 0 0 1 .25.579v3.28a.8.8 0 0 1-.25.578.88.88 0 0 1-.607.24h-1.719v3.259h7.714c.228 0 .446.086.606.24a.8.8 0 0 1 .251.578v4.094h1.72c.227 0 .445.086.606.24a.8.8 0 0 1 .251.578v3.275a.8.8 0 0 1-.251.58.88.88 0 0 1-.606.239H38a.88.88 0 0 1-.606-.24.8.8 0 0 1-.251-.579v-3.274a.8.8 0 0 1 .25-.58.88.88 0 0 1 .607-.24h1.71v-3.274h-6.858v3.275h1.72c.227 0 .445.086.605.24a.8.8 0 0 1 .252.578v3.275a.8.8 0 0 1-.252.58.88.88 0 0 1-.606.239H29.43a.88.88 0 0 1-.606-.24.8.8 0 0 1-.252-.579v-3.274a.8.8 0 0 1 .252-.58.88.88 0 0 1 .606-.24h1.709v-3.274H24.28v3.275H26c.227 0 .445.086.606.24a.8.8 0 0 1 .251.578v3.275a.8.8 0 0 1-.25.58A.88.88 0 0 1 26 41h-5.143a.88.88 0 0 1-.606-.24.8.8 0 0 1-.251-.579v-3.274a.8.8 0 0 1 .251-.58.88.88 0 0 1 .606-.24h1.71v-4.093a.8.8 0 0 1 .25-.579.88.88 0 0 1 .606-.24h7.715v-3.258h-1.71a.88.88 0 0 1-.605-.24.8.8 0 0 1-.252-.579V23.82q0-.163.065-.313a.8.8 0 0 1 .186-.268"
        }
      )
    ]
  }
);
var useMultiLevelReferralScript = () => {
  const {
    isMultiLevelReferralUnlocked,
    maxRebateRate,
    multiLevelRebateInfoMutate
  } = useReferralContext();
  const createReferralCode = () => {
    modal.show(ReferralCodeFormDialogId, {
      type: "create",
      maxRebateRate,
      directBonusRebateRate: 10,
      // Hardcoded value before API is ready
      onSuccess: () => {
        multiLevelRebateInfoMutate();
      }
    });
  };
  return {
    isMultiLevelReferralUnlocked,
    createReferralCode
  };
};
var MultiLevelReferralWidget = () => {
  const state = useMultiLevelReferralScript();
  return (0, import_jsx_runtime.jsx)(MultiLevelReferral, { ...state });
};
var ReferralInfo = (props) => {
  const { t } = useTranslation();
  return (0, import_jsx_runtime.jsxs)(
    Flex,
    {
      id: "oui-affiliate-affiliate-referralLink",
      r: "2xl",
      width: "100%",
      height: "100%",
      gap: 4,
      p: 5,
      direction: "column",
      intensity: 900,
      className: "oui-border oui-border-line-6",
      children: [
        (0, import_jsx_runtime.jsxs)(Flex, { direction: "row", justify: "between", width: "100%", children: [
          (0, import_jsx_runtime.jsx)(Text2, { size: "lg", children: t("affiliate.referral") }),
          (0, import_jsx_runtime.jsx)(
            Button,
            {
              size: "xs",
              color: "secondary",
              onClick: () => props.onEdit(
                "referralCode"
                /* ReferralCode */
              ),
              children: t("affiliate.configure")
            }
          )
        ] }),
        (0, import_jsx_runtime.jsxs)(Container, { className: "oui-flex-col oui-items-start oui-gap-4 oui-p-5", children: [
          (0, import_jsx_runtime.jsx)(
            Flex,
            {
              r: "full",
              px: 2,
              height: 20,
              justify: "center",
              itemAlign: "center",
              className: "oui-border oui-border-primary-light",
              children: (0, import_jsx_runtime.jsx)(Text2, { intensity: 98, className: "oui-text-[10px] oui-leading-[10px]", children: t("affiliate.multiLevel") })
            }
          ),
          (0, import_jsx_runtime.jsxs)(Flex, { direction: "row", justify: "between", width: "100%", children: [
            (0, import_jsx_runtime.jsx)(Text2, { size: "lg", color: "primaryLight", children: props.referralCode }),
            (0, import_jsx_runtime.jsx)(CopyButton, { value: props.referralCode })
          ] })
        ] }),
        (0, import_jsx_runtime.jsxs)(Container, { className: "oui-p-5", children: [
          (0, import_jsx_runtime.jsx)(Text2, { size: "lg", color: "primaryLight", children: props.referralLink }),
          (0, import_jsx_runtime.jsx)(CopyButton, { value: props.referralLink })
        ] }),
        (0, import_jsx_runtime.jsxs)(
          Box,
          {
            width: "100%",
            r: "2xl",
            className: "oui-bg-base-contrast-4 oui-p-5 md:oui-py-7",
            children: [
              (0, import_jsx_runtime.jsxs)(Flex, { direction: "row", justify: "between", width: "100%", children: [
                (0, import_jsx_runtime.jsx)(Text2, { size: "sm", children: t("affiliate.revenueSplitStrategy") }),
                (0, import_jsx_runtime.jsx)(Button, { size: "xs", color: "secondary", onClick: () => props.onEdit(), children: t("common.edit") })
              ] }),
              (0, import_jsx_runtime.jsxs)(Flex, { justify: "between", width: "100%", mt: 4, children: [
                (0, import_jsx_runtime.jsx)(Text2, { size: "sm", intensity: 54, children: t("affiliate.youKeep") }),
                (0, import_jsx_runtime.jsx)(Text2, { size: "sm", intensity: 54, children: t("affiliate.refereesGet") })
              ] }),
              (0, import_jsx_runtime.jsxs)(Flex, { justify: "between", width: "100%", mt: 2, children: [
                (0, import_jsx_runtime.jsxs)(Text2.formatted, { size: "3xl", intensity: 80, children: [
                  props.referrerRebateRate,
                  "%"
                ] }),
                (0, import_jsx_runtime.jsxs)(Text2.formatted, { size: "3xl", intensity: 36, children: [
                  props.refereeRebateRate,
                  "%"
                ] })
              ] }),
              props.directBonusRebateRate > 0 && (0, import_jsx_runtime.jsxs)(Flex, { gap: 2, mt: 4, width: "100%", className: "oui-items-center", children: [
                (0, import_jsx_runtime.jsx)(
                  GiftIcon,
                  {
                    size: 16,
                    className: "oui-shrink-0 oui-text-base-contrast oui-mt-[1px]"
                  }
                ),
                (0, import_jsx_runtime.jsx)(
                  Text2,
                  {
                    intensity: 54,
                    as: "span",
                    className: "oui-flex-1 oui-min-w-0 oui-tracking-[0.03em]",
                    children: (0, import_jsx_runtime.jsx)(
                      Trans,
                      {
                        i18nKey: "affiliate.extraBonusOnDirectReferrals",
                        values: { amount: props.directBonusRebateRate },
                        components: [(0, import_jsx_runtime.jsx)(Text2, { as: "span", color: "primaryLight" }, "0")]
                      }
                    )
                  }
                )
              ] })
            ]
          }
        )
      ]
    }
  );
};
var Container = (props) => {
  return (0, import_jsx_runtime.jsx)(
    Flex,
    {
      width: "100%",
      justify: "between",
      px: 5,
      r: "2xl",
      className: M("oui-bg-base-contrast-4", props.className),
      children: props.children
    }
  );
};
var CopyButton = (props) => {
  const { t } = useTranslation();
  const onCopy = () => {
    if (!props.value) return;
    navigator.clipboard.writeText(props.value);
    Vt.success(t("common.copy.copied"));
  };
  return (0, import_jsx_runtime.jsx)(Button, { size: "xs", color: "secondary", onClick: onCopy, children: t("common.copy") });
};
function generateReferralLink(referralLinkUrl, code) {
  return `${referralLinkUrl || window?.location?.origin}?ref=${code}`;
}
async function copyText(content) {
  try {
    await navigator.clipboard.writeText(content);
    Vt.success(i18n_default.t("common.copy.copied"));
  } catch (error) {
    Vt.success(i18n_default.t("common.copy.failed"));
  }
}
function formatDateTimeToUTC(input) {
  if (input === void 0) {
    return "";
  }
  const date = toDate(input);
  const utcDate = toUTCDate(date);
  return format(utcDate, "yyyy-MM-dd HH:mm:ss 'UTC'");
}
function formatYMDTime(time) {
  if (time === void 0) {
    return void 0;
  }
  const date = toDate(time);
  const utcDate = toUTCDate(date);
  return format(utcDate, "yyyy-MM-dd");
}
function compareDate(d1, d2) {
  const isEqual = d1 && d2 && d1.toISOString().substring(0, 10) === d2.toISOString().substring(0, 10);
  return isEqual;
}
function toUTCDate(date) {
  return new Date(date.toUTCString());
}
var useReferralInfoScript = () => {
  const {
    referralLinkUrl,
    multiLevelRebateInfo,
    multiLevelRebateInfoMutate,
    maxRebateRate
  } = useReferralContext();
  const referralCode = multiLevelRebateInfo?.referral_code;
  const referralLink = (0, import_react.useMemo)(() => {
    if (!referralCode) return "";
    return generateReferralLink(referralLinkUrl, referralCode);
  }, [referralCode]);
  const referrerRebateRate = (0, import_react.useMemo)(() => {
    return new decimal_default(multiLevelRebateInfo?.referrer_rebate_rate || 0).mul(100).toNumber();
  }, [multiLevelRebateInfo]);
  const refereeRebateRate = (0, import_react.useMemo)(() => {
    return new decimal_default(multiLevelRebateInfo?.referee_rebate_rate || 0).mul(100).toNumber();
  }, [multiLevelRebateInfo]);
  const directBonusRebateRate = (0, import_react.useMemo)(() => {
    return new decimal_default(multiLevelRebateInfo?.direct_bonus_rebate_rate ?? 0).mul(100).toNumber();
  }, [multiLevelRebateInfo]);
  const onEdit = (focusField) => {
    modal.show(ReferralCodeFormDialogId, {
      type: "edit",
      focusField,
      referralCode: multiLevelRebateInfo?.referral_code,
      maxRebateRate,
      referrerRebateRate: multiLevelRebateInfo?.referrer_rebate_rate,
      directInvites: multiLevelRebateInfo?.direct_invites,
      directBonusRebateRate,
      onSuccess: () => {
        multiLevelRebateInfoMutate();
      }
    });
  };
  return {
    onEdit,
    referralCode,
    referralLink,
    multiLevelRebateInfo,
    referrerRebateRate,
    refereeRebateRate,
    directBonusRebateRate
  };
};
var ReferralInfoWidget = () => {
  const state = useReferralInfoScript();
  return (0, import_jsx_runtime.jsx)(ReferralInfo, { ...state });
};
var useReferrerTableScript = () => {
  const [activeTab, _setActiveTab] = (0, import_react.useState)("commission");
  const setActiveTab = (tab) => _setActiveTab(tab);
  return {
    activeTab,
    setActiveTab
  };
};
var useReferralHistory = (params) => {
  const { size, startDate, endDate, page, fetchAll, enabled = true } = params;
  const query = (0, import_react.useMemo)(() => {
    if (!enabled) return null;
    const search = new URLSearchParams({
      ...startDate ? { start_date: startDate } : {},
      ...endDate ? { end_date: endDate } : {},
      ...!fetchAll && page !== void 0 ? { page: String(page) } : {},
      ...!fetchAll && size !== void 0 ? { size: String(size) } : {}
    });
    const qs = search.toString();
    return qs ? `/v1/referral/referral_history?${qs}` : "/v1/referral/referral_history";
  }, [enabled, endDate, fetchAll, page, size, startDate]);
  const response = usePrivateQuery(query, {
    formatter: (data) => data,
    revalidateOnFocus: false
  });
  const loadMore = () => {
  };
  const meta = (0, import_react.useMemo)(() => {
    const payload = response.data?.data ?? response.data;
    if (payload?.meta) return payload.meta;
    if (!payload?.rows) return void 0;
    return {
      total: payload.rows.length,
      records_per_page: payload.rows.length,
      current_page: 1
    };
  }, [response.data]);
  const total = (0, import_react.useMemo)(() => {
    if (typeof meta?.total === "number") return meta.total;
    return 0;
  }, [meta]);
  const flattenRows = (0, import_react.useMemo)(() => {
    if (!response.data) return null;
    const payload = response.data?.data ?? response.data;
    return payload?.rows ?? null;
  }, [response.data]);
  return [
    flattenRows,
    {
      total,
      isLoading: response.isLoading,
      refresh: response.mutate,
      loadMore,
      meta
    }
  ];
};
var useCommissionTableScript = (props = {}) => {
  const { enabled = true } = props;
  const { isMobile } = useScreen();
  const { state } = useAccount();
  const [dateRange, setDateRange] = (0, import_react.useState)({
    from: subDays(/* @__PURE__ */ new Date(), 30),
    to: /* @__PURE__ */ new Date()
  });
  const { page, pageSize, parsePagination } = usePagination();
  const [sort, setSort] = (0, import_react.useState)(null);
  const [data, { meta, isLoading: isDataLoading }] = useReferralHistory({
    enabled: enabled && state.status >= AccountStatusEnum.EnableTrading,
    startDate: dateRange.from ? format(dateRange.from, "yyyy-MM-dd") : void 0,
    endDate: dateRange.to ? format(dateRange.to, "yyyy-MM-dd") : void 0,
    page: isMobile ? void 0 : page,
    size: isMobile ? void 0 : pageSize,
    fetchAll: isMobile
  });
  const pagination = (0, import_react.useMemo)(() => {
    return parsePagination(meta);
  }, [meta, parsePagination]);
  const commissionData = (0, import_react.useMemo)(() => {
    if (!data) return [];
    const rows = data.map((row) => ({
      date: row.date,
      code: row.referral_code,
      volume: row.volume,
      commission: row.referral_rebate,
      directBonusRebate: row.direct_bonus_rebate,
      address: row.user_address
    }));
    if (!sort) return rows;
    rows.sort((a, b) => {
      const valA = a[sort.key];
      const valB = b[sort.key];
      if (valA < valB) return sort.order === "asc" ? -1 : 1;
      if (valA > valB) return sort.order === "asc" ? 1 : -1;
      return 0;
    });
    return rows;
  }, [data, sort]);
  const onSort = (sort2) => {
    if (!sort2) {
      setSort(null);
      return;
    }
    const sortKey = sort2.sortKey;
    if (sortKey && (sortKey === "volume" || sortKey === "commission")) {
      setSort({
        key: sortKey,
        order: sort2.sort === "asc" ? "asc" : "desc"
      });
    }
  };
  return {
    commissionData,
    dateRange,
    setDateRange,
    pagination,
    onSort,
    isLoading: state.status < AccountStatusEnum.EnableTrading || !enabled || isDataLoading
  };
};
var dashedUnderline = "oui-cursor-pointer oui-underline oui-decoration-dashed oui-underline-offset-4 oui-decoration-base-contrast-36 oui-tracking-[0.03em]";
var useMobileTooltipModal = (content, title) => {
  const { isMobile } = useScreen();
  return (0, import_react.useCallback)(
    (e) => {
      if (!isMobile) return;
      e.preventDefault();
      e.stopPropagation();
      modal.dialog({
        title,
        closable: true,
        size: "sm",
        content: (0, import_jsx_runtime.jsx)("div", { className: "oui-text-sm oui-leading-5 oui-text-base-contrast", children: content })
      });
    },
    [isMobile, content, title]
  );
};
var AddressCell = ({ address, title }) => {
  const onClick = useMobileTooltipModal(address, title);
  const { isMobile } = useScreen();
  return (0, import_jsx_runtime.jsx)(Tooltip, { content: address, open: isMobile ? false : void 0, children: (0, import_jsx_runtime.jsx)(Text2, { className: dashedUnderline, onClick, children: formatAddress(address) }) });
};
var TooltipCell = ({ text, tooltip, title }) => {
  const onClick = useMobileTooltipModal(tooltip, title);
  const { isMobile } = useScreen();
  return (0, import_jsx_runtime.jsx)(Tooltip, { content: tooltip, open: isMobile ? false : void 0, children: (0, import_jsx_runtime.jsx)(Text2, { className: dashedUnderline, onClick, children: text }) });
};
var BreakdownCell = ({ total, direct, indirect, directBonus, prefix, fix = 0, title }) => {
  const { t } = useTranslation();
  const format9 = (val) => commifyOptional(val, {
    fix,
    fallback: "0",
    padEnd: true,
    prefix
  });
  const directLine = directBonus ? (0, import_jsx_runtime.jsxs)(import_jsx_runtime.Fragment, { children: [
    "• ",
    t("affiliate.direct"),
    ": ",
    format9(direct + directBonus),
    " ",
    (0, import_jsx_runtime.jsxs)("span", { className: "oui-text-base-contrast-54", children: [
      "(",
      t("affiliate.directInclBonus", { amount: format9(directBonus) }),
      ")"
    ] })
  ] }) : (0, import_jsx_runtime.jsxs)(import_jsx_runtime.Fragment, { children: [
    "• ",
    t("affiliate.direct"),
    ": ",
    format9(direct)
  ] });
  const tooltipContent = (0, import_jsx_runtime.jsxs)("div", { className: "oui-flex oui-flex-col oui-gap-1", children: [
    (0, import_jsx_runtime.jsx)("div", { children: directLine }),
    (0, import_jsx_runtime.jsxs)("div", { children: [
      "• ",
      t("affiliate.indirect"),
      ": ",
      format9(indirect)
    ] })
  ] });
  const onClick = useMobileTooltipModal(tooltipContent, title);
  const { isMobile } = useScreen();
  return (0, import_jsx_runtime.jsx)(Tooltip, { content: tooltipContent, open: isMobile ? false : void 0, children: (0, import_jsx_runtime.jsx)(Text2, { className: dashedUnderline, onClick, children: format9(total) }) });
};
var MobileCell = ({ label, children, align = "start", className }) => {
  return (0, import_jsx_runtime.jsxs)(
    Flex,
    {
      direction: "column",
      itemAlign: align,
      className: M(
        "oui-gap-1.5 oui-font-medium oui-tracking-[0.01em]",
        className
      ),
      children: [
        (0, import_jsx_runtime.jsx)(Text2, { intensity: 36, size: "2xs", className: "oui-leading-[15px]", children: label }),
        (0, import_jsx_runtime.jsx)("div", { className: "oui-text-sm oui-leading-[20px]", children })
      ]
    }
  );
};
var MobileCard = ({ children, className }) => {
  return (0, import_jsx_runtime.jsx)(
    Grid2,
    {
      cols: 3,
      py: 3,
      r: "xl",
      width: "100%",
      className: M("oui-bg-base-9 oui-gap-x-[6px] oui-gap-y-3", className),
      children
    }
  );
};
var formatCommissionAmt = (v) => commifyOptional(v, {
  fix: 6,
  fallback: "0",
  padEnd: true,
  prefix: "$"
});
var CommissionCell = ({ item }) => {
  const { t } = useTranslation();
  const totalCommission = item.commission + item.directBonusRebate;
  const tooltip = (0, import_jsx_runtime.jsxs)(import_jsx_runtime.Fragment, { children: [
    (0, import_jsx_runtime.jsxs)("div", { children: [
      "• ",
      t("affiliate.base"),
      ": ",
      formatCommissionAmt(item.commission)
    ] }),
    (0, import_jsx_runtime.jsxs)("div", { children: [
      "• ",
      t("affiliate.directBonus"),
      ":",
      " ",
      formatCommissionAmt(item.directBonusRebate)
    ] })
  ] });
  return (0, import_jsx_runtime.jsx)(
    TooltipCell,
    {
      text: formatCommissionAmt(totalCommission),
      tooltip,
      title: t("affiliate.commission")
    }
  );
};
var MobileCommissionItem = ({ item }) => {
  const { t } = useTranslation();
  return (0, import_jsx_runtime.jsxs)(MobileCard, { children: [
    (0, import_jsx_runtime.jsx)(MobileCell, { label: t("common.date"), children: (0, import_jsx_runtime.jsx)(Text2, { children: formatYMDTime(item.date) }) }),
    (0, import_jsx_runtime.jsx)(MobileCell, { label: t("common.code"), children: (0, import_jsx_runtime.jsx)(Text2, { children: item.code }) }),
    (0, import_jsx_runtime.jsx)(MobileCell, { label: t("common.volume"), align: "end", children: (0, import_jsx_runtime.jsx)(Text2, { children: commifyOptional(item.volume, {
      fix: 2,
      fallback: "0",
      padEnd: true,
      prefix: "$"
    }) }) }),
    (0, import_jsx_runtime.jsx)(MobileCell, { label: t("affiliate.commission"), children: (0, import_jsx_runtime.jsx)(CommissionCell, { item }) }),
    (0, import_jsx_runtime.jsx)(
      MobileCell,
      {
        label: t("common.address"),
        align: "end",
        className: "oui-col-start-3",
        children: (0, import_jsx_runtime.jsx)(AddressCell, { address: item.address, title: t("common.address") })
      }
    )
  ] });
};
var CommissionTableUI = (props) => {
  const { t } = useTranslation();
  const { isMobile } = useScreen();
  const showPagination = (props.pagination?.count ?? 0) >= 10;
  const columns = (0, import_react.useMemo)(() => {
    return [
      {
        title: t("common.date"),
        dataIndex: "date",
        render: (value) => formatYMDTime(value)
      },
      {
        title: t("common.code"),
        dataIndex: "code"
      },
      {
        title: t("common.volume"),
        dataIndex: "volume",
        render: (value) => (0, import_jsx_runtime.jsx)(Text2, { children: commifyOptional(value, {
          fix: 2,
          fallback: "0",
          padEnd: true,
          prefix: "$"
        }) }),
        onSort: true
      },
      {
        title: t("affiliate.commission"),
        dataIndex: "commission",
        render: (_value, record) => (0, import_jsx_runtime.jsx)(CommissionCell, { item: record }),
        onSort: true
      },
      {
        title: t("common.address"),
        dataIndex: "address",
        render: (value) => (0, import_jsx_runtime.jsx)(AddressCell, { address: value, title: t("common.address") })
      }
    ];
  }, [t]);
  return (0, import_jsx_runtime.jsxs)("div", { className: "oui-flex oui-w-full oui-flex-col", children: [
    (0, import_jsx_runtime.jsx)(Flex, { width: "100%", height: 49, className: "oui-border-b oui-border-line-6", children: (0, import_jsx_runtime.jsx)("div", { className: "oui-px-3", children: (0, import_jsx_runtime.jsx)(
      DatePicker2.range,
      {
        size: "xs",
        value: props.dateRange,
        onChange: props.setDateRange,
        max: 90,
        disabled: { after: /* @__PURE__ */ new Date() }
      }
    ) }) }),
    isMobile ? (0, import_jsx_runtime.jsx)("div", { className: "oui-flex oui-flex-col oui-px-4", children: (0, import_jsx_runtime.jsx)(
      ListView,
      {
        dataSource: props.commissionData,
        contentClassName: "!oui-space-y-0 oui-pb-3",
        renderItem: (item, index) => (0, import_jsx_runtime.jsxs)("div", { children: [
          (0, import_jsx_runtime.jsx)(MobileCommissionItem, { item }),
          (0, import_jsx_runtime.jsx)(Divider, { intensity: 8 })
        ] }, index)
      }
    ) }) : (0, import_jsx_runtime.jsx)("div", { className: `oui-px-3 ${showPagination ? "" : "oui-pb-3"}`, children: (0, import_jsx_runtime.jsx)(
      AuthGuardDataTable,
      {
        bordered: true,
        columns,
        dataSource: props.commissionData,
        loading: props.isLoading,
        pagination: showPagination ? props.pagination : void 0,
        onSort: props.onSort,
        onRow: () => ({ className: "oui-h-12" }),
        className: "[&_.oui-h-10.oui-w-full]:!oui-mx-0 [&_.oui-table-pagination]:!oui-justify-end [&_th]:!oui-tracking-[0.03em] [&_th]:!oui-px-3 [&_td]:!oui-px-3"
      }
    ) })
  ] });
};
var CommissionTableWidget = (props) => {
  const state = useCommissionTableScript(props);
  return (0, import_jsx_runtime.jsx)(CommissionTableUI, { ...state });
};
var useMultiLevelReferees = (params = {}) => {
  const { enabled = true, fetchAll = false, page, pageSize } = params;
  const { state } = useAccount();
  const canQuery = enabled && state.status >= AccountStatusEnum.EnableTrading;
  const queryKey = (0, import_react.useMemo)(() => {
    if (!canQuery) return null;
    if (fetchAll) return "/v1/referral/multi_level/referee_list";
    if (page !== void 0 && pageSize !== void 0) {
      return `/v1/referral/multi_level/referee_list?page=${page}&size=${pageSize}`;
    }
    return "/v1/referral/multi_level/referee_list";
  }, [canQuery, fetchAll, page, pageSize]);
  const response = usePrivateQuery(queryKey, {
    formatter: (payload) => {
      const rows = payload.rows ?? payload.data?.rows ?? [];
      const meta = payload.meta ?? payload.data?.meta;
      return {
        ...payload,
        meta,
        rows: rows.map((row) => ({
          ...row,
          network_size: row.direct_invites + row.indirect_invites,
          volume: row.direct_volume + row.indirect_volume,
          commission: row.direct_rebate + row.indirect_rebate + row.direct_bonus_rebate
        }))
      };
    },
    revalidateOnFocus: false
  });
  return {
    ...response,
    isLoading: state.status < AccountStatusEnum.EnableTrading || !enabled || response.isLoading,
    rows: response.data?.rows ?? [],
    meta: response.data?.meta
  };
};
var useRefereesTableScript = (props = {}) => {
  const { enabled = true } = props;
  const { isMobile } = useScreen();
  const refereesPaginationUtils = usePagination();
  const [refereesSort, setRefereesSort] = (0, import_react.useState)(null);
  const {
    isLoading: isRefereesLoading,
    mutate: refereesMutate,
    rows: refereesRows,
    meta: refereesMeta
  } = useMultiLevelReferees({
    enabled,
    fetchAll: isMobile,
    page: refereesPaginationUtils.page,
    pageSize: refereesPaginationUtils.pageSize
  });
  const refereesPagination = (0, import_react.useMemo)(() => {
    return refereesPaginationUtils.parsePagination(refereesMeta);
  }, [refereesMeta, refereesPaginationUtils]);
  const { multiLevelRebateInfo, maxRebateRate, multiLevelRebateInfoMutate } = useReferralContext();
  const onEditReferee = (0, import_react.useCallback)(
    (type, item) => {
      const referrerRebateRate = type === "reset" ? multiLevelRebateInfo?.referrer_rebate_rate : item.referral_rebate_rate;
      modal.show(ReferralCodeFormDialogId, {
        type,
        referralCode: multiLevelRebateInfo?.referral_code,
        maxRebateRate,
        referrerRebateRate,
        onSuccess: () => {
          multiLevelRebateInfoMutate();
          refereesMutate();
        },
        accountId: item.account_id,
        multiLevelRebateInfo,
        directInvites: multiLevelRebateInfo?.direct_invites,
        directBonusRebateRate: new decimal_default(
          multiLevelRebateInfo?.direct_bonus_rebate_rate ?? 0
        ).mul(100).toNumber()
      });
    },
    [
      maxRebateRate,
      multiLevelRebateInfo?.referral_code,
      multiLevelRebateInfo?.referrer_rebate_rate,
      multiLevelRebateInfoMutate,
      refereesMutate
    ]
  );
  const refereesData = (0, import_react.useMemo)(() => {
    const rows = refereesRows ?? [];
    if (!refereesSort) return rows;
    const sortableRows = rows.map((row, index) => ({
      row,
      index
    }));
    sortableRows.sort((a, b) => {
      const valA = Number(a.row[refereesSort.key] ?? 0);
      const valB = Number(b.row[refereesSort.key] ?? 0);
      if (valA < valB) return refereesSort.order === "asc" ? -1 : 1;
      if (valA > valB) return refereesSort.order === "asc" ? 1 : -1;
      return a.index - b.index;
    });
    return sortableRows.map((item) => item.row);
  }, [refereesRows, refereesSort]);
  const onRefereesSort = (sort) => {
    if (!sort) {
      setRefereesSort(null);
      return;
    }
    const sortKey = sort.sortKey;
    if (sortKey && (sortKey === "referee_rebate_rate" || sortKey === "network_size" || sortKey === "volume" || sortKey === "commission")) {
      setRefereesSort({
        key: sortKey,
        order: sort.sort === "asc" ? "asc" : "desc"
      });
    }
  };
  return {
    refereesData,
    refereesPagination,
    isRefereesLoading,
    onEditReferee,
    onRefereesSort
  };
};
var getRebateRateText = (rate) => {
  return (rate * 100).toFixed(0) + "%";
};
var getRefereeType = (bindType) => {
  const { t } = useTranslation();
  if (bindType === "legacy") {
    return {
      text: t("affiliate.singleLevelLegacy"),
      tooltip: t("affiliate.singleLevelLegacy.tooltip")
    };
  }
  return {
    text: t("affiliate.multiLevel"),
    tooltip: t("affiliate.multiLevel.tooltip")
  };
};
var MobileRefereeItem = ({ item, onEditReferee }) => {
  const { t } = useTranslation();
  const typeInfo = getRefereeType(item.bind_type);
  return (0, import_jsx_runtime.jsxs)(MobileCard, { children: [
    (0, import_jsx_runtime.jsx)(MobileCell, { label: t("common.address"), children: (0, import_jsx_runtime.jsx)(AddressCell, { address: item.address, title: t("common.address") }) }),
    (0, import_jsx_runtime.jsx)(MobileCell, { label: t("common.type"), children: (0, import_jsx_runtime.jsx)(
      TooltipCell,
      {
        text: typeInfo.text,
        tooltip: typeInfo.tooltip,
        title: t("common.type")
      }
    ) }),
    (0, import_jsx_runtime.jsx)(MobileCell, { label: t("affiliate.boundAt"), align: "end", children: (0, import_jsx_runtime.jsx)(Text2, { size: "sm", children: formatYMDTime(item.code_binding_time) }) }),
    (0, import_jsx_runtime.jsx)(MobileCell, { label: t("affiliate.referralCodes"), children: (0, import_jsx_runtime.jsx)(Text2, { size: "sm", children: item.bind_code }) }),
    (0, import_jsx_runtime.jsx)(MobileCell, { label: t("affiliate.referralCodes.column.you&Referee"), children: (0, import_jsx_runtime.jsxs)(Text2, { size: "sm", children: [
      getRebateRateText(item.referral_rebate_rate) + "/",
      (0, import_jsx_runtime.jsxs)("span", { className: "oui-text-base-contrast-54", children: [
        getRebateRateText(item.referee_rebate_rate),
        " "
      ] }),
      !item.is_default_rate && (0, import_jsx_runtime.jsx)("span", { className: "oui-text-primary-light", children: `(${t("affiliate.customized")})` })
    ] }) }),
    (0, import_jsx_runtime.jsx)(MobileCell, { label: t("affiliate.networkSize"), align: "end", children: (0, import_jsx_runtime.jsx)(
      BreakdownCell,
      {
        total: item.network_size,
        direct: item.direct_invites,
        indirect: item.indirect_invites,
        title: t("affiliate.networkSize")
      }
    ) }),
    (0, import_jsx_runtime.jsx)(MobileCell, { label: t("common.volume"), children: (0, import_jsx_runtime.jsx)(
      BreakdownCell,
      {
        total: item.volume,
        direct: item.direct_volume,
        indirect: item.indirect_volume,
        prefix: "$",
        fix: 2,
        title: t("common.volume")
      }
    ) }),
    (0, import_jsx_runtime.jsx)(MobileCell, { label: t("affiliate.commission"), children: (0, import_jsx_runtime.jsx)(
      BreakdownCell,
      {
        total: item.commission,
        direct: item.direct_rebate,
        indirect: item.indirect_rebate,
        directBonus: item.direct_bonus_rebate,
        prefix: "$",
        fix: 6,
        title: t("affiliate.commission")
      }
    ) }),
    (0, import_jsx_runtime.jsx)(
      MobileCell,
      {
        label: t("common.action"),
        align: "end",
        className: "oui-col-start-3",
        children: item.bind_type !== "legacy" && (0, import_jsx_runtime.jsxs)(Flex, { gap: 2, children: [
          (0, import_jsx_runtime.jsx)(
            Text2,
            {
              className: "oui-cursor-pointer oui-text-primary-light",
              onClick: () => onEditReferee("edit", item),
              children: t("common.edit")
            }
          ),
          !item.is_default_rate && (0, import_jsx_runtime.jsx)(
            Text2,
            {
              className: "oui-cursor-pointer oui-text-primary-light",
              onClick: () => onEditReferee("reset", item),
              children: t("common.reset")
            }
          )
        ] })
      }
    )
  ] });
};
var RefereesTableUI = (props) => {
  const { t } = useTranslation();
  const { isMobile } = useScreen();
  const showPagination = (props.refereesPagination?.count ?? 0) >= 10;
  const refereeColumns = (0, import_react.useMemo)(() => {
    return [
      {
        title: t("common.address"),
        dataIndex: "address",
        render: (value) => (0, import_jsx_runtime.jsx)(AddressCell, { address: value, title: t("common.address") })
      },
      {
        title: t("common.type"),
        dataIndex: "bind_type",
        render: (_, record) => {
          const typeInfo = getRefereeType(record.bind_type);
          return (0, import_jsx_runtime.jsx)(
            TooltipCell,
            {
              text: typeInfo.text,
              tooltip: typeInfo.tooltip,
              title: t("common.type")
            }
          );
        }
      },
      {
        title: t("affiliate.boundAt"),
        dataIndex: "code_binding_time",
        render: (value) => (0, import_jsx_runtime.jsx)(Text2, { children: formatYMDTime(value) })
      },
      {
        title: t("affiliate.referralCodes"),
        dataIndex: "bind_code",
        render: (value) => (0, import_jsx_runtime.jsx)(Text2, { children: value })
      },
      {
        title: t("affiliate.referralCodes.column.you&Referee"),
        dataIndex: "referee_rebate_rate",
        render: (_, record) => (0, import_jsx_runtime.jsxs)(Text2, { children: [
          getRebateRateText(record.referral_rebate_rate) + "/",
          (0, import_jsx_runtime.jsxs)("span", { className: "oui-text-base-contrast-54", children: [
            getRebateRateText(record.referee_rebate_rate),
            " "
          ] }),
          !record.is_default_rate && (0, import_jsx_runtime.jsx)("span", { className: "oui-text-primary-light", children: `(${t("affiliate.customized")})` })
        ] }),
        onSort: true
      },
      {
        title: t("affiliate.networkSize"),
        dataIndex: "network_size",
        render: (_, record) => (0, import_jsx_runtime.jsx)(
          BreakdownCell,
          {
            total: record.network_size,
            direct: record.direct_invites,
            indirect: record.indirect_invites,
            title: t("affiliate.networkSize")
          }
        ),
        onSort: true
      },
      {
        title: t("common.volume"),
        dataIndex: "volume",
        render: (_, record) => (0, import_jsx_runtime.jsx)(
          BreakdownCell,
          {
            total: record.volume,
            direct: record.direct_volume,
            indirect: record.indirect_volume,
            prefix: "$",
            fix: 2,
            title: t("common.volume")
          }
        ),
        onSort: true
      },
      {
        title: t("affiliate.commission"),
        dataIndex: "commission",
        render: (_, record) => (0, import_jsx_runtime.jsx)(
          BreakdownCell,
          {
            total: record.commission,
            direct: record.direct_rebate,
            indirect: record.indirect_rebate,
            directBonus: record.direct_bonus_rebate,
            prefix: "$",
            fix: 6,
            title: t("affiliate.commission")
          }
        ),
        onSort: true
      },
      {
        title: t("common.action"),
        dataIndex: "action",
        render: (_, record) => record.bind_type !== "legacy" ? (0, import_jsx_runtime.jsxs)(import_jsx_runtime.Fragment, { children: [
          (0, import_jsx_runtime.jsx)(
            Text2,
            {
              className: "oui-cursor-pointer oui-text-primary-light",
              onClick: () => props.onEditReferee("edit", record),
              children: t("common.edit")
            }
          ),
          !record.is_default_rate && (0, import_jsx_runtime.jsx)(
            Text2,
            {
              className: "oui-ml-2 oui-cursor-pointer oui-text-primary-light",
              onClick: () => props.onEditReferee("reset", record),
              children: t("common.reset")
            }
          )
        ] }) : null
      }
    ];
  }, [t, props.onEditReferee]);
  return (0, import_jsx_runtime.jsx)(import_jsx_runtime.Fragment, { children: isMobile ? (0, import_jsx_runtime.jsx)("div", { className: "oui-flex oui-flex-col oui-px-4", children: (0, import_jsx_runtime.jsx)(
    ListView,
    {
      dataSource: props.refereesData,
      contentClassName: "!oui-space-y-0 oui-pb-3",
      renderItem: (item, index) => (0, import_jsx_runtime.jsxs)("div", { children: [
        (0, import_jsx_runtime.jsx)(
          MobileRefereeItem,
          {
            item,
            onEditReferee: props.onEditReferee
          }
        ),
        (0, import_jsx_runtime.jsx)(Divider, { intensity: 8 })
      ] }, index)
    }
  ) }) : (0, import_jsx_runtime.jsx)("div", { className: `oui-px-3 ${showPagination ? "" : "oui-pb-3"}`, children: (0, import_jsx_runtime.jsx)(
    AuthGuardDataTable,
    {
      bordered: true,
      columns: refereeColumns,
      dataSource: props.refereesData,
      loading: props.isRefereesLoading,
      pagination: showPagination ? props.refereesPagination : void 0,
      onSort: props.onRefereesSort,
      onRow: () => ({ className: "oui-h-12" }),
      className: "[&_.oui-h-10.oui-w-full]:!oui-mx-0 [&_.oui-table-pagination]:!oui-justify-end [&_th]:!oui-tracking-[0.03em] [&_th]:!oui-px-3 [&_td]:!oui-px-3"
    }
  ) }) });
};
var RefereesTableWidget = (props) => {
  const state = useRefereesTableScript(props);
  return (0, import_jsx_runtime.jsx)(RefereesTableUI, { ...state });
};
var useMultiLevelReferralCodes = () => {
  const { referralInfo } = useReferralContext();
  const { data: multiLevelRebateInfo } = useMultiLevelRebateInfo();
  const [refereesData] = useRefereeInfo({});
  const copyCode = (code) => {
    copyText(code);
  };
  const singleLevelTotals = (0, import_react.useMemo)(() => {
    const totalsMap = /* @__PURE__ */ new Map();
    if (refereesData) {
      refereesData.forEach((referee) => {
        const code = referee?.referral_code;
        if (!code) return;
        const existing = totalsMap.get(code) || { volume: 0, commission: 0 };
        totalsMap.set(code, {
          volume: existing.volume + (referee?.volume || 0),
          commission: existing.commission + (referee?.referral_rebate || 0)
        });
      });
    }
    return totalsMap;
  }, [refereesData]);
  const codes = (0, import_react.useMemo)(() => {
    const allCodes = [];
    const multiLevelReferralCode = multiLevelRebateInfo?.referral_code;
    if (multiLevelRebateInfo?.referral_code) {
      const maxRebateRate = multiLevelRebateInfo.max_rebate_rate ?? 0;
      const defaultRefereeRebateRate = multiLevelRebateInfo.default_referee_rebate_rate ?? 0;
      const referrerRebateRate = Math.max(
        0,
        maxRebateRate - defaultRefereeRebateRate
      );
      const directInvites = multiLevelRebateInfo?.direct_invites ?? 0;
      const indirectInvites = multiLevelRebateInfo?.indirect_invites ?? 0;
      const directVolume = multiLevelRebateInfo?.direct_volume ?? 0;
      const indirectVolume = multiLevelRebateInfo?.indirect_volume ?? 0;
      const directRebate = multiLevelRebateInfo?.direct_rebate ?? 0;
      const indirectRebate = multiLevelRebateInfo?.indirect_rebate ?? 0;
      const directBonusRebate = multiLevelRebateInfo?.direct_bonus_rebate ?? 0;
      const multiLevelCode = {
        code: multiLevelRebateInfo.referral_code ?? "",
        direct_invites: directInvites,
        indirect_invites: indirectInvites,
        direct_volume: directVolume,
        indirect_volume: indirectVolume,
        direct_rebate: directRebate,
        indirect_rebate: indirectRebate,
        direct_bonus_rebate: directBonusRebate,
        max_rebate_rate: maxRebateRate,
        referee_rebate_rate: defaultRefereeRebateRate,
        referrer_rebate_rate: referrerRebateRate,
        total_invites: directInvites + indirectInvites,
        total_volume: directVolume + indirectVolume,
        total_rebate: directRebate + indirectRebate + directBonusRebate,
        referral_type: "multi"
      };
      allCodes.push(multiLevelCode);
    }
    if (referralInfo?.referrer_info?.referral_codes) {
      allCodes.push(
        ...referralInfo.referrer_info.referral_codes.filter(
          (item) => multiLevelReferralCode ? item.code !== multiLevelReferralCode : true
        ).map((item) => {
          const totals = singleLevelTotals.get(item.code);
          const totalVolume = totals?.volume ?? item.total_volume ?? 0;
          const totalRebate = totals?.commission ?? item.total_rebate ?? 0;
          const totalInvites = item.total_invites ?? 0;
          const row = {
            code: item.code,
            referral_type: "single",
            direct_invites: totalInvites,
            indirect_invites: 0,
            direct_volume: totalVolume,
            indirect_volume: 0,
            direct_rebate: totalRebate,
            indirect_rebate: 0,
            max_rebate_rate: item.max_rebate_rate ?? 0,
            referee_rebate_rate: item.referee_rebate_rate ?? 0,
            referrer_rebate_rate: item.referrer_rebate_rate ?? 0,
            total_invites: totalInvites,
            total_volume: totalVolume,
            total_rebate: totalRebate
          };
          return row;
        })
      );
    }
    if (allCodes.length === 0) return void 0;
    return allCodes;
  }, [
    referralInfo?.referrer_info?.referral_codes,
    multiLevelRebateInfo,
    singleLevelTotals
  ]);
  return {
    codes,
    copyCode
  };
};
var useReferralCodesTableScript = (props = {}) => {
  const { enabled = true } = props;
  const { codes, copyCode } = useMultiLevelReferralCodes();
  const [sort, setSort] = (0, import_react.useState)(null);
  const sortedCodes = (0, import_react.useMemo)(() => {
    if (!enabled) return void 0;
    if (!codes || !sort) return codes;
    const sortableCodes = codes.map((code, index) => ({
      code,
      index
    }));
    sortableCodes.sort((a, b) => {
      const valA = Number(a.code[sort.key] ?? 0);
      const valB = Number(b.code[sort.key] ?? 0);
      if (valA < valB) return sort.order === "asc" ? -1 : 1;
      if (valA > valB) return sort.order === "asc" ? 1 : -1;
      return a.index - b.index;
    });
    return sortableCodes.map((item) => item.code);
  }, [codes, enabled, sort]);
  const onSort = (sort2) => {
    if (!sort2) {
      setSort(null);
      return;
    }
    const sortKey = sort2.sortKey;
    if (sortKey && (sortKey === "total_invites" || sortKey === "total_volume" || sortKey === "total_rebate")) {
      setSort({
        key: sortKey,
        order: sort2.sort === "asc" ? "asc" : "desc"
      });
    }
  };
  return {
    codes,
    sortedCodes,
    copyCode,
    onSort,
    isLoading: enabled && !codes
  };
};
var getReferralCodeType = (referralType) => {
  const { t } = useTranslation();
  if (referralType === "multi") {
    return {
      text: t("affiliate.multiLevel"),
      tooltip: t("affiliate.multiLevel.tooltip")
    };
  }
  return {
    text: t("affiliate.singleLevelLegacy"),
    tooltip: t("affiliate.singleLevelLegacy.tooltip")
  };
};
var MobileReferralCodeItem = ({ item, copyCode }) => {
  const { t } = useTranslation();
  const referrerRate = new decimal_default(item.referrer_rebate_rate ?? 0).mul(100).toFixed(0);
  const refereeRate = new decimal_default(item.referee_rebate_rate ?? 0).mul(100).toFixed(0);
  const typeInfo = getReferralCodeType(item.referral_type);
  return (0, import_jsx_runtime.jsxs)(MobileCard, { children: [
    (0, import_jsx_runtime.jsx)(MobileCell, { label: t("affiliate.referralCode"), children: (0, import_jsx_runtime.jsx)(Text2, { children: item.code || "" }) }),
    (0, import_jsx_runtime.jsx)(MobileCell, { label: t("common.type"), children: (0, import_jsx_runtime.jsx)(
      TooltipCell,
      {
        text: typeInfo.text,
        tooltip: typeInfo.tooltip,
        title: t("common.type")
      }
    ) }),
    (0, import_jsx_runtime.jsx)(
      MobileCell,
      {
        label: t("affiliate.referralCodes.column.you&Referee"),
        align: "end",
        children: (0, import_jsx_runtime.jsxs)(Text2, { children: [
          referrerRate,
          "%/",
          (0, import_jsx_runtime.jsxs)("span", { className: "oui-text-base-contrast-54", children: [
            refereeRate,
            "%"
          ] })
        ] })
      }
    ),
    (0, import_jsx_runtime.jsx)(MobileCell, { label: t("affiliate.networkSize"), children: (0, import_jsx_runtime.jsx)(
      BreakdownCell,
      {
        total: item.total_invites ?? 0,
        direct: item.direct_invites ?? 0,
        indirect: item.indirect_invites ?? 0,
        title: t("affiliate.networkSize")
      }
    ) }),
    (0, import_jsx_runtime.jsx)(MobileCell, { label: t("common.volume"), children: (0, import_jsx_runtime.jsx)(
      BreakdownCell,
      {
        total: item.total_volume ?? 0,
        direct: item.direct_volume ?? 0,
        indirect: item.indirect_volume ?? 0,
        prefix: "$",
        fix: 2,
        title: t("common.volume")
      }
    ) }),
    (0, import_jsx_runtime.jsx)(MobileCell, { label: t("affiliate.commission"), align: "end", children: (0, import_jsx_runtime.jsx)(
      BreakdownCell,
      {
        total: item.total_rebate ?? 0,
        direct: item.direct_rebate ?? 0,
        indirect: item.indirect_rebate ?? 0,
        directBonus: item.direct_bonus_rebate,
        prefix: "$",
        fix: 6,
        title: t("affiliate.commission")
      }
    ) }),
    (0, import_jsx_runtime.jsx)(MobileCell, { label: t("common.action"), children: (0, import_jsx_runtime.jsx)(
      Text2,
      {
        className: "oui-cursor-pointer oui-text-primary-light",
        onClick: (e) => {
          e.stopPropagation();
          copyCode(item.code);
        },
        children: t("common.copy")
      }
    ) })
  ] });
};
var ReferralCodesTableUI = (props) => {
  const { t } = useTranslation();
  const { isMobile } = useScreen();
  const columns = (0, import_react.useMemo)(() => {
    return [
      {
        title: t("affiliate.referralCode"),
        dataIndex: "code",
        render: (value) => (0, import_jsx_runtime.jsx)(Text2, { children: value || "" })
      },
      {
        title: t("common.type"),
        dataIndex: "referral_type",
        render: (_, record) => {
          const typeInfo = getReferralCodeType(record.referral_type);
          return (0, import_jsx_runtime.jsx)(
            TooltipCell,
            {
              text: typeInfo.text,
              tooltip: typeInfo.tooltip,
              title: t("common.type")
            }
          );
        }
      },
      {
        title: (0, import_jsx_runtime.jsxs)(Text2, { children: [
          t("affiliate.referralCodes.column.defaultSplit"),
          (0, import_jsx_runtime.jsx)("br", {}),
          "(",
          t("affiliate.referralCodes.column.you&Referee"),
          ")"
        ] }),
        dataIndex: "rate",
        render: (_, record) => {
          const referrerRate = new decimal_default(record.referrer_rebate_rate ?? 0).mul(100).toFixed(0);
          const refereeRate = new decimal_default(record.referee_rebate_rate ?? 0).mul(100).toFixed(0);
          return (0, import_jsx_runtime.jsxs)(Text2, { children: [
            referrerRate,
            "%",
            (0, import_jsx_runtime.jsxs)("span", { className: "oui-text-base-contrast-54", children: [
              "/",
              refereeRate,
              "%"
            ] })
          ] });
        }
      },
      {
        title: t("affiliate.networkSize"),
        dataIndex: "total_invites",
        render: (_, record) => {
          return (0, import_jsx_runtime.jsx)(
            BreakdownCell,
            {
              total: record.total_invites ?? 0,
              direct: record.direct_invites ?? 0,
              indirect: record.indirect_invites ?? 0,
              title: t("affiliate.networkSize")
            }
          );
        },
        onSort: true
      },
      {
        title: t("common.volume"),
        dataIndex: "total_volume",
        render: (_, record) => {
          return (0, import_jsx_runtime.jsx)(
            BreakdownCell,
            {
              total: record.total_volume ?? 0,
              direct: record.direct_volume ?? 0,
              indirect: record.indirect_volume ?? 0,
              prefix: "$",
              fix: 2,
              title: t("common.volume")
            }
          );
        },
        onSort: true
      },
      {
        title: t("affiliate.commission"),
        dataIndex: "total_rebate",
        render: (_, record) => {
          return (0, import_jsx_runtime.jsx)(
            BreakdownCell,
            {
              total: record.total_rebate ?? 0,
              direct: record.direct_rebate ?? 0,
              indirect: record.indirect_rebate ?? 0,
              directBonus: record.direct_bonus_rebate,
              prefix: "$",
              fix: 6,
              title: t("affiliate.commission")
            }
          );
        },
        onSort: true
      },
      {
        title: t("common.action"),
        dataIndex: "action",
        render: (_, record) => (0, import_jsx_runtime.jsx)(
          Text2,
          {
            className: "oui-cursor-pointer oui-text-primary-light",
            onClick: (e) => {
              e.stopPropagation();
              props.copyCode(record.code);
            },
            children: t("common.copy")
          }
        )
      }
    ];
  }, [t, props.copyCode]);
  return (0, import_jsx_runtime.jsx)(import_jsx_runtime.Fragment, { children: isMobile ? (0, import_jsx_runtime.jsx)("div", { className: "oui-flex oui-flex-col oui-px-4", children: (0, import_jsx_runtime.jsx)(
    ListView,
    {
      dataSource: props.sortedCodes,
      contentClassName: "!oui-space-y-0 oui-pb-3",
      renderItem: (item, index) => (0, import_jsx_runtime.jsxs)("div", { children: [
        (0, import_jsx_runtime.jsx)(MobileReferralCodeItem, { item, copyCode: props.copyCode }),
        (0, import_jsx_runtime.jsx)(Divider, { intensity: 8 })
      ] }, index)
    }
  ) }) : (0, import_jsx_runtime.jsx)("div", { className: "oui-px-3", children: (0, import_jsx_runtime.jsx)(
    AuthGuardDataTable,
    {
      bordered: true,
      columns,
      dataSource: props.sortedCodes,
      loading: props.isLoading,
      onSort: props.onSort,
      onRow: () => ({ className: "oui-h-12" }),
      className: "oui-pb-3 [&_.oui-h-10.oui-w-full]:!oui-mx-0 [&_.oui-table-pagination]:!oui-justify-end [&_th]:!oui-tracking-[0.03em] [&_th]:!oui-px-3 [&_td]:!oui-px-3"
    }
  ) }) });
};
var ReferralCodesTableWidget = (props) => {
  const state = useReferralCodesTableScript(props);
  return (0, import_jsx_runtime.jsx)(ReferralCodesTableUI, { ...state });
};
var ReferrerTableUI = (props) => {
  const { t } = useTranslation();
  return (0, import_jsx_runtime.jsx)(
    Flex,
    {
      direction: "column",
      className: "oui-rounded-2xl oui-bg-base-9 [&_td]:oui-font-medium [&_td]:oui-tracking-[0.03em] [&_th]:oui-font-semibold",
      px: 3,
      pt: 4,
      gap: 4,
      width: "100%",
      children: (0, import_jsx_runtime.jsxs)(
        Tabs,
        {
          value: props.activeTab,
          onValueChange: (value) => props.setActiveTab(value),
          className: "oui-w-full oui-text-base",
          variant: "text",
          classNames: {
            tabsListContainer: "oui-px-3",
            trigger: "oui-pb-[9px] oui-tracking-[0.01em]",
            scrollIndicator: "[&_.oui-scroll-indicator-leading]:oui-hidden [&_.oui-scroll-indicator-tailing]:oui-hidden"
          },
          showScrollIndicator: true,
          children: [
            (0, import_jsx_runtime.jsx)(TabPanel, { value: "commission", title: t("affiliate.commission"), children: (0, import_jsx_runtime.jsx)(CommissionTableWidget, { enabled: props.activeTab === "commission" }) }),
            (0, import_jsx_runtime.jsx)(TabPanel, { value: "referees", title: t("affiliate.referees"), children: (0, import_jsx_runtime.jsx)(RefereesTableWidget, { enabled: props.activeTab === "referees" }) }),
            (0, import_jsx_runtime.jsx)(TabPanel, { value: "codes", title: t("affiliate.referralCodes"), children: (0, import_jsx_runtime.jsx)(ReferralCodesTableWidget, { enabled: props.activeTab === "codes" }) })
          ]
        }
      )
    }
  );
};
var ReferrerTableWidget = () => {
  const state = useReferrerTableScript();
  return (0, import_jsx_runtime.jsx)(ReferrerTableUI, { ...state });
};
var Summary = (props) => {
  const { statistics } = props;
  const { t } = useTranslation();
  return (0, import_jsx_runtime.jsxs)(
    Flex,
    {
      id: "oui-affiliate-affiliate-summary",
      r: "2xl",
      p: 5,
      width: "100%",
      height: "100%",
      gap: 6,
      direction: "column",
      intensity: 900,
      className: "oui-border oui-border-line-6",
      children: [
        (0, import_jsx_runtime.jsxs)(Flex, { direction: "row", justify: "between", width: "100%", children: [
          (0, import_jsx_runtime.jsx)(Text2, { className: "oui-text-lg", children: t("affiliate.summary") }),
          (0, import_jsx_runtime.jsx)("div", { className: "oui-min-w-14", children: (0, import_jsx_runtime.jsx)(
            Select2.options,
            {
              size: "xs",
              value: props.period,
              onValueChange: props.onPeriodChange,
              options: props.periodTypes
            }
          ) })
        ] }),
        (0, import_jsx_runtime.jsx)(
          SummaryItem,
          {
            prefix: "$",
            rule: "human",
            label: `${t("affiliate.commission")} (USDC)`,
            value: statistics.total_rebate,
            direct: statistics.direct_rebate + statistics.direct_bonus_rebate,
            indirect: statistics.indirect_rebate,
            classNames: {
              root: "!oui-py-12",
              value: "oui-text-trade-profit",
              direct: "oui-text-trade-profit",
              indirect: "oui-text-trade-profit"
            }
          }
        ),
        (0, import_jsx_runtime.jsxs)(Flex, { width: "100%", gap: 6, className: "oui-flex-col md:oui-flex-row", children: [
          (0, import_jsx_runtime.jsx)(
            SummaryItem,
            {
              prefix: "$",
              rule: "human",
              label: `${t("affiliate.referralVol")} (USDC)`,
              value: statistics.total_volume,
              direct: statistics.direct_volume,
              indirect: statistics.indirect_volume,
              classNames: {
                root: "oui-w-full md:oui-w-1/2"
              }
            }
          ),
          (0, import_jsx_runtime.jsx)(
            SummaryItem,
            {
              dp: 0,
              label: t("affiliate.referrals"),
              value: statistics.total_invites,
              direct: statistics.direct_invites,
              indirect: statistics.indirect_invites,
              classNames: {
                root: "oui-w-full md:oui-w-1/2"
              }
            }
          )
        ] })
      ]
    }
  );
};
var SummaryItem = (props) => {
  const { t } = useTranslation();
  const { rule, dp = 2 } = props;
  return (0, import_jsx_runtime.jsxs)(
    Flex,
    {
      r: "2xl",
      width: "100%",
      direction: "column",
      itemAlign: "start",
      gap: 2,
      className: M(
        "oui-bg-base-contrast-4 oui-p-5 md:oui-py-[38px]",
        "oui-text-sm oui-text-base-contrast-54",
        props.classNames?.root
      ),
      children: [
        (0, import_jsx_runtime.jsx)(Text2, { children: props.label }),
        (0, import_jsx_runtime.jsx)(
          Text2.numeral,
          {
            rule,
            dp: props.dp,
            size: "3xl",
            prefix: props.prefix,
            placeholder: "--",
            intensity: 98,
            className: props.classNames?.value,
            children: props.value
          }
        ),
        (0, import_jsx_runtime.jsxs)(Flex, { className: "oui-flex-wrap oui-text-base-contrast-54", children: [
          (0, import_jsx_runtime.jsxs)(Flex, { gap: 1, children: [
            (0, import_jsx_runtime.jsxs)(Text2, { children: [
              t("affiliate.direct"),
              ":"
            ] }),
            (0, import_jsx_runtime.jsx)(
              Text2.numeral,
              {
                rule,
                dp: props.dp,
                prefix: props.prefix,
                intensity: 54,
                className: props.classNames?.direct,
                children: props.direct
              }
            )
          ] }),
          "/",
          (0, import_jsx_runtime.jsxs)(Flex, { gap: 1, children: [
            (0, import_jsx_runtime.jsxs)(Text2, { children: [
              t("affiliate.indirect"),
              ":"
            ] }),
            (0, import_jsx_runtime.jsx)(
              Text2.numeral,
              {
                rule,
                dp: props.dp,
                prefix: props.prefix,
                intensity: 54,
                className: props.classNames?.indirect,
                children: props.indirect
              }
            )
          ] })
        ] })
      ]
    }
  );
};
var useSummaryScript = () => {
  const { t } = useTranslation();
  const [period, setPeriod] = (0, import_react.useState)(
    "all_time"
    /* All */
  );
  const { data } = useMultiLevelStatistics(period);
  const periodTypes = [
    {
      label: t("common.all"),
      value: "all_time"
      /* All */
    },
    {
      label: t("common.select.1d"),
      value: "1d"
      /* 1d */
    },
    {
      label: t("common.select.7d"),
      value: "7d"
      /* 7d */
    },
    {
      label: t("common.select.30d"),
      value: "30d"
      /* 30d */
    }
  ];
  const onPeriodChange = (item) => {
    setPeriod(item);
  };
  const statistics = (0, import_react.useMemo)(() => {
    const {
      direct_invites = 0,
      indirect_invites = 0,
      direct_traded = 0,
      indirect_traded = 0,
      direct_volume = 0,
      indirect_volume = 0,
      direct_rebate = 0,
      indirect_rebate = 0,
      direct_bonus_rebate = 0
    } = data || {};
    return {
      direct_invites,
      indirect_invites,
      total_invites: direct_invites + indirect_invites,
      direct_traded,
      indirect_traded,
      total_traded: direct_traded + indirect_traded,
      direct_volume,
      indirect_volume,
      total_volume: direct_volume + indirect_volume,
      direct_rebate,
      indirect_rebate,
      direct_bonus_rebate,
      total_rebate: direct_rebate + indirect_rebate + direct_bonus_rebate
    };
  }, [data]);
  return {
    period,
    periodTypes,
    onPeriodChange,
    statistics
  };
};
var SummaryWidget = () => {
  const state = useSummaryScript();
  return (0, import_jsx_runtime.jsx)(Summary, { ...state });
};
var MultiLevelAffiliatePage = () => {
  const { t } = useTranslation();
  const { multiLevelRebateInfo } = useReferralContext();
  return (0, import_jsx_runtime.jsx)(
    Flex,
    {
      id: "oui-affiliate-affiliate-page",
      className: M("oui-h-lvw", "oui-font-semibold", "oui-pb-24 oui-pt-16"),
      direction: "column",
      gap: 4,
      children: (0, import_jsx_runtime.jsx)("div", { className: "oui-w-full oui-px-5 md:oui-mx-auto md:oui-max-w-[1040px]", children: (0, import_jsx_runtime.jsxs)(
        Flex,
        {
          width: "100%",
          direction: "column",
          className: "oui-gap-8 md:oui-gap-10",
          children: [
            (0, import_jsx_runtime.jsxs)(Flex, { width: "100%", direction: "column", itemAlign: "start", gap: 4, children: [
              (0, import_jsx_runtime.jsx)(Text2, { className: "oui-text-[32px] oui-leading-[40px] md:oui-text-6xl md:oui-leading-[72px]", children: t("affiliate.earnReferralCommissions") }),
              (0, import_jsx_runtime.jsx)(Text2, { size: "sm", intensity: 54, children: t("affiliate.earnReferralCommissions.description") })
            ] }),
            (0, import_jsx_runtime.jsxs)(
              Grid2,
              {
                width: "100%",
                height: "100%",
                gap: 6,
                cols: 1,
                rows: 1,
                className: "md:oui-grid-cols-2",
                children: [
                  (0, import_jsx_runtime.jsx)("div", { className: "oui-order-2 md:oui-order-1", children: (0, import_jsx_runtime.jsx)(SummaryWidget, {}) }),
                  (0, import_jsx_runtime.jsx)("div", { className: "oui-order-1 md:oui-order-2", children: multiLevelRebateInfo?.referral_code ? (0, import_jsx_runtime.jsx)(ReferralInfoWidget, {}) : (0, import_jsx_runtime.jsx)(MultiLevelReferralWidget, {}) })
                ]
              }
            ),
            (0, import_jsx_runtime.jsx)(CommissionChartWidget, {}),
            (0, import_jsx_runtime.jsx)(ReferrerTableWidget, {})
          ]
        }
      ) })
    }
  );
};
var AffiliateIcon = (props) => {
  const { size = 16, className, ...rest } = props;
  return (0, import_jsx_runtime.jsxs)(
    "svg",
    {
      width: size,
      height: size,
      viewBox: "0 0 16 16",
      fill: "#fff",
      fillOpacity: ".98",
      xmlns: "http://www.w3.org/2000/svg",
      className: props.className,
      ...rest,
      children: [
        (0, import_jsx_runtime.jsx)("path", { d: "M8 1.302c-3.683 0-6.655 2.985-6.667 6.667-.013 3.676 2.987 6.68 6.666 6.687s6.674-3.037 6.667-6.687c-.007-3.682-2.985-6.667-6.667-6.667m0 1.333a5.333 5.333 0 0 1 5.333 5.334c0 1.5-.625 2.852-1.622 3.821-.49-1.062-1.545-1.821-2.753-1.821H7.04c-1.207 0-2.259.75-2.75 1.812A5.3 5.3 0 0 1 2.666 7.97a5.333 5.333 0 0 1 5.333-5.334M8 3.97a2.667 2.667 0 1 0 0 5.333A2.667 2.667 0 0 0 8 3.97" }),
        (0, import_jsx_runtime.jsx)(
          "path",
          {
            d: "M16 12.667a3.333 3.333 0 1 1-6.667 0 3.333 3.333 0 0 1 6.667 0",
            fill: "#1828C3"
          }
        ),
        (0, import_jsx_runtime.jsx)("path", { d: "M11.917 11.433a.917.917 0 0 0-.917.917c0 .53.302 1.031.802 1.49a4.6 4.6 0 0 0 .713.53l.068.043c.05.027.117.027.167 0l.068-.042q.08-.05.177-.115c.182-.123.366-.26.536-.416.5-.459.802-.96.802-1.49a.917.917 0 0 0-.917-.917.97.97 0 0 0-.744.386.95.95 0 0 0-.756-.386" })
      ]
    }
  );
};
var TraderIcon = (props) => {
  const { size = 16, className, ...rest } = props;
  return (0, import_jsx_runtime.jsxs)(
    "svg",
    {
      width: size,
      height: size,
      viewBox: "0 0 16 16",
      fill: "white",
      fillOpacity: "0.36",
      xmlns: "http://www.w3.org/2000/svg",
      className: props.className,
      ...rest,
      children: [
        (0, import_jsx_runtime.jsx)("path", { d: "M7.99929 1.30212C4.31735 1.30212 1.34509 4.28679 1.33262 7.96878C1.32015 11.6454 4.32042 14.6488 7.99929 14.6561C11.6782 14.6634 14.6726 11.6194 14.666 7.96878C14.6593 4.28679 11.6812 1.30212 7.99929 1.30212ZM7.99929 2.63546C10.9448 2.63546 13.3326 5.02345 13.3326 7.96878C13.3326 9.46878 12.7078 10.8208 11.7112 11.7901C11.2202 10.7281 10.1656 9.96878 8.95762 9.96878C8.71802 9.96878 7.28042 9.96878 7.04095 9.96878C5.83369 9.96878 4.78215 10.7188 4.29095 11.7814C3.29429 10.8121 2.66595 9.46878 2.66595 7.96878C2.66595 5.02345 5.05375 2.63546 7.99929 2.63546ZM7.99929 3.96879C6.52649 3.96879 5.33255 5.16279 5.33262 6.63545C5.33262 8.10812 6.52649 9.30212 7.99929 9.30212C9.47202 9.30212 10.666 8.10812 10.666 6.63545C10.666 5.16279 9.47202 3.96879 7.99929 3.96879Z" }),
        (0, import_jsx_runtime.jsx)(
          "path",
          {
            d: "M16 12.6667C16 14.5076 14.5076 16 12.6666 16C10.8257 16 9.33331 14.5076 9.33331 12.6667C9.33331 10.8257 10.8257 9.33333 12.6666 9.33333C14.5076 9.33333 16 10.8257 16 12.6667Z",
            fill: "#005A4F"
          }
        ),
        (0, import_jsx_runtime.jsx)(
          "path",
          {
            d: "M14.3325 13.3345C14.3325 13.2918 14.3182 13.2473 14.2857 13.2147L13.6658 12.6002L13.4315 12.8345L13.7595 13.1678H11.3325C11.2405 13.1678 11.1658 13.2425 11.1658 13.3345C11.1658 13.4265 11.2405 13.5012 11.3325 13.5012H13.7595L13.4315 13.8345L13.6658 14.0688L14.2857 13.4543C14.3182 13.4217 14.3325 13.3772 14.3325 13.3345ZM14.1658 12.0012C14.1658 11.9092 14.0912 11.8345 13.9992 11.8345H11.572L11.9002 11.5012L11.6658 11.2668L11.046 11.8813C10.981 11.9465 10.981 12.0558 11.046 12.121L11.6658 12.7355L11.9002 12.5012L11.572 12.1678H13.9992C14.0912 12.1678 14.1658 12.0932 14.1658 12.0012Z",
            fill: "white",
            fillOpacity: "0.98"
          }
        )
      ]
    }
  );
};
var USDCIcon = (props) => {
  return (0, import_jsx_runtime.jsxs)(
    "svg",
    {
      width: "21",
      height: "20",
      viewBox: "0 0 21 20",
      fill: "none",
      xmlns: "http://www.w3.org/2000/svg",
      className: props.className,
      children: [
        (0, import_jsx_runtime.jsxs)("g", { clipPath: "url(#a)", children: [
          (0, import_jsx_runtime.jsx)(
            "path",
            {
              d: "M10.5 20c5.542 0 10-4.458 10-10s-4.458-10-10-10S.5 4.458.5 10s4.458 10 10 10",
              fill: "#2775CA"
            }
          ),
          (0, import_jsx_runtime.jsx)(
            "path",
            {
              d: "M13.5 11.473c0-1.452-.94-1.95-2.82-2.158-1.344-.166-1.613-.498-1.613-1.078s.448-.955 1.343-.955c.806 0 1.254.25 1.478.872.045.124.18.207.313.207h.717c.179 0 .313-.125.313-.29v-.042c-.179-.913-.985-1.618-2.015-1.701v-.996c0-.166-.134-.29-.358-.332h-.671c-.18 0-.314.124-.359.332v.954c-1.343.166-2.194.996-2.194 2.034 0 1.369.896 1.908 2.776 2.116 1.254.207 1.657.456 1.657 1.12s-.627 1.12-1.477 1.12c-1.165 0-1.568-.456-1.702-1.078a.32.32 0 0 0-.313-.25h-.762c-.179 0-.313.125-.313.291v.042c.179 1.037.895 1.784 2.373 1.991v.996c0 .166.134.29.358.332h.672c.18 0 .313-.124.358-.332v-.996c1.344-.207 2.239-1.079 2.239-2.199",
              fill: "#fff",
              fillOpacity: ".98"
            }
          ),
          (0, import_jsx_runtime.jsx)(
            "path",
            {
              d: "M8.24 16.57c-3.458-1.215-5.23-4.986-3.945-8.324.665-1.821 2.127-3.208 3.944-3.859.178-.086.266-.216.266-.433v-.607c0-.174-.088-.304-.266-.347-.044 0-.133 0-.177.043-4.21 1.3-6.516 5.68-5.186 9.798.798 2.428 2.704 4.292 5.186 5.072.177.087.355 0 .4-.173.043-.043.043-.087.043-.173v-.608c0-.13-.133-.303-.266-.39m4.698-13.527c-.178-.086-.355 0-.4.174-.043.043-.043.086-.043.173v.607c0 .174.133.347.266.434 3.457 1.213 5.23 4.985 3.944 8.323-.664 1.821-2.127 3.209-3.944 3.859-.178.086-.266.216-.266.433v.607c0 .174.088.304.266.347.044 0 .133 0 .177-.043 4.21-1.3 6.515-5.68 5.186-9.798-.798-2.471-2.748-4.335-5.186-5.116",
              fill: "#fff",
              fillOpacity: ".98"
            }
          )
        ] }),
        (0, import_jsx_runtime.jsx)("defs", { children: (0, import_jsx_runtime.jsx)("clipPath", { id: "a", children: (0, import_jsx_runtime.jsx)("path", { fill: "#fff", d: "M.5 0h20v20H.5z" }) }) })
      ]
    }
  );
};
var Summary2 = (props) => {
  const { t } = useTranslation();
  return (0, import_jsx_runtime.jsxs)(
    Flex,
    {
      id: "oui-affiliate-affiliate-summary",
      r: "2xl",
      p: 6,
      width: "100%",
      height: "100%",
      gap: 4,
      direction: "column",
      className: "oui-bg-base-9",
      children: [
        (0, import_jsx_runtime.jsx)(Title, { ...props }),
        (0, import_jsx_runtime.jsx)(CommissionData, { ...props }),
        (0, import_jsx_runtime.jsxs)(Flex, { direction: "column", width: "100%", gap: 2, children: [
          (0, import_jsx_runtime.jsx)(
            Row,
            {
              title: `${t("affiliate.referralVol")} (USDC)`,
              value: props.referralVol,
              dp: 2,
              ...props
            }
          ),
          (0, import_jsx_runtime.jsx)(
            Row,
            {
              title: t("affiliate.referees"),
              value: props.referees,
              dp: 0,
              ...props
            }
          ),
          (0, import_jsx_runtime.jsx)(
            Row,
            {
              title: t("affiliate.summary.refereesTraded"),
              value: props.refereesTades,
              dp: 0,
              ...props
            }
          )
        ] })
      ]
    }
  );
};
var Title = (props) => {
  const { t } = useTranslation();
  return (0, import_jsx_runtime.jsxs)(Flex, { direction: "row", justify: "between", width: "100%", children: [
    (0, import_jsx_runtime.jsx)(Text2, { className: "oui-text-lg", children: t("affiliate.summary") }),
    (0, import_jsx_runtime.jsx)("div", { className: "oui-min-w-14", children: (0, import_jsx_runtime.jsx)(
      Select2.options,
      {
        size: "xs",
        value: props.period,
        onValueChange: props.onPeriodChange,
        options: props.periodTypes
      }
    ) })
  ] });
};
var CommissionData = (props) => {
  const { t } = useTranslation();
  return (0, import_jsx_runtime.jsxs)(
    Flex,
    {
      gradient: "primary",
      angle: 180,
      r: "xl",
      py: 4,
      px: 6,
      width: "100%",
      direction: "column",
      gap: 3,
      height: "100%",
      className: "oui-max-h-[104px]",
      children: [
        (0, import_jsx_runtime.jsx)(Text2, { intensity: 54, className: "oui-text-base 2xl:oui-text-lg", children: `${t("affiliate.commission")} (USDC)` }),
        (0, import_jsx_runtime.jsxs)(
          Flex,
          {
            direction: "row",
            gap: 3,
            className: "oui-text-xl md:oui-text-2xl xl:oui-text-3xl",
            children: [
              (0, import_jsx_runtime.jsx)(USDCIcon, { className: "md:oui-w-[24px] md:oui-h-[24px] lg:oui-w-[28px] lg:oui-h-[28px] " }),
              (0, import_jsx_runtime.jsx)(Text2, { children: commifyOptional(props.commission, { fix: 2, fallback: "0" }) })
            ]
          }
        )
      ]
    }
  );
};
var Row = (props) => {
  return (0, import_jsx_runtime.jsxs)(Flex, { direction: "row", justify: "between", width: "100%", children: [
    (0, import_jsx_runtime.jsx)(
      Text2,
      {
        intensity: 54,
        className: "oui-text-2xs md:oui-text-xs xl:oui-text-sm",
        children: props.title
      }
    ),
    (0, import_jsx_runtime.jsx)(Text2, { className: "oui-text-xs md:oui-text-sm xl:oui-text-base", children: commifyOptional(props.value, { fix: props.dp, fallback: "0" }) })
  ] });
};
var useSummaryScript2 = () => {
  const { t } = useTranslation();
  const [period, setPeriod] = (0, import_react.useState)("All");
  const periodTypes = [
    { label: t("common.all"), value: "All" },
    { label: t("common.select.1d"), value: "1D" },
    { label: t("common.select.7d"), value: "7D" },
    { label: t("common.select.30d"), value: "30D" }
  ];
  const { referralInfo } = useReferralContext();
  const commission = (0, import_react.useMemo)(() => {
    if (!referralInfo || !referralInfo.referrer_info) {
      return 0;
    }
    switch (period) {
      case "All":
        return referralInfo.referrer_info.total_referrer_rebate;
      case "1D":
        return referralInfo.referrer_info["1d_referrer_rebate"];
      case "7D":
        return referralInfo.referrer_info["7d_referrer_rebate"];
      case "30D":
        return referralInfo.referrer_info["30d_referrer_rebate"];
    }
  }, [referralInfo, period]);
  const referralVol = (0, import_react.useMemo)(() => {
    if (!referralInfo || !referralInfo.referrer_info) {
      return 0;
    }
    switch (period) {
      case "All":
        return referralInfo.referrer_info.total_referee_volume;
      case "1D":
        return referralInfo.referrer_info["1d_referee_volume"];
      case "7D":
        return referralInfo.referrer_info["7d_referee_volume"];
      case "30D":
        return referralInfo.referrer_info["30d_referee_volume"];
    }
  }, [referralInfo, period]);
  const referees = (0, import_react.useMemo)(() => {
    if (!referralInfo || !referralInfo.referrer_info) {
      return 0;
    }
    switch (period) {
      case "All":
        return referralInfo.referrer_info.total_invites;
      case "1D":
        return referralInfo.referrer_info["1d_invites"];
      case "7D":
        return referralInfo.referrer_info["7d_invites"];
      case "30D":
        return referralInfo.referrer_info["30d_invites"];
    }
  }, [referralInfo, period]);
  const refereesTades = (0, import_react.useMemo)(() => {
    if (!referralInfo || !referralInfo.referrer_info) {
      return 0;
    }
    switch (period) {
      case "All":
        return referralInfo.referrer_info.total_traded;
      case "1D":
        return referralInfo.referrer_info["1d_traded"];
      case "7D":
        return referralInfo.referrer_info["7d_traded"];
      case "30D":
        return referralInfo.referrer_info["30d_traded"];
    }
  }, [referralInfo, period]);
  const onPeriodChange = (item) => {
    setPeriod(item);
  };
  return {
    period,
    periodTypes,
    onPeriodChange,
    commission,
    referralVol,
    referees,
    refereesTades
  };
};
var SummaryWidget2 = () => {
  const state = useSummaryScript2();
  return (0, import_jsx_runtime.jsx)(Summary2, { ...state });
};
var AutoHideText = (props) => {
  const { text, visibleCount } = props;
  const containerRef = (0, import_react.useRef)(null);
  const textRef = (0, import_react.useRef)(null);
  const lastContainerWidth = (0, import_react.useRef)(void 0);
  function getTextWidth(ref, text2) {
    const canvas = document.createElement("canvas");
    const context = canvas.getContext("2d");
    const element = ref;
    if (context && element) {
      let fontStyle = getComputedStyle(element).font;
      if (fontStyle.length === 0) {
        fontStyle = [
          getComputedStyle(element).fontWeight,
          getComputedStyle(element).fontSize,
          getComputedStyle(element).fontFamily
        ].join(" ");
      }
      context.font = fontStyle;
      context.letterSpacing = getComputedStyle(element).letterSpacing;
      const width = context.measureText(text2).width;
      return width;
    }
    return void 0;
  }
  const [visibleChars, setVisibleChars] = (0, import_react.useState)(0);
  (0, import_react.useEffect)(() => {
    const calculateVisibleChars = () => {
      if (props.visibleCount !== void 0) return;
      if (containerRef.current && textRef.current) {
        const containerWidth = containerRef.current.clientWidth;
        if (containerWidth == lastContainerWidth.current) return;
        lastContainerWidth.current = containerWidth;
        const textElement = textRef.current;
        const textContent = text;
        const pWidth = Math.ceil(getTextWidth(textElement, "...") || 0);
        const maxDisplayW = containerWidth - pWidth;
        let count = 0;
        while (count < text.length) {
          let contentW = getTextWidth(textElement, subText(text, count++)) || maxDisplayW;
          if (contentW <= maxDisplayW) {
            break;
          }
        }
        setVisibleChars(textContent.length - count);
      }
    };
    lastContainerWidth.current = void 0;
    calculateVisibleChars();
    window.addEventListener("resize", calculateVisibleChars);
    return () => {
      lastContainerWidth.current = void 0;
      window.removeEventListener("resize", calculateVisibleChars);
    };
  }, [text, textRef]);
  const truncatedText = (0, import_react.useMemo)(() => {
    if (visibleChars + 1 >= text.length) {
      return text;
    }
    const maxCharacters = visibleCount || visibleChars;
    const startText = text.slice(0, Math.ceil(maxCharacters / 2));
    const endText = text.slice(
      text.length - Math.floor(maxCharacters / 2),
      text.length
    );
    return `${startText}...${endText}`;
  }, [text, visibleCount, visibleChars]);
  return (0, import_jsx_runtime.jsxs)("div", { ref: containerRef, className: M("oui-relative oui-w-full oui-text-end", props.className), children: [
    (0, import_jsx_runtime.jsx)(
      "div",
      {
        className: M(
          "oui-hidden oui-whitespace-nowrap oui-absolute oui-top-0 oui-bottom-0 oui-right-0 oui-left-0"
        ),
        children: (0, import_jsx_runtime.jsx)("span", { ref: textRef, children: text })
      }
    ),
    (0, import_jsx_runtime.jsx)("span", { children: truncatedText })
  ] });
};
var subText = (text, count) => {
  const start = Math.floor(text.length / 2 - (count > 0 ? count / 2 : 1));
  const first = text.slice(0, start);
  const end = text.slice(start + count);
  return first + end;
};
var ReferralLink = (props) => {
  const { t } = useTranslation();
  return (0, import_jsx_runtime.jsxs)(
    Flex,
    {
      id: "oui-affiliate-affiliate-referralLink",
      r: "2xl",
      p: 6,
      width: "100%",
      gap: 4,
      direction: "column",
      className: "oui-bg-base-9",
      children: [
        (0, import_jsx_runtime.jsx)(Title2, { ...props }),
        (0, import_jsx_runtime.jsxs)(
          Flex,
          {
            width: "100%",
            className: "oui-flex oui-flex-col 3xl:oui-flex-row",
            gap: 4,
            children: [
              (0, import_jsx_runtime.jsx)("div", { className: "oui-w-full 3xl:oui-w-1/3", children: (0, import_jsx_runtime.jsx)(Subtitle, { ...props }) }),
              (0, import_jsx_runtime.jsxs)(
                Flex,
                {
                  direction: "column",
                  width: "100%",
                  className: "3xl:oui-w-2/3 3xl:oui-gap-2",
                  children: [
                    (0, import_jsx_runtime.jsx)(
                      Input,
                      {
                        title: t("affiliate.referralCode"),
                        value: props.refCode,
                        ...props
                      }
                    ),
                    (0, import_jsx_runtime.jsx)(
                      Input,
                      {
                        title: t("affiliate.referralLink"),
                        value: props.refLink,
                        ...props
                      }
                    )
                  ]
                }
              )
            ]
          }
        )
      ]
    }
  );
};
var Title2 = (props) => {
  const { t } = useTranslation();
  return (0, import_jsx_runtime.jsx)(Flex, { direction: "row", justify: "between", width: "100%", children: (0, import_jsx_runtime.jsx)(Text2, { className: "oui-text-lg", children: t("affiliate.referralLink") }) });
};
var Subtitle = (props) => {
  const { t } = useTranslation();
  const item = (title, value, gradient, className, tooltip) => {
    const valueClsName = "oui-text-lg md:oui-text-xl lg:oui-text-2xl xl:oui-text-3xl";
    return (0, import_jsx_runtime.jsxs)(Flex, { direction: "column", itemAlign: "start", className, children: [
      (0, import_jsx_runtime.jsx)(Flex, { direction: "row", gap: 2, children: (0, import_jsx_runtime.jsx)(Alert, { title, tooltip }) }),
      gradient ? (0, import_jsx_runtime.jsx)(Text2.gradient, { color: "brand", className: valueClsName, children: value }) : (0, import_jsx_runtime.jsx)(Text2, { className: valueClsName, children: value })
    ] });
  };
  return (0, import_jsx_runtime.jsxs)(Flex, { direction: "row", width: "100%", children: [
    item(
      t("affiliate.referralLink.earn"),
      props.earn || "-",
      true,
      "oui-flex-1",
      (0, import_jsx_runtime.jsxs)("span", { children: [
        (0, import_jsx_runtime.jsx)(Text2.gradient, { color: "brand", children: props.earn || "-" }),
        " ",
        t("affiliate.referralLink.earn.tooltip", {
          brokerName: props.brokerName
        })
      ] })
      // @ts-ignore
      // <Trans
      //   i18nKey="affiliate.referralLink.earn.tooltip"
      //   values={{
      //     value: props.earn || "-",
      //     brokerName: props.brokerName,
      //   }}
      //   components={[<Text.gradient color="brand" />]}
      // />
    ),
    item(
      t("affiliate.referralLink.share"),
      props.share || "-",
      false,
      "oui-flex-1",
      // @ts-ignore
      (0, import_jsx_runtime.jsx)(
        Trans,
        {
          i18nKey: "affiliate.referralLink.share.tooltip",
          values: {
            value: props.share || "-",
            brokerName: props.brokerName
          },
          components: [(0, import_jsx_runtime.jsx)(Text2.gradient, { color: "brand" }, "0")]
        }
      )
    )
  ] });
};
var Input = (props) => {
  const { title, value = "-" } = props;
  return (0, import_jsx_runtime.jsxs)(
    Flex,
    {
      r: "xl",
      p: 3,
      gap: 4,
      direction: "row",
      width: "100%",
      className: "oui-bg-base-8",
      children: [
        (0, import_jsx_runtime.jsx)(Text2, { className: "oui-text-base-contrast-54 oui-flex-shrink oui-text-2xs md:oui-text-xs xl:oui-text-sm", children: title }),
        (0, import_jsx_runtime.jsxs)(
          Flex,
          {
            direction: "row",
            justify: "end",
            gap: 2,
            className: "oui-flex-1 oui-text-xs md:oui-text-sm xl:oui-text-base",
            children: [
              (0, import_jsx_runtime.jsx)(AutoHideText, { text: value }),
              (0, import_jsx_runtime.jsx)(
                "button",
                {
                  className: "oui-cursor-pointer oui-text-sm oui-flex-shrink",
                  onClick: () => {
                    navigator.clipboard.writeText(value);
                    props.onCopy?.(value);
                  },
                  children: (0, import_jsx_runtime.jsx)(
                    CopyIcon,
                    {
                      size: 12,
                      color: "white",
                      className: "oui-w-[13px] oui-h-[13px] md:oui-w-[14px] md:oui-h-[14px] xl:oui-w-4 xl:oui-h-4"
                    }
                  )
                }
              )
            ]
          }
        )
      ]
    }
  );
};
var Alert = (props) => {
  return (0, import_jsx_runtime.jsx)(Tooltip, { content: props.tooltip, className: "oui-max-w-[200px]", children: (0, import_jsx_runtime.jsx)(
    "div",
    {
      onClick: () => {
        modal.alert({
          title: props.title,
          message: (0, import_jsx_runtime.jsx)("div", { className: "oui-text-base-contrast/30 oui-space-y-3 oui-text-xs desktop:oui-text-xs", children: props.tooltip })
        });
      },
      children: (0, import_jsx_runtime.jsxs)(Flex, { direction: "column", children: [
        (0, import_jsx_runtime.jsx)(
          Text2,
          {
            intensity: 54,
            className: "oui-text-2xs md:oui-text-xs xl:oui-text-sm",
            children: props.title
          }
        ),
        (0, import_jsx_runtime.jsx)(Divider, { className: "oui-w-full", lineStyle: "dotted", intensity: 16 })
      ] })
    }
  ) });
};
var useReferralLinkScript = () => {
  const { t } = useTranslation();
  const onCopy = (value) => {
    Vt.success(t("common.copy.copied"));
  };
  const { referralInfo, referralLinkUrl, overwrite } = useReferralContext();
  const [pinCodes, setPinCodes] = useLocalStorage(
    "orderly_referral_codes",
    []
  );
  const codes = (0, import_react.useMemo)(() => {
    if (!referralInfo?.referrer_info?.referral_codes)
      return [];
    const referralCodes = [...referralInfo?.referrer_info?.referral_codes];
    const pinedItems = [];
    for (let i = 0; i < pinCodes.length; i++) {
      const code2 = pinCodes[i];
      const index = referralCodes.findIndex((item) => item.code === code2);
      if (index !== -1) {
        pinedItems.push({ ...referralCodes[index] });
        referralCodes.splice(index, 1);
      }
    }
    return [...pinedItems, ...referralCodes];
  }, [referralInfo?.referrer_info?.referral_codes, pinCodes]);
  const firstCode = (0, import_react.useMemo)(() => {
    if (codes.length === 0) {
      return void 0;
    }
    return codes[0];
  }, [codes]);
  const code = (0, import_react.useMemo)(() => {
    return firstCode?.code;
  }, [firstCode]);
  const referralLink = (0, import_react.useMemo)(() => {
    if (!firstCode) return "";
    return generateReferralLink(referralLinkUrl, firstCode.code);
  }, [firstCode]);
  const earn = (0, import_react.useMemo)(() => {
    const value = new decimal_default(firstCode?.referrer_rebate_rate || "0").mul(100).toDecimalPlaces(0, decimal_default.ROUND_DOWN).toString();
    return `${value}%`;
  }, [firstCode?.referrer_rebate_rate]);
  const share = (0, import_react.useMemo)(() => {
    const value = new decimal_default(firstCode?.referee_rebate_rate || "0").mul(100).toDecimalPlaces(0, decimal_default.ROUND_DOWN).toString();
    return `${value}%`;
  }, [firstCode?.referee_rebate_rate]);
  return {
    onCopy,
    refLink: referralLink,
    refCode: code,
    share,
    earn,
    brokerName: overwrite?.brokerName ?? overwrite?.shortBrokerName
  };
};
var ReferralLinkWidget = () => {
  const state = useReferralLinkScript();
  return (0, import_jsx_runtime.jsx)(ReferralLink, { ...state });
};
var TitleStatistic = (props) => {
  return (0, import_jsx_runtime.jsxs)(
    Flex,
    {
      id: "oui-affiliate-affiliate-titleStatistic",
      r: "2xl",
      p: 6,
      width: "100%",
      gap: 4,
      direction: "column",
      className: "oui-bg-base-9",
      children: [
        (0, import_jsx_runtime.jsx)(Title3, { ...props }),
        (0, import_jsx_runtime.jsx)(Flex, { className: "oui-h-[170px] 2xl:oui-h-[196px] oui-w-full oui-flex oui-flex-row oui-items-stretch", children: (0, import_jsx_runtime.jsx)(
          VolBarChart,
          {
            data: props.dataSource || EMPTY_LIST,
            colors: { fill: "rgba(0, 180, 158, 1)" },
            className: "oui-w-full oui-flex-1",
            tooltip: {
              rm: decimal_default.ROUND_DOWN,
              dp: props.volType === "Commission" ? 6 : 2
            }
          }
        ) })
      ]
    }
  );
};
var Title3 = (props) => {
  const { t } = useTranslation();
  return (0, import_jsx_runtime.jsxs)(Flex, { direction: "row", justify: "between", width: "100%", children: [
    (0, import_jsx_runtime.jsx)(Text2, { className: "oui-text-lg", children: t("affiliate.statistics") }),
    (0, import_jsx_runtime.jsxs)(Flex, { direction: "row", gap: 2, className: "oui-min-w-14", children: [
      (0, import_jsx_runtime.jsx)(
        Select2.options,
        {
          size: "xs",
          value: props.period,
          onValueChange: props.onPeriodChange,
          options: props.periodTypes
        }
      ),
      (0, import_jsx_runtime.jsx)(
        Select2.options,
        {
          size: "xs",
          value: props.volType,
          onValueChange: props.onVolTypeChange,
          options: props.volTypes
        }
      )
    ] })
  ] });
};
var useTitleStatisticScript = () => {
  const { t } = useTranslation();
  const [period, setPeriod] = (0, import_react.useState)("7");
  const periodTypes = [
    { label: t("common.select.7d"), value: "7" },
    { label: t("common.select.30d"), value: "30" },
    { label: t("common.select.90d"), value: "90" }
  ];
  const onPeriodChange = (item) => {
    setPeriod(item);
  };
  const [volType, setVolType] = (0, import_react.useState)("Commission");
  const volTypes = [
    { label: t("affiliate.commission"), value: "Commission" },
    { label: t("affiliate.referralVol"), value: "ref" }
  ];
  const onVolTypeChange = (item) => {
    setVolType(item);
  };
  const dateRange = (0, import_react.useMemo)(() => {
    if (period === "7") {
      return {
        startDate: subDays(/* @__PURE__ */ new Date(), 7),
        endDate: subDays(/* @__PURE__ */ new Date(), 1)
      };
    } else if (period === "30") {
      return {
        startDate: subDays(/* @__PURE__ */ new Date(), 30),
        endDate: subDays(/* @__PURE__ */ new Date(), 1)
      };
    } else if (period === "90") {
      return {
        startDate: subDays(/* @__PURE__ */ new Date(), 90),
        endDate: subDays(/* @__PURE__ */ new Date(), 1)
      };
    } else {
      return {
        startDate: subDays(/* @__PURE__ */ new Date(), 7),
        endDate: subDays(/* @__PURE__ */ new Date(), 1)
      };
    }
  }, [period]);
  const [rebateSummary] = useReferralRebateSummary({
    startDate: format(dateRange.startDate, "yyyy-MM-dd"),
    endDate: format(dateRange.endDate, "yyyy-MM-dd"),
    size: Number(period)
  });
  const dataSource = (0, import_react.useMemo)(() => {
    return (rebateSummary?.map(
      (e) => ({
        date: e.date,
        volume: volType === "Commission" ? e.referral_rebate : e.volume
      })
    ) || []).reverse();
  }, [rebateSummary, volType]);
  return {
    period,
    periodTypes,
    onPeriodChange,
    volType,
    volTypes,
    onVolTypeChange,
    dataSource: fillData(Number(period), dataSource)
  };
};
var TitleStatisticWidget = () => {
  const state = useTitleStatisticScript();
  return (0, import_jsx_runtime.jsx)(TitleStatistic, { ...state });
};
var EditIcon = (props) => {
  const { className, ...rest } = props;
  return (0, import_jsx_runtime.jsx)(
    "svg",
    {
      width: "12",
      height: "13",
      viewBox: "0 0 12 13",
      xmlns: "http://www.w3.org/2000/svg",
      fill: "white",
      fillOpacity: "0.36",
      className,
      ...rest,
      children: (0, import_jsx_runtime.jsx)("path", { d: "M8.49773 1.47656C8.36523 1.47656 8.23223 1.52357 8.13823 1.61707C7.86023 1.89507 6.85973 2.89558 6.63773 3.11808L2.13523 7.62059L1.63473 8.12059C1.56523 8.19059 1.52923 8.28958 1.50973 8.38658L1.00973 10.8881C0.939727 11.2381 1.23773 11.5361 1.58773 11.4666C1.90073 11.4036 3.77673 11.0286 4.08923 10.9661C4.18623 10.9466 4.28523 10.9106 4.35523 10.8411L4.85523 10.3406L9.35773 5.83808C9.58023 5.61608 10.5807 4.61506 10.8587 4.33756C10.9522 4.24356 10.9992 4.11056 10.9992 3.97806C10.9992 3.15956 10.7907 2.57456 10.3582 2.13306C9.92173 1.68756 9.33873 1.47656 8.49773 1.47656ZM8.69473 2.48606C9.14623 2.51256 9.43873 2.61608 9.63923 2.82108C9.84423 3.03008 9.97373 3.32008 10.0017 3.76258C9.72773 4.03608 9.32673 4.43106 8.99823 4.75956C8.60173 4.36306 8.11273 3.87407 7.71623 3.47757C8.04523 3.14907 8.42123 2.75956 8.69473 2.48606ZM6.99723 4.19657L8.27923 5.47858L4.49573 9.26207L3.21373 7.98009L6.99723 4.19657ZM2.49473 8.69908L3.77673 9.98107L3.72973 10.0281C3.39973 10.0941 2.73323 10.2336 2.11923 10.3566L2.44773 8.74608L2.49473 8.69908Z" })
    }
  );
};
var EditCode = (props) => {
  return (0, import_jsx_runtime.jsx)("button", { children: (0, import_jsx_runtime.jsx)(
    EditIcon,
    {
      className: " oui-mt-px oui-cursor-pointer oui-fill-base-contrast-36 hover:oui-fill-base-contrast-80",
      fillOpacity: 1,
      fill: "currentColor",
      onClick: props.onClick
    }
  ) });
};
var PinBtn = (props) => {
  return (0, import_jsx_runtime.jsx)(
    "button",
    {
      onClick: (e) => {
        props?.onClick?.(!props.pinned);
      },
      children: props.pinned === false ? (0, import_jsx_runtime.jsx)(
        UnPinIcon,
        {
          size: props.size,
          fillOpacity: 1,
          className: "orderly-fill-primary-darken hover:orderly-fill-primary-darken/80"
        }
      ) : (0, import_jsx_runtime.jsx)(
        PinnedIcon,
        {
          size: props.size,
          fillOpacity: 1,
          className: "orderly-fill-base-contrast-36 hover:orderly-fill-base-contrast"
        }
      )
    }
  );
};
var PinnedIcon = (props) => {
  const { size = 12 } = props;
  return (0, import_jsx_runtime.jsx)(
    "svg",
    {
      width: size,
      height: size,
      viewBox: "0 0 16 16",
      fill: "#608CFF",
      xmlns: "http://www.w3.org/2000/svg",
      children: (0, import_jsx_runtime.jsx)("path", { d: "M10.008 1.302a.74.74 0 0 0-.486.215c-1.033.988-1.349 1.815-.972 2.947-.88.675-1.437.841-2.536.841-1.503 0-2.484.181-3.152.848v.021a1.583 1.583 0 0 0 0 2.249l1.867 1.881-3.181 3.18c-.26.26-.28.696-.02.956.261.26.699.26.959 0l3.193-3.193 1.87 1.86a1.585 1.585 0 0 0 2.25 0h.02c.668-.667.854-1.522.854-3.144 0-1.03.212-1.758.853-2.523 1.232.361 1.95.015 2.96-.995a.68.68 0 0 0 .188-.479c0-.234-.06-.594-.209-1.041a5.34 5.34 0 0 0-1.312-2.103A5.35 5.35 0 0 0 11.05 1.51c-.448-.149-.808-.208-1.042-.208" })
    }
  );
};
var UnPinIcon = (props) => {
  const { size = 12 } = props;
  return (0, import_jsx_runtime.jsx)(
    "svg",
    {
      width: size,
      height: size,
      viewBox: "0 0 16 16",
      fill: "#fff",
      fillOpacity: ".36",
      xmlns: "http://www.w3.org/2000/svg",
      children: (0, import_jsx_runtime.jsx)("path", { d: "M10.008 1.302a.74.74 0 0 0-.486.215c-1.033.988-1.349 1.815-.972 2.947-.88.675-1.437.841-2.536.841-1.503 0-2.484.181-3.152.848v.021a1.583 1.583 0 0 0 0 2.249l1.867 1.881-3.181 3.18c-.26.26-.28.696-.02.956.261.26.699.26.959 0l3.193-3.193 1.87 1.86a1.585 1.585 0 0 0 2.25 0h.02c.668-.667.854-1.522.854-3.144 0-1.03.212-1.758.853-2.523 1.232.361 1.95.015 2.96-.995a.68.68 0 0 0 .188-.479c0-.234-.06-.594-.209-1.041a5.34 5.34 0 0 0-1.312-2.103A5.35 5.35 0 0 0 11.05 1.51c-.448-.149-.808-.208-1.042-.208m.258 1.37c.708.131 1.421.6 1.93 1.108.507.507.94 1.13 1.119 1.944-.636.61-1.026.659-1.662.324a.67.67 0 0 0-.779.116c-1.214 1.213-1.533 2.314-1.533 3.8 0 1.293-.076 1.774-.48 2.207-.113.123-.27.104-.374 0L3.799 7.486a.24.24 0 0 1-.017-.34c.239-.29.769-.514 2.226-.514 1.742.001 2.668-.447 3.812-1.52a.67.67 0 0 0 .125-.77c-.343-.685-.29-1.046.321-1.67" })
    }
  );
};
var ReferralCodes = (props) => {
  const isTablet = useMediaQuery("(max-width: 767px)");
  return (0, import_jsx_runtime.jsxs)(
    Flex,
    {
      r: "2xl",
      p: 6,
      width: "100%",
      gap: 4,
      direction: "column",
      className: "oui-h-full oui-bg-base-9 oui-p-6 oui-tabular-nums",
      children: [
        (0, import_jsx_runtime.jsx)(Title4, { ...props }),
        (0, import_jsx_runtime.jsxs)("div", { className: "oui-flex oui-w-full oui-flex-col 2xl:oui-h-full", children: [
          (0, import_jsx_runtime.jsx)(Divider, {}),
          isTablet ? (0, import_jsx_runtime.jsx)(MobileLayout, { ...props }) : (0, import_jsx_runtime.jsx)(DesktopLayout, { ...props })
        ] })
      ]
    }
  );
};
var Title4 = (props) => {
  const { t } = useTranslation();
  return (0, import_jsx_runtime.jsxs)(Flex, { direction: "row", justify: "between", width: "100%", children: [
    (0, import_jsx_runtime.jsx)(Text2, { className: "oui-text-lg", children: t("affiliate.referralCodes") }),
    (0, import_jsx_runtime.jsxs)("div", { className: "oui-text-2xs md:oui-text-xs xl:oui-text-sm", children: [
      (0, import_jsx_runtime.jsx)(Text2, { className: "oui-text-base-contrast-54", children: `${t("affiliate.referralCodes.remaining")}: ` }),
      (0, import_jsx_runtime.jsx)(Text2, { className: "oui-text-primary-light", children: props.codes?.length || "--" })
    ] })
  ] });
};
var MobileLayout = (props) => {
  return (0, import_jsx_runtime.jsx)(
    ListView,
    {
      dataSource: props.codes,
      className: "oui-max-h-[240px] oui-w-full",
      renderItem: (e, index) => {
        return (0, import_jsx_runtime.jsxs)(Flex, { direction: "column", children: [
          (0, import_jsx_runtime.jsx)(
            MobileCell2,
            {
              data: e,
              editRate: props.editRate,
              copyLink: props.copyLink,
              copyCode: props.copyCode,
              setPinCode: props.setPinCode,
              editCode: props.editCode
            },
            index
          ),
          (0, import_jsx_runtime.jsx)(Divider, { className: "oui-mt-3 oui-w-full" })
        ] });
      }
    }
  );
};
var MobileCellItem = (props) => {
  const { title, copyable, value, align, className, editRate, onCopy } = props;
  return (0, import_jsx_runtime.jsx)(
    Statistic,
    {
      id: "oui-affiliate-affiliate-referralCodes",
      className: M("oui-flex-1", className),
      label: (0, import_jsx_runtime.jsx)(Text2, { className: "oui-text-2xs oui-text-base-contrast-36", children: title }),
      align,
      children: (0, import_jsx_runtime.jsxs)(Flex, { direction: "row", gap: 1, children: [
        (0, import_jsx_runtime.jsx)(
          Text2.formatted,
          {
            copyable,
            onCopy: () => {
              onCopy?.();
            },
            className: "oui-mt-[6px] oui-text-sm oui-text-base-contrast-80",
            children: value
          }
        ),
        editRate && (0, import_jsx_runtime.jsx)(
          EditIcon,
          {
            className: "oui-mt-px oui-cursor-pointer oui-fill-white/[.36] hover:oui-fill-white/80",
            fillOpacity: 1,
            fill: "currentColor",
            onClick: () => editRate()
          }
        )
      ] })
    }
  );
};
var MobileCell2 = (props) => {
  const { data, setPinCode, copyLink, editRate, editCode } = props;
  const { t } = useTranslation();
  return (0, import_jsx_runtime.jsxs)(Flex, { gap: 3, direction: "column", className: "oui-w-full", children: [
    (0, import_jsx_runtime.jsxs)(
      Flex,
      {
        direction: "row",
        justify: "between",
        itemAlign: "stretch",
        width: "100%",
        children: [
          (0, import_jsx_runtime.jsx)(
            MobileCellItem,
            {
              title: t("affiliate.referralCode"),
              value: data.code,
              copyable: true,
              onCopy: () => {
                props.copyCode?.(data.code);
              },
              editRate: () => {
                editCode(data);
              }
            }
          ),
          (0, import_jsx_runtime.jsx)(
            MobileCellItem,
            {
              title: t("affiliate.referralCodes.column.you&Referee"),
              value: getRate(data),
              align: "end",
              editRate: () => {
                editRate(data);
              }
            }
          ),
          (0, import_jsx_runtime.jsx)(
            MobileCellItem,
            {
              title: t("affiliate.referralCodes.column.referees&Traders"),
              value: getCount(data),
              align: "end",
              className: "oui-hidden md:oui-flex"
            }
          )
        ]
      }
    ),
    (0, import_jsx_runtime.jsxs)(
      Flex,
      {
        direction: "row",
        justify: "between",
        itemAlign: "stretch",
        width: "100%",
        className: "md:oui-hidden",
        children: [
          (0, import_jsx_runtime.jsx)(
            MobileCellItem,
            {
              title: t("affiliate.referees"),
              value: getCount(data).split("/")?.[0],
              align: "start"
            }
          ),
          (0, import_jsx_runtime.jsx)(
            MobileCellItem,
            {
              title: t("affiliate.referralCodes.column.traders"),
              value: getCount(data).split("/")?.[1],
              align: "end"
            }
          )
        ]
      }
    ),
    (0, import_jsx_runtime.jsxs)(
      Flex,
      {
        direction: "row",
        justify: "between",
        itemAlign: "stretch",
        width: "100%",
        children: [
          (0, import_jsx_runtime.jsx)(
            PinBtn,
            {
              pinned: data.isPined || false,
              onClick: (e) => {
                setPinCode(data.code, !e);
              }
            }
          ),
          (0, import_jsx_runtime.jsx)(
            Button,
            {
              variant: "outlined",
              size: "xs",
              className: "oui-px-[20px]",
              onClick: (e) => {
                copyLink(data.code);
              },
              children: t("affiliate.referralCodes.copyLink")
            }
          )
        ]
      }
    )
  ] }, data.code);
};
var DesktopLayout = (props) => {
  const { t } = useTranslation();
  const moreColumn = useMediaQuery("(min-width: 1024px)");
  const columns = (0, import_react.useMemo)(() => {
    const cols = [
      {
        title: t("affiliate.referralCode"),
        dataIndex: "code",
        width: moreColumn ? 115 : 120,
        className: "!oui-px-0",
        render: (value, data) => {
          return (0, import_jsx_runtime.jsxs)(Flex, { direction: "row", itemAlign: "center", gap: 1, children: [
            (0, import_jsx_runtime.jsx)(
              PinBtn,
              {
                size: 12,
                pinned: data.isPined || false,
                onClick: (e) => {
                  props.setPinCode(data.code, !e);
                }
              }
            ),
            (0, import_jsx_runtime.jsx)(
              Text2.formatted,
              {
                suffix: (0, import_jsx_runtime.jsxs)(import_jsx_runtime.Fragment, { children: [
                  data.isAutoGenerated && data.total_invites < 1 && (0, import_jsx_runtime.jsx)(EditCode, { onClick: () => props.editCode?.(data) }),
                  (0, import_jsx_runtime.jsx)(
                    CopyIcon,
                    {
                      className: "oui-cursor-pointer",
                      size: 12,
                      color: "white",
                      onClick: () => props.copyCode?.(data.code)
                    }
                  )
                ] }),
                copyable: true,
                onCopy: () => {
                  props.copyCode?.(data.code);
                },
                children: value
              }
            )
          ] });
        }
      },
      {
        title: t("affiliate.referralCodes.column.you&Referee"),
        dataIndex: "dffd",
        width: moreColumn ? 120 : 120,
        className: "oui-pr-0",
        render: (value, data) => {
          return (0, import_jsx_runtime.jsxs)(Flex, { direction: "row", itemAlign: "center", gap: 1, children: [
            getRate(data),
            (0, import_jsx_runtime.jsx)(
              EditIcon,
              {
                className: "oui-mt-[6px] oui-cursor-pointer oui-fill-white/[.36] hover:oui-fill-white/80",
                fillOpacity: 1,
                fill: "currentColor",
                onClick: (e) => props.editRate?.(data)
              }
            )
          ] });
        }
      }
    ];
    if (moreColumn) {
      cols.push({
        title: t("affiliate.referees"),
        dataIndex: "referee_rebate_rate",
        width: 65,
        className: "oui-pr-0",
        render: (value, data) => getCount(data).split("/")[0]
      });
      cols.push({
        title: t("affiliate.referralCodes.column.traders"),
        dataIndex: "referrer_rebate_rate",
        width: 65,
        className: "oui-pr-0",
        render: (value, data) => getCount(data).split("/")[1]
      });
    } else {
      cols.push({
        title: t("affiliate.referralCodes.column.referees&Traders"),
        dataIndex: "total_invites/total_traded",
        width: 120,
        fixed: "left",
        render: (value, data) => getCount(data)
      });
    }
    cols.push({
      dataIndex: "link",
      align: "right",
      width: 74,
      className: "!oui-px-0",
      render: (value, data) => (0, import_jsx_runtime.jsx)(
        Button,
        {
          variant: "outlined",
          size: "sm",
          className: "oui-px-5",
          onClick: (e) => {
            props?.copyLink?.(data.code);
          },
          children: t("affiliate.referralCodes.copyLink")
        }
      )
    });
    return cols;
  }, [moreColumn, t]);
  return (0, import_jsx_runtime.jsx)(
    DataTable,
    {
      bordered: true,
      columns,
      dataSource: props.codes,
      classNames: {
        header: "oui-px-0",
        root: "2xl:oui-flex-1 2xl:oui-max-h-[230px] 3xl:oui-max-h-[300px]"
      },
      onRow: (record) => {
        return {
          className: "oui-h-[45px]"
        };
      }
    }
  );
};
var getRate = (item) => {
  const refereeRate = new decimal_default(item.referee_rebate_rate).mul(100).toFixed(1, decimal_default.ROUND_DOWN).toString();
  const referralRate = new decimal_default(item.referrer_rebate_rate).mul(100).toFixed(1, decimal_default.ROUND_DOWN).toString();
  return `${referralRate}% / ${refereeRate}%`;
};
var getCount = (item) => {
  return `${item.total_invites} / ${item.total_traded}`;
};
var EditCodeModal = modal.create((props) => {
  const { t } = useTranslation();
  const { visible, onOpenChange } = useModal();
  const [newCode, setNewCode] = (0, import_react.useState)("");
  const [filedError, setFiledError] = (0, import_react.useState)({
    length: false,
    format: false
  });
  const [editCode, { error, isMutating }] = useMutation(
    "/v1/referral/edit_referral_code",
    "POST"
  );
  (0, import_react.useEffect)(() => {
    setNewCode(props.code.code);
  }, [props.code]);
  (0, import_react.useEffect)(() => {
    const _code = newCode.toUpperCase();
    const _fieldError = {
      length: false,
      format: false
    };
    if (_code.length < 4 || _code.length > 10) {
      _fieldError.length = true;
    }
    if (!/^[A-Z0-9]+$/.test(_code)) {
      _fieldError.format = true;
    }
    setFiledError(_fieldError);
  }, [newCode]);
  return (0, import_jsx_runtime.jsxs)(
    SimpleDialog,
    {
      classNames: {
        content: "oui-max-w-[360px]"
      },
      title: (0, import_jsx_runtime.jsx)("div", { children: t("affiliate.referralCode.editCodeModal.title") }),
      open: visible,
      onOpenChange,
      actions: {
        primary: {
          loading: isMutating,
          disabled: filedError.length || filedError.format,
          label: t("common.confirm"),
          onClick: async () => {
            try {
              const res = await editCode({
                current_referral_code: props.code.code,
                new_referral_code: newCode.toUpperCase()
              });
              if (res.success) {
                Vt.success(
                  t("affiliate.referralCode.editCodeModal.success")
                );
                props.successCallback();
                onOpenChange(false);
                return;
              }
              Vt.error(res.message);
            } catch (e) {
            }
          }
        }
      },
      children: [
        (0, import_jsx_runtime.jsx)("div", { className: "oui-mb-6 oui-text-xs oui-text-base-contrast-54", children: t("affiliate.referralCode.editCodeModal.description") }),
        (0, import_jsx_runtime.jsx)(
          TextField,
          {
            placeholder: "",
            fullWidth: true,
            label: t("affiliate.referralCode.editCodeModal.label"),
            onClean: () => {
              setNewCode("");
            },
            value: newCode,
            onChange: (e) => {
              const _value = e.target.value.toUpperCase().replace(/[^A-Z0-9]/g, "");
              setNewCode(_value);
            },
            formatters: [
              formatter_exports.createRegexInputFormatter((value) => {
                return String(value).replace(
                  /[a-z]/g,
                  (char) => char.toUpperCase()
                );
              }),
              formatter_exports.createRegexInputFormatter(/[^A-Z0-9]/g)
            ],
            classNames: {
              label: "oui-text-base-contrast-54 oui-text-xs",
              input: "placeholder:oui-text-base-contrast-20 placeholder:oui-text-sm"
            },
            maxLength: 10,
            minLength: 4,
            autoComplete: "off",
            helpText: ""
          }
        ),
        (0, import_jsx_runtime.jsxs)(
          Flex,
          {
            direction: "column",
            justify: "center",
            itemAlign: "start",
            gap: 1,
            className: "oui-mt-3",
            children: [
              (0, import_jsx_runtime.jsx)(
                Text2,
                {
                  className: M(
                    "oui-ml-4 oui-list-item oui-list-outside oui-list-disc oui-text-xs oui-text-base-contrast-36 marker:oui-text-3xs",
                    filedError.length ? "oui-text-danger" : "oui-text-success"
                  ),
                  children: t("affiliate.referralCode.editCodeModal.helpText.length")
                }
              ),
              (0, import_jsx_runtime.jsx)(
                Text2,
                {
                  className: M(
                    "oui-ml-4 oui-list-item oui-list-outside oui-list-disc oui-text-xs oui-text-base-contrast-36 marker:oui-text-3xs",
                    "oui-text-success"
                  ),
                  children: t("affiliate.referralCode.editCodeModal.helpText.format")
                }
              )
            ]
          }
        )
      ]
    }
  );
});
var EditReferralRate = modal.create((props) => {
  const { code, mutate } = props;
  const { visible, hide, resolve, reject, onOpenChange } = useModal();
  const maxRate = new decimal_default(code.max_rebate_rate).mul(100);
  const [refereeRebateRate, setRefereeRebateRate] = (0, import_react.useState)(
    `${new decimal_default(code.referee_rebate_rate).mul(100)}`
  );
  const [referrerRebateRate, setReferrerRebateRate] = (0, import_react.useState)(
    `${new decimal_default(code.referrer_rebate_rate).mul(100)}`
  );
  const [showError, setShowError] = (0, import_react.useState)(false);
  const inputRef = (0, import_react.useRef)(null);
  const { t } = useTranslation();
  (0, import_react.useEffect)(() => {
    if (inputRef?.current) {
      inputRef.current.focus();
      inputRef.current.setSelectionRange(
        inputRef.current.value.length,
        inputRef.current.value.length
      );
    }
  }, [inputRef]);
  (0, import_react.useEffect)(() => {
    setRefereeRebateRate(`${new decimal_default(code.referee_rebate_rate).mul(100)}`);
    setReferrerRebateRate(`${new decimal_default(code.referrer_rebate_rate).mul(100)}`);
  }, [code]);
  const [editRate, { error, isMutating }] = useMutation(
    "/v1/referral/edit_split",
    "POST"
  );
  const onClickConfirm = async () => {
    try {
      const r1 = Number.parseFloat(refereeRebateRate);
      const r2 = Number.parseFloat(referrerRebateRate);
      await editRate({
        referral_code: code.code,
        referee_rebate_rate: r1 / 100,
        referrer_rebate_rate: r2 / 100
      });
      Vt.success(t("affiliate.referralRate.editRateModal.success"));
      mutate();
      hide();
    } catch (e) {
      Vt.error(e?.message || e || "");
    }
  };
  return (0, import_jsx_runtime.jsx)(Dialog, { open: visible, onOpenChange, children: (0, import_jsx_runtime.jsxs)(
    DialogContent,
    {
      className: "oui-px-6 oui-max-w-[320px] oui-bg-base-8 oui-shadow-[0px_12px_20px_0px_rgba(0,0,0,0.25)]",
      closable: true,
      children: [
        (0, import_jsx_runtime.jsxs)(DialogTitle, { children: [
          (0, import_jsx_runtime.jsx)("div", { className: "oui-my-3", children: t("affiliate.referralRate.editRateModal.title") }),
          (0, import_jsx_runtime.jsx)(Divider, {})
        ] }),
        (0, import_jsx_runtime.jsxs)("div", { className: "oui-mt-3 oui-h-full oui-flex oui-flex-col oui-justify-end", children: [
          (0, import_jsx_runtime.jsx)("div", { className: "oui-text-xs oui-text-base-contrast-54", children: t("affiliate.referralRate.editRateModal.description") }),
          (0, import_jsx_runtime.jsxs)("div", { className: "oui-text-xs oui-text-base-contrast-80 oui-mt-2 oui-flex", children: [
            t("affiliate.referralRate.editRateModal.label"),
            (0, import_jsx_runtime.jsx)("div", { className: "oui-text-warning-darken oui-pl-1", children: `${new decimal_default(
              code.max_rebate_rate
            ).mul(100).toFixed(0, decimal_default.ROUND_DOWN)}%` })
          ] }),
          (0, import_jsx_runtime.jsx)("div", { className: "oui-text-2xs oui-mt-6 oui-text-base-contrast-80", children: t("affiliate.referralRate.editRateModal.label.you") }),
          (0, import_jsx_runtime.jsx)(
            Input2,
            {
              ref: inputRef,
              containerClassName: "oui-h-[40px] oui-mt-3 oui-bg-base-700 oui-outline oui-outline-1 oui-outline-base-contrast-12 focus-within:oui-outline-primary-darken",
              placeholder: "Enter code",
              type: "text",
              inputMode: "decimal",
              autoComplete: "off",
              value: referrerRebateRate,
              onChange: (e) => {
                const text = cleanStringStyle(e.target.value);
                const rate = Number.parseFloat(text);
                setReferrerRebateRate(text);
                if (!Number.isNaN(rate)) {
                  setRefereeRebateRate(
                    `${maxDecimal(new decimal_default(0), maxRate.sub(rate))}`
                  );
                  setShowError(maxRate.sub(rate) < new decimal_default(0));
                } else {
                  setRefereeRebateRate("");
                  setReferrerRebateRate("");
                }
              },
              suffix: (0, import_jsx_runtime.jsx)("div", { className: "oui-px-3 oui-text-base-contrast-54 oui-text-base", children: "%" }),
              color: showError ? "danger" : void 0
            }
          ),
          (0, import_jsx_runtime.jsx)("div", { className: "oui-text-2xs oui-mt-6 oui-text-base-contrast-80", children: t("affiliate.referralRate.editRateModal.label.referee") }),
          (0, import_jsx_runtime.jsx)(
            Input2,
            {
              containerClassName: "oui-h-[40px] oui-mt-3 oui-bg-base-700 oui-outline oui-outline-1 oui-outline-base-contrast-12 focus-within:oui-outline-primary-darken",
              placeholder: "Enter code",
              type: "text",
              inputMode: "decimal",
              autoComplete: "off",
              autoFocus: false,
              value: refereeRebateRate,
              onChange: (e) => {
                const text = cleanStringStyle(e.target.value);
                const rate = Number.parseFloat(text);
                setRefereeRebateRate(text);
                if (!Number.isNaN(rate)) {
                  setReferrerRebateRate(
                    `${maxDecimal(new decimal_default(0), maxRate.sub(rate))}`
                  );
                  setShowError(maxRate.sub(rate) < new decimal_default(0));
                } else {
                  setRefereeRebateRate("");
                  setReferrerRebateRate("");
                }
              },
              suffix: (0, import_jsx_runtime.jsx)("div", { className: "oui-px-3 oui-text-base-contrast-54 oui-text-base", children: "%" }),
              color: showError ? "danger" : void 0
            }
          ),
          showError && (0, import_jsx_runtime.jsx)("div", { className: "oui-text-danger oui-text-xs oui-mt-8 oui-text-center oui-px-4", children: t("affiliate.referralRate.editRateModal.helpText.max") }),
          (0, import_jsx_runtime.jsx)(Flex, { width: "100%", justify: "center", children: (0, import_jsx_runtime.jsx)(
            Button,
            {
              id: "referral_bind_referral_code_btn",
              disabled: refereeRebateRate.length === 0 || referrerRebateRate.length === 0 || showError,
              loading: isMutating,
              className: M(
                "oui-mt-8 oui-mb-4 oui-w-[154px]",
                showError && "oui-mt-3"
              ),
              onClick: (e) => {
                e.stopPropagation();
                onClickConfirm();
              },
              children: t("common.confirm")
            }
          ) })
        ] })
      ]
    }
  ) });
});
function maxDecimal(a, b) {
  return a > b ? a : b;
}
var useReferralCodesScript = () => {
  const { referralInfo, referralLinkUrl, mutate, generateCode } = useReferralContext();
  const copyLink = (code) => {
    copyText(generateReferralLink(referralLinkUrl, code));
  };
  const copyCode = (code) => {
    copyText(code);
  };
  const editRate = (code) => {
    modal.show(EditReferralRate, { code: { ...code }, mutate });
  };
  const editCode = (code) => {
    modal.show(EditCodeModal, {
      code: { ...code },
      successCallback: () => {
        mutate();
      }
    });
  };
  const [pinCodes, setPinCodes] = useLocalStorage(
    "orderly_referral_codes",
    []
  );
  const setPinCode = (code, del) => {
    if (del) {
      const index = pinCodes.findIndex((item) => item === code);
      if (index !== -1) {
        pinCodes.splice(index, 1);
      }
    } else {
      pinCodes.splice(0, 0, code);
    }
    if (pinCodes.length > 6) {
      pinCodes.splice(pinCodes.length - 1, 1);
    }
    setPinCodes([...pinCodes]);
  };
  const codes = (0, import_react.useMemo)(() => {
    if (referralInfo?.referrer_info?.referral_codes) {
      const referralCodes = referralInfo.referrer_info.referral_codes.map((item) => {
        const code = {
          ...item
        };
        if (generateCode && generateCode.code === item.code) {
          code.isAutoGenerated = true;
        }
        return code;
      });
      const pinedItems = [];
      const unpinedItems = [...referralCodes];
      for (let i = 0; i < pinCodes.length; i++) {
        const code = pinCodes[i];
        const index = unpinedItems.findIndex((item) => item.code === code);
        if (index !== -1) {
          pinedItems.push({ ...unpinedItems[index], isPined: true });
          unpinedItems.splice(index, 1);
        }
      }
      const allCodes = [...pinedItems, ...unpinedItems];
      if (allCodes.length === 0) {
        return void 0;
      }
      return allCodes;
    }
    return void 0;
  }, [referralInfo?.referrer_info?.referral_codes, pinCodes, generateCode]);
  return {
    copyCode,
    copyLink,
    editRate,
    editCode,
    setPinCode,
    codes
  };
};
var ReferralCodesWidget = () => {
  const state = useReferralCodesScript();
  return (0, import_jsx_runtime.jsx)(ReferralCodes, { ...state });
};
var CommissionAndReferees = (props) => {
  const { t } = useTranslation();
  return (0, import_jsx_runtime.jsx)(
    Flex,
    {
      id: "oui-affiliate-affiliate-commissionAndReferees",
      r: "2xl",
      p: 6,
      width: "100%",
      gap: 4,
      direction: "column",
      className: "oui-bg-base-9 oui-tabular-nums",
      children: (0, import_jsx_runtime.jsxs)(Tabs, { defaultValue: "account", className: "oui-w-full", variant: "contained", children: [
        (0, import_jsx_runtime.jsx)(TabPanel, { value: "account", title: t("affiliate.commission"), children: (0, import_jsx_runtime.jsx)(CommissionList, { ...props }) }),
        (0, import_jsx_runtime.jsx)(TabPanel, { value: "password", title: t("affiliate.myReferees"), children: (0, import_jsx_runtime.jsx)(RefereesList, { ...props }) })
      ] })
    }
  );
};
var MobileCellItem2 = (props) => {
  const { title, value, align, className, rule, formatString, prefix } = props;
  return (0, import_jsx_runtime.jsx)(
    Statistic,
    {
      className: M("oui-flex-1", className),
      label: (0, import_jsx_runtime.jsx)(Text2, { className: "oui-text-base-contrast-36 oui-text-2xs", children: title }),
      align,
      children: (0, import_jsx_runtime.jsx)(
        Text2.formatted,
        {
          rule: rule || "",
          formatString,
          prefix,
          className: "oui-text-base-contrast-80 oui-text-sm oui-mt-[6px]",
          children: value
        }
      )
    }
  );
};
var CommissionList = (props) => {
  const { t } = useTranslation();
  const isLG = useMediaQuery("(max-width: 767px)");
  const columns = (0, import_react.useMemo)(() => {
    const cols = [
      {
        title: `${t("affiliate.commission")} (USDC)`,
        dataIndex: "referral_rebate",
        render: (value) => (0, import_jsx_runtime.jsx)(Text2, { children: commifyOptional(value, {
          fix: 6,
          fallback: "0",
          padEnd: true,
          prefix: "$"
        }) }),
        width: 216
      },
      {
        title: `${t("affiliate.referralVol")} (USDC)`,
        dataIndex: "volume",
        render: (value) => (0, import_jsx_runtime.jsx)(Text2, { children: commifyOptional(value, {
          fix: 2,
          fallback: "0",
          padEnd: true,
          prefix: "$"
        }) }),
        width: 216
      },
      {
        title: t("affiliate.commission.column.activeUsers"),
        dataIndex: "daily_traded_referral",
        render: (value) => (0, import_jsx_runtime.jsx)(Text2, { children: value }),
        width: 216
      },
      {
        title: t("common.date"),
        dataIndex: "date",
        render: (value) => formatYMDTime(value),
        width: 216
      }
    ];
    return cols;
  }, [t]);
  const body = (0, import_react.useMemo)(() => {
    if (isLG) {
      return (0, import_jsx_runtime.jsx)(
        ListView,
        {
          className: "oui-max-h-[200px] oui-w-full",
          dataSource: props.commission.data,
          loadMore: props.commission.loadMore,
          isLoading: props.commission.isLoading,
          renderItem: (e) => {
            return (0, import_jsx_runtime.jsxs)("div", { children: [
              (0, import_jsx_runtime.jsxs)(Flex, { direction: "row", width: "100%", children: [
                (0, import_jsx_runtime.jsx)(
                  MobileCellItem2,
                  {
                    title: t("affiliate.commission"),
                    value: commifyOptional(e.referral_rebate, {
                      fix: 6,
                      fallback: "0",
                      padEnd: true
                    }),
                    prefix: "$"
                  }
                ),
                (0, import_jsx_runtime.jsx)(
                  MobileCellItem2,
                  {
                    title: t("affiliate.referralVol"),
                    value: commifyOptional(e.volume, {
                      fix: 2,
                      fallback: "0",
                      padEnd: true
                    }),
                    prefix: "$"
                  }
                ),
                (0, import_jsx_runtime.jsx)(
                  MobileCellItem2,
                  {
                    title: t("affiliate.commission.column.activeUsers"),
                    value: e.daily_traded_referral
                  }
                ),
                (0, import_jsx_runtime.jsx)(
                  MobileCellItem2,
                  {
                    title: t("common.date"),
                    value: e.date,
                    rule: "date",
                    formatString: "yyyy-MM-dd",
                    align: "end"
                  }
                )
              ] }),
              (0, import_jsx_runtime.jsx)(Divider, { className: "oui-mt-3 oui-w-full" })
            ] });
          }
        }
      );
    }
    return (0, import_jsx_runtime.jsx)(
      AuthGuardDataTable,
      {
        bordered: true,
        columns,
        loading: props.commission.isLoading,
        ignoreLoadingCheck: true,
        dataSource: props.commission.data,
        pagination: props.commission.pagination,
        onRow: (record) => {
          return {
            className: "oui-h-[41px]"
          };
        }
      }
    );
  }, [isLG, props.commission]);
  return (0, import_jsx_runtime.jsxs)(
    Flex,
    {
      direction: "column",
      width: "100%",
      justify: "start",
      itemAlign: "start",
      children: [
        (0, import_jsx_runtime.jsx)(
          DateFiler,
          {
            value: props.commission.dateRange,
            setValue: props.commission.setDateRange
          }
        ),
        body
      ]
    }
  );
};
var RefereesList = (props) => {
  const { t } = useTranslation();
  const isLG = useMediaQuery("(max-width: 767px)");
  const columns = (0, import_react.useMemo)(() => {
    const cols = [
      {
        title: t("affiliate.referees.column.refereeAddress"),
        dataIndex: "user_address",
        render: (value) => (0, import_jsx_runtime.jsx)(Text2.formatted, { rule: "address", children: value }),
        className: "oui-w-1/5"
      },
      {
        title: t("affiliate.referralCode"),
        dataIndex: "referral_code",
        // render: (value) => value,
        className: "oui-w-1/5"
      },
      {
        title: `${t("affiliate.referees.column.totalCommission")} (USDC)`,
        dataIndex: "referral_rebate",
        render: (value) => (0, import_jsx_runtime.jsx)(Text2, { children: commifyOptional(value, { fix: 6, prefix: "$", padEnd: true }) }),
        className: "oui-w-1/5"
      },
      {
        title: `${t("affiliate.referees.column.totalVol")} (USDC)`,
        dataIndex: "volume",
        render: (value) => (0, import_jsx_runtime.jsx)(Text2, { children: commifyOptional(value, { fix: 2, prefix: "$", padEnd: true }) }),
        className: "oui-w-1/5"
      },
      {
        title: t("affiliate.referees.column.invitationTime"),
        dataIndex: "code_binding_time",
        render: (value) => (0, import_jsx_runtime.jsx)(
          Text2.formatted,
          {
            rule: "date",
            formatString: "yyyy-MM-dd",
            children: value
          }
        ),
        className: "oui-w-1/5"
      }
    ];
    return cols;
  }, [t]);
  const body = (0, import_react.useMemo)(() => {
    if (isLG) {
      return (0, import_jsx_runtime.jsx)(
        ListView,
        {
          className: "oui-w-full oui-max-h-[200px]",
          dataSource: props.referees.data,
          loadMore: props.referees.loadMore,
          isLoading: props.referees.isLoading,
          renderItem: (e, index) => {
            return (0, import_jsx_runtime.jsxs)(
              Flex,
              {
                direction: "column",
                gap: 3,
                className: "oui-border-b-2 oui-border-line-6 oui-pb-3",
                children: [
                  (0, import_jsx_runtime.jsxs)(Flex, { direction: "row", width: "100%", children: [
                    (0, import_jsx_runtime.jsx)(
                      MobileCellItem2,
                      {
                        title: t("affiliate.referralCode"),
                        value: e.referral_code
                      }
                    ),
                    (0, import_jsx_runtime.jsx)(
                      MobileCellItem2,
                      {
                        title: `${t(
                          "affiliate.referees.column.totalCommission"
                        )} (USDC)`,
                        value: commifyOptional(e.referral_rebate, {
                          fix: 6,
                          prefix: "$",
                          padEnd: true
                        }),
                        className: "oui-min-w-[102px]"
                      }
                    ),
                    (0, import_jsx_runtime.jsx)(
                      MobileCellItem2,
                      {
                        title: t("affiliate.referees.column.totalVol"),
                        value: commifyOptional(e.volume, {
                          fix: 2,
                          prefix: "$",
                          padEnd: true
                        }),
                        align: "end"
                      }
                    )
                  ] }),
                  (0, import_jsx_runtime.jsxs)(Flex, { direction: "row", pt: 3, gap: 3, width: "100%", children: [
                    (0, import_jsx_runtime.jsx)(
                      MobileCellItem2,
                      {
                        title: t("affiliate.referees.column.refereeAddress"),
                        value: e.user_address,
                        rule: "address"
                      }
                    ),
                    (0, import_jsx_runtime.jsx)(
                      MobileCellItem2,
                      {
                        title: t("affiliate.referees.column.invitationTime"),
                        value: e.code_binding_time,
                        align: "end",
                        rule: "date",
                        formatString: "yyyy-MM-dd"
                      }
                    )
                  ] })
                ]
              },
              index
            );
          }
        }
      );
    }
    return (0, import_jsx_runtime.jsx)(
      AuthGuardDataTable,
      {
        bordered: true,
        loading: props.referees.isLoading,
        ignoreLoadingCheck: true,
        columns,
        dataSource: props.referees.data,
        pagination: props.referees.pagination,
        onRow: (record) => {
          return {
            className: "oui-h-[41px]"
          };
        }
      }
    );
  }, [isLG, props.referees]);
  return (0, import_jsx_runtime.jsx)(
    Flex,
    {
      direction: "column",
      width: "100%",
      justify: "start",
      itemAlign: "start",
      children: body
    }
  );
};
var DateFiler = (props) => {
  return (0, import_jsx_runtime.jsx)(Flex, { width: "100%", height: 49, className: "oui-border-b oui-border-line-6", children: (0, import_jsx_runtime.jsx)("div", { children: (0, import_jsx_runtime.jsx)(
    DatePicker2.range,
    {
      size: "xs",
      value: props.value,
      onChange: (range) => {
        props.setValue(range);
      },
      max: 90,
      disabled: {
        after: /* @__PURE__ */ new Date()
      }
    }
  ) }) });
};
var useCommissionAndRefereesScript = () => {
  const commission = useCommissionDataScript();
  const referees = useRefereesDataScript();
  return {
    commission,
    referees
  };
};
var useCommissionDataScript = () => {
  const [commissionRange, setCommissionRange] = (0, import_react.useState)(
    {
      from: subDays(/* @__PURE__ */ new Date(), 90),
      to: subDays(/* @__PURE__ */ new Date(), 1)
    }
  );
  const isLG = useMediaQuery("(max-width: 767px)");
  const { page, pageSize, setPage, parsePagination } = usePagination();
  const [commissionData, { refresh, isLoading, loadMore, meta }] = useReferralRebateSummary({
    startDate: commissionRange?.from !== void 0 ? format(commissionRange.from, "yyyy-MM-dd") : void 0,
    endDate: commissionRange?.to !== void 0 ? format(commissionRange.to, "yyyy-MM-dd") : void 0,
    size: pageSize,
    page: !isLG ? page : void 0
  });
  (0, import_react.useEffect)(() => {
    refresh();
  }, [commissionRange]);
  const pagination = (0, import_react.useMemo)(
    () => parsePagination(meta),
    [parsePagination, meta]
  );
  (0, import_react.useEffect)(() => {
    setPage(1);
  }, [commissionRange]);
  return {
    data: commissionData || void 0,
    pagination,
    dateRange: commissionRange,
    setDateRange: setCommissionRange,
    isLoading,
    loadMore
  };
};
var useRefereesDataScript = () => {
  const [commissionRange, setCommissionRange] = (0, import_react.useState)(
    {
      from: subDays(/* @__PURE__ */ new Date(), 90),
      to: subDays(/* @__PURE__ */ new Date(), 1)
    }
  );
  const isLG = useMediaQuery("(max-width: 767px)");
  const { page, pageSize, setPage, parsePagination } = usePagination();
  const [commissionData, { refresh, isLoading, loadMore, meta }] = useRefereeInfo({
    startDate: commissionRange?.from !== void 0 ? format(commissionRange.from, "yyyy-MM-dd") : void 0,
    endDate: commissionRange?.to !== void 0 ? format(commissionRange.to, "yyyy-MM-dd") : void 0,
    size: pageSize,
    page: !isLG ? page : void 0,
    sort: "descending_code_binding_time"
  });
  (0, import_react.useEffect)(() => {
    refresh();
  }, [commissionRange]);
  const pagination = (0, import_react.useMemo)(
    () => parsePagination(meta),
    [parsePagination, meta]
  );
  (0, import_react.useEffect)(() => {
    setPage(1);
  }, [commissionRange]);
  return {
    data: commissionData || void 0,
    pagination,
    dateRange: commissionRange,
    setDateRange: setCommissionRange,
    isLoading,
    loadMore
  };
};
var CommissionAndRefereesWidget = () => {
  const state = useCommissionAndRefereesScript();
  return (0, import_jsx_runtime.jsx)(CommissionAndReferees, { ...state });
};
var AffiliatePage = () => {
  return (0, import_jsx_runtime.jsxs)(
    Flex,
    {
      id: "oui-affiliate-affiliate-page",
      className: M(
        "oui-h-lvw",
        // padding
        // "oui-p-4 lg:oui-p-6 xl:oui-p-3",
        "oui-font-semibold"
      ),
      direction: "column",
      gap: 4,
      children: [
        (0, import_jsx_runtime.jsx)(Layout, {}),
        (0, import_jsx_runtime.jsx)(CommissionAndRefereesWidget, {})
      ]
    }
  );
};
var Layout = () => {
  const is2XL = useMediaQuery("(max-width: 1279px)");
  if (is2XL) {
    return (0, import_jsx_runtime.jsx)(Layout1024, {});
  }
  return (0, import_jsx_runtime.jsx)(Layout1280, {});
};
var Layout1024 = () => {
  return (0, import_jsx_runtime.jsxs)(import_jsx_runtime.Fragment, { children: [
    (0, import_jsx_runtime.jsxs)("div", { className: "oui-flex oui-flex-col xl:oui-flex-row oui-gap-4 oui-w-full", children: [
      (0, import_jsx_runtime.jsx)(SummaryWidget2, {}),
      (0, import_jsx_runtime.jsx)(ReferralLinkWidget, {})
    ] }),
    (0, import_jsx_runtime.jsx)(TitleStatisticWidget, {}),
    (0, import_jsx_runtime.jsx)(ReferralCodesWidget, {})
  ] });
};
var Layout1280 = () => {
  return (0, import_jsx_runtime.jsxs)(Flex, { direction: "row", itemAlign: "stretch", gap: 4, children: [
    (0, import_jsx_runtime.jsxs)(Flex, { direction: "column", gap: 4, className: "oui-flex-1 oui-w-1/2", children: [
      (0, import_jsx_runtime.jsx)("div", { className: "oui-flex-1 oui-h-full oui-w-full", children: (0, import_jsx_runtime.jsx)(SummaryWidget2, {}) }),
      (0, import_jsx_runtime.jsx)(TitleStatisticWidget, {})
    ] }),
    (0, import_jsx_runtime.jsxs)(Flex, { direction: "column", gap: 4, className: "oui-w-1/2", children: [
      (0, import_jsx_runtime.jsx)(ReferralLinkWidget, {}),
      (0, import_jsx_runtime.jsx)("div", { className: "oui-flex-1", children: (0, import_jsx_runtime.jsx)(ReferralCodesWidget, {}) })
    ] })
  ] });
};
var BecomeAffiliate = (props) => {
  const { t } = useTranslation();
  const { generateCode } = useReferralContext();
  const applyText = (0, import_react.useMemo)(() => {
    if (!generateCode) {
      return {
        title: t("affiliate.process.step1.title"),
        desc: t("affiliate.process.step1.description")
      };
    }
    if (generateCode.requireVolume > 0) {
      return {
        title: t("affiliate.process.step1.volumeGt0.title", {
          requireVolume: generateCode.requireVolume
        }),
        desc: t("affiliate.process.step1.volumeGt0.description", {
          volume: commify(generateCode.completedVolume ?? 0, 2),
          requireVolume: commify(generateCode.requireVolume)
        })
      };
    }
    return {
      title: t("affiliate.process.step1.volumeEq0.title"),
      desc: t("affiliate.process.step1.volumeEq0.description")
    };
  }, [generateCode, t]);
  if (typeof props.overwrite === "function") {
    return props.overwrite?.(props.state);
  }
  return (0, import_jsx_runtime.jsxs)(
    Flex,
    {
      id: "oui-affiliate-home-becomeAffiliate",
      direction: "column",
      gap: 6,
      p: 6,
      itemAlign: "center",
      r: "2xl",
      className: "oui-bg-base-9",
      width: "100%",
      children: [
        (0, import_jsx_runtime.jsx)(Text2, { children: t("affiliate.process.title") }),
        (0, import_jsx_runtime.jsxs)(Flex, { className: "oui-flex oui-flex-col oui-gap-3 lg:oui-w-full lg:oui-flex-row lg:oui-items-stretch", children: [
          (0, import_jsx_runtime.jsx)(
            Item,
            {
              icon: (0, import_jsx_runtime.jsx)(ApplyIcon, {}),
              title: applyText.title,
              content: applyText.desc
            }
          ),
          (0, import_jsx_runtime.jsx)("div", { className: "oui-shrink lg:oui-flex lg:-oui-rotate-90 lg:oui-flex-row lg:oui-items-center lg:oui-justify-center", children: (0, import_jsx_runtime.jsx)(ArrowDownIcon, {}) }),
          (0, import_jsx_runtime.jsx)(
            Item,
            {
              icon: (0, import_jsx_runtime.jsx)(ShareIcon, {}),
              title: t("affiliate.process.step2.title"),
              content: t("affiliate.process.step2.description")
            }
          ),
          (0, import_jsx_runtime.jsx)("div", { className: "oui-shrink lg:oui-flex lg:-oui-rotate-90 lg:oui-flex-row lg:oui-items-center lg:oui-justify-center", children: (0, import_jsx_runtime.jsx)(ArrowDownIcon, {}) }),
          (0, import_jsx_runtime.jsx)(
            Item,
            {
              icon: (0, import_jsx_runtime.jsx)(EarnIcon, {}),
              title: t("affiliate.process.step3.title"),
              content: t("affiliate.process.step3.description")
            }
          )
        ] })
      ]
    }
  );
};
var Item = (props) => {
  return (0, import_jsx_runtime.jsxs)(
    Flex,
    {
      className: M(
        "oui-flex oui-flex-row oui-gap-3",
        "lg:oui-flex-1 lg:oui-flex-col lg:oui-gap-[6px]"
      ),
      width: "100%",
      children: [
        (0, import_jsx_runtime.jsx)("div", { className: "oui-shrink-0", children: props.icon }),
        (0, import_jsx_runtime.jsxs)(
          Flex,
          {
            className: M(
              "oui-flex oui-flex-col oui-items-start oui-h-full oui-justify-between",
              // lg
              "lg:oui-items-center lg:oui-justify-start"
            ),
            children: [
              (0, import_jsx_runtime.jsx)(Text2, { className: "oui-text-sm md:oui-text-base 2xl:oui-text-lg lg:oui-text-center", children: props.title }),
              (0, import_jsx_runtime.jsx)(Text2, { className: "oui-text-2xs oui-text-base-contrast-36  lg:oui-text-center", children: props.content })
            ]
          }
        )
      ]
    }
  );
};
var ApplyIcon = () => {
  return (0, import_jsx_runtime.jsxs)(
    "svg",
    {
      width: "48",
      height: "48",
      viewBox: "0 0 48 48",
      fill: "none",
      xmlns: "http://www.w3.org/2000/svg",
      className: "lg:oui-w-[64px] lg:oui-h-[64px] 2xl:oui-w-[80px] 2xl:oui-h-[80px]",
      children: [
        (0, import_jsx_runtime.jsx)(
          "path",
          {
            fillRule: "evenodd",
            clipRule: "evenodd",
            d: "M24.941 5.4h-6.682a.6.6 0 0 0-.593.509l-.462 3a.6.6 0 0 0 .593.691h7.606a.6.6 0 0 0 .593-.691l-.462-3a.6.6 0 0 0-.593-.509m-6.682-1.8a2.4 2.4 0 0 0-2.372 2.035l-.462 3a2.4 2.4 0 0 0 2.372 2.765h7.606a2.4 2.4 0 0 0 2.372-2.765l-.462-3A2.4 2.4 0 0 0 24.941 3.6z",
            fill: "#fff",
            fillOpacity: ".36"
          }
        ),
        (0, import_jsx_runtime.jsx)(
          "path",
          {
            d: "M32.721 28.2c.924-1.6 3.233-1.6 4.157 0l7.275 12.6c.924 1.6-.231 3.6-2.079 3.6H27.525c-1.848 0-3.002-2-2.078-3.6z",
            fill: "#335FFC"
          }
        ),
        (0, import_jsx_runtime.jsx)(
          "path",
          {
            d: "M33.492 38.12H31.56c-.252 0-.426-.276-.336-.535l1.92-5.53a.4.4 0 0 1 .132-.185.34.34 0 0 1 .204-.07h3.24c.255 0 .43.284.333.544l-1.118 3.011h2.104c.31 0 .475.4.27.656l-5.158 6.453c-.251.313-.716.041-.619-.362z",
            fill: "#fff",
            fillOpacity: ".98"
          }
        ),
        (0, import_jsx_runtime.jsx)(
          "path",
          {
            fillRule: "evenodd",
            clipRule: "evenodd",
            d: "M14.4 16.8v2.4h2.4v-2.4zM13.2 15a.6.6 0 0 0-.6.6v4.8a.6.6 0 0 0 .6.6H18a.6.6 0 0 0 .6-.6v-4.8a.6.6 0 0 0-.6-.6z",
            fill: "#fff",
            fillOpacity: ".36"
          }
        ),
        (0, import_jsx_runtime.jsx)(
          "path",
          {
            d: "M20.4 16.2a.6.6 0 0 1 .6-.6h2.4a.6.6 0 0 1 .6.6v.6a.6.6 0 0 1-.6.6H21a.6.6 0 0 1-.6-.6zm0 3a.6.6 0 0 1 .6-.6h9a.6.6 0 0 1 .6.6v.6a.6.6 0 0 1-.6.6h-9a.6.6 0 0 1-.6-.6zm-7.8 5.4a.6.6 0 0 1 .6-.6H30a.6.6 0 0 1 .6.6v.6a.6.6 0 0 1-.6.6H13.2a.6.6 0 0 1-.6-.6zm0 3.6a.6.6 0 0 1 .6-.6H30a.6.6 0 0 1 .6.6v.6a.6.6 0 0 1-.6.6H13.2a.6.6 0 0 1-.6-.6zm0 3.6a.6.6 0 0 1 .6-.6h9a.6.6 0 0 1 .6.6v.6a.6.6 0 0 1-.6.6h-9a.6.6 0 0 1-.6-.6z",
            fill: "#fff",
            fillOpacity: ".36"
          }
        ),
        (0, import_jsx_runtime.jsx)(
          "path",
          {
            fillRule: "evenodd",
            clipRule: "evenodd",
            d: "M15.739 6.6H10.2A2.4 2.4 0 0 0 7.8 9v30a2.4 2.4 0 0 0 2.4 2.4h13.84l1.014-1.8H10.2a.6.6 0 0 1-.6-.6V9a.6.6 0 0 1 .6-.6h5.262zm12 1.8H33a.6.6 0 0 1 .6.6v16.47a2.8 2.8 0 0 1 1.8-.205V9A2.4 2.4 0 0 0 33 6.6h-5.538z",
            fill: "#fff",
            fillOpacity: ".36"
          }
        )
      ]
    }
  );
};
var ShareIcon = () => {
  return (0, import_jsx_runtime.jsxs)(
    "svg",
    {
      width: "48",
      height: "48",
      viewBox: "0 0 48 48",
      fill: "none",
      xmlns: "http://www.w3.org/2000/svg",
      className: "lg:oui-w-[64px] lg:oui-h-[64px] 2xl:oui-w-[80px] 2xl:oui-h-[80px]",
      children: [
        (0, import_jsx_runtime.jsx)("path", { d: "M24 33a9 9 0 1 1-18 0 9 9 0 0 1 18 0", fill: "#335FFC" }),
        (0, import_jsx_runtime.jsx)(
          "path",
          {
            fillRule: "evenodd",
            clipRule: "evenodd",
            d: "M15 32.4a1.8 1.8 0 1 0 0-3.6 1.8 1.8 0 0 0 0 3.6m0 1.8a3.6 3.6 0 1 0 0-7.2 3.6 3.6 0 0 0 0 7.2m-6.578 4.942A4.8 4.8 0 0 1 13.2 34.8h3.6a4.8 4.8 0 0 1 4.778 4.342 9 9 0 0 1-1.778 1.473V39.6a3 3 0 0 0-3-3h-3.6a3 3 0 0 0-3 3v1.015a9 9 0 0 1-1.778-1.473",
            fill: "#fff",
            fillOpacity: ".98"
          }
        ),
        (0, import_jsx_runtime.jsx)(
          "path",
          {
            fillRule: "evenodd",
            clipRule: "evenodd",
            d: "M16.2 12a4.2 4.2 0 1 1-8.4 0 4.2 4.2 0 0 1 8.4 0m1.8 0c0 1.37-.459 2.632-1.231 3.642l2.308 2.308a7.76 7.76 0 0 1 4.023-1.7v-.764A3.602 3.602 0 0 1 24 8.4a3.6 3.6 0 0 1 .9 7.087v.764c1.48.17 2.833.754 3.942 1.633l2.326-2.326a6 6 0 1 1 1.273 1.273l-2.326 2.326a7.76 7.76 0 0 1 1.634 3.943h.764a3.602 3.602 0 0 1 7.087.9 3.6 3.6 0 0 1-7.087.9h-.764a7.76 7.76 0 0 1-1.699 4.023l2.308 2.308a6 6 0 1 1-1.25 1.295l-2.347-2.347a7.8 7.8 0 0 1-3.05 1.433 11 11 0 0 0-.377-1.76 6 6 0 1 0-7.185-7.185q-.855-.261-1.76-.379a7.8 7.8 0 0 1 1.432-3.049l-2.347-2.347A6 6 0 1 1 18 12m7.8 0a1.8 1.8 0 1 1-3.6 0 1.8 1.8 0 0 1 3.6 0M36 25.8a1.8 1.8 0 1 0 0-3.6 1.8 1.8 0 0 0 0 3.6m0-9.6a4.2 4.2 0 1 0 0-8.4 4.2 4.2 0 0 0 0 8.4M40.2 36a4.2 4.2 0 1 1-8.4 0 4.2 4.2 0 0 1 8.4 0",
            fill: "#fff",
            fillOpacity: ".36"
          }
        )
      ]
    }
  );
};
var EarnIcon = () => {
  return (0, import_jsx_runtime.jsxs)(
    "svg",
    {
      width: "48",
      height: "48",
      viewBox: "0 0 48 48",
      fill: "none",
      xmlns: "http://www.w3.org/2000/svg",
      className: "lg:oui-w-[64px] lg:oui-h-[64px] 2xl:oui-w-[80px] 2xl:oui-h-[80px]",
      children: [
        (0, import_jsx_runtime.jsx)(
          "path",
          {
            fillRule: "evenodd",
            clipRule: "evenodd",
            d: "M24 24a2.4 2.4 0 1 0 0-4.8 2.4 2.4 0 0 0 0 4.8m0 1.8a4.2 4.2 0 1 0 0-8.4 4.2 4.2 0 0 0 0 8.4",
            fill: "#fff",
            fillOpacity: ".36"
          }
        ),
        (0, import_jsx_runtime.jsx)("path", { d: "M45 12a9 9 0 1 1-18 0 9 9 0 0 1 18 0", fill: "#335FFC" }),
        (0, import_jsx_runtime.jsx)(
          "path",
          {
            fillRule: "evenodd",
            clipRule: "evenodd",
            d: "M35.1 6.6a.9.9 0 1 1 1.8 0v.437A3 3 0 0 1 39 9.9h-1.8a1.2 1.2 0 1 0-1.2 1.2 3 3 0 0 1 .9 5.863v.437a.9.9 0 0 1-1.8 0v-.437A3 3 0 0 1 33 14.1h1.8a1.2 1.2 0 1 0 1.2-1.2 3 3 0 0 1-.9-5.863z",
            fill: "#fff",
            fillOpacity: ".98"
          }
        ),
        (0, import_jsx_runtime.jsx)(
          "path",
          {
            fillRule: "evenodd",
            clipRule: "evenodd",
            d: "M40.2 23.7a.9.9 0 0 1 1.8 0v.6q0 .045-.004.086c-.197 9.351-7.525 16.948-16.765 17.573l1.205 1.205a.9.9 0 1 1-1.272 1.272l-2.7-2.7a.9.9 0 0 1 0-1.272l2.7-2.7a.9.9 0 1 1 1.272 1.272l-1.11 1.11c8.325-.674 14.87-7.642 14.874-16.14zM21.564 4.836a.9.9 0 1 1 1.272-1.272l2.7 2.7a.9.9 0 0 1 0 1.272l-2.7 2.7a.9.9 0 1 1-1.272-1.272l1.11-1.11C14.347 8.528 7.8 15.5 7.8 24v.3a.9.9 0 1 1-1.8 0v-.6l.004-.086C6.201 14.263 13.53 6.666 22.77 6.042z",
            fill: "#fff",
            fillOpacity: ".36"
          }
        ),
        (0, import_jsx_runtime.jsx)(
          "path",
          {
            fillRule: "evenodd",
            clipRule: "evenodd",
            d: "M24 11.4q.615 0 1.213.058a11 11 0 0 0 .061 1.816Q24.647 13.2 24 13.2c-5.965 0-10.8 4.835-10.8 10.8 0 2.898 1.142 5.53 3 7.47a4.8 4.8 0 0 1 4.8-4.79h6a4.8 4.8 0 0 1 4.8 4.789c1.858-1.94 3-4.572 3-7.47q0-.647-.074-1.274a11 11 0 0 0 1.816.06q.057.6.058 1.214c0 6.959-5.641 12.6-12.6 12.6S11.4 30.959 11.4 24 17.041 11.4 24 11.4m6 21.581v-1.5a3 3 0 0 0-3-3h-6a3 3 0 0 0-3 3v1.5a10.75 10.75 0 0 0 6 1.819c2.22 0 4.284-.67 6-1.819",
            fill: "#fff",
            fillOpacity: ".36"
          }
        )
      ]
    }
  );
};
var ArrowDownIcon = () => {
  return (0, import_jsx_runtime.jsx)(
    "svg",
    {
      width: "17",
      height: "16",
      viewBox: "0 0 17 16",
      fill: "none",
      xmlns: "http://www.w3.org/2000/svg",
      children: (0, import_jsx_runtime.jsx)(
        "path",
        {
          d: "M8.5048 4.00806C8.8728 4.00806 9.1708 4.30626 9.1708 4.67406C9.1708 4.99259 9.1708 8.71739 9.1708 9.71072L11.1688 7.73339L12.1055 8.67005L8.98346 11.8127C8.85346 11.9434 8.6788 12.0081 8.50413 12.0081C8.33013 12.0081 8.15612 11.9427 8.02612 11.8127L4.90413 8.67005L5.8408 7.73339L7.83881 9.71072C7.83881 8.71739 7.83881 4.99259 7.83881 4.67406C7.83881 4.30626 8.1368 4.00806 8.5048 4.00806Z",
          fill: "#608CFF"
        }
      )
    }
  );
};
var useBecomeAffiliateScript = () => {
  const state = useReferralContext();
  return {
    overwrite: state.overwrite?.ref?.step,
    state
  };
};
var BecomeAffiliateWidget = () => {
  const state = useBecomeAffiliateScript();
  return (0, import_jsx_runtime.jsx)(BecomeAffiliate, { ...state });
};
var Title5 = () => {
  const { t } = useTranslation();
  return (0, import_jsx_runtime.jsx)(
    "div",
    {
      id: "oui-affiliate-home-title",
      className: "oui-text-center oui-text-3xl oui-font-bold md:oui-text-3xl lg:oui-text-4xl xl:oui-text-5xl",
      children: t("affiliate.page.title")
    }
  );
};
var TitleWidget = () => {
  return (0, import_jsx_runtime.jsx)(Title5, {});
};
var Subtitle2 = (props) => {
  const { t } = useTranslation();
  return (0, import_jsx_runtime.jsxs)(
    Flex,
    {
      id: "oui-affiliate-home-subtitle",
      direction: "column",
      className: "oui-text-sm md:oui-text-base xl:oui-text-lg oui-text-base-contrast-80 oui-text-center",
      gap: 3,
      children: [
        (0, import_jsx_runtime.jsx)(Text2, { children: t("affiliate.page.subTitle") }),
        (0, import_jsx_runtime.jsxs)(
          Flex,
          {
            direction: "row",
            gap: 1,
            className: "oui-text-primary-light oui-fill-primary-light hover:oui-text-primary-darken oui-cursor-pointer oui-text-2xs md:oui-text-xs xl:oui-text-sm",
            onClick: (e) => {
              if (props.onLearnAffiliate) {
                props.onLearnAffiliate?.();
              } else if (props.learnAffiliateUrl) {
                window.open(props.learnAffiliateUrl, "_blank");
              }
            },
            children: [
              (0, import_jsx_runtime.jsx)(Text2, { children: t("affiliate.page.learnMore") }),
              (0, import_jsx_runtime.jsx)(
                "svg",
                {
                  width: "16",
                  height: "16",
                  viewBox: "0 0 16 16",
                  fill: "currentColor",
                  xmlns: "http://www.w3.org/2000/svg",
                  children: (0, import_jsx_runtime.jsx)("path", { d: "M4.008 7.995c0-.368.298-.666.666-.666H9.71L7.733 5.331l.937-.936 3.143 3.122c.13.13.195.304.195.479a.67.67 0 0 1-.195.478L8.67 11.596l-.937-.937 1.978-1.998H4.674a.666.666 0 0 1-.666-.666" })
                }
              )
            ]
          }
        )
      ]
    }
  );
};
var useSubtitleScript = () => {
  const state = useReferralContext();
  const { onLearnAffiliate, learnAffiliateUrl } = state;
  return {
    onLearnAffiliate,
    learnAffiliateUrl
  };
};
var SubtitleWidget = () => {
  const state = useSubtitleScript();
  return (0, import_jsx_runtime.jsx)(Subtitle2, { ...state });
};
var Top = (props) => {
  if (props.overwriteTop !== void 0) {
    return props.overwriteTop?.(props.state);
  }
  return (0, import_jsx_runtime.jsxs)(Flex, { id: "oui-affiliate-home-top", direction: "column", gap: 6, children: [
    (0, import_jsx_runtime.jsx)(TitleWidget, {}),
    (0, import_jsx_runtime.jsx)(SubtitleWidget, {})
  ] });
};
var useTopScript = () => {
  const state = useReferralContext();
  return {
    overwriteTop: state.overwrite?.ref?.top,
    state
  };
};
var TopWidget = () => {
  const state = useTopScript();
  return (0, import_jsx_runtime.jsx)(Top, { ...state });
};
var ArrowRightIcon = (props) => {
  return (0, import_jsx_runtime.jsx)(
    "svg",
    {
      width: "16",
      height: "16",
      viewBox: "0 0 16 16",
      fill: "white",
      fillOpacity: "0.98",
      xmlns: "http://www.w3.org/2000/svg",
      className: props.className,
      children: (0, import_jsx_runtime.jsx)("path", { d: "M4.00806 7.9952C4.00806 7.6272 4.30626 7.3292 4.67406 7.3292C4.99259 7.3292 8.71739 7.3292 9.71072 7.3292L7.73339 5.3312L8.67005 4.39453L11.8127 7.51654C11.9434 7.64654 12.0081 7.8212 12.0081 7.99587C12.0081 8.16987 11.9427 8.34388 11.8127 8.47388L8.67005 11.5959L7.73339 10.6592L9.71072 8.66119C8.71739 8.66119 4.99259 8.66119 4.67406 8.66119C4.30626 8.66119 4.00806 8.3632 4.00806 7.9952Z" })
    }
  );
};
var AsTrader = (props) => {
  const { t } = useTranslation();
  return (0, import_jsx_runtime.jsxs)(
    Flex,
    {
      id: "oui-affiliate-home-asTrader",
      gradient: "success",
      r: "2xl",
      p: 6,
      gap: 6,
      direction: "column",
      angle: 180,
      width: "100%",
      justify: "between",
      children: [
        (0, import_jsx_runtime.jsxs)(
          Flex,
          {
            height: 80,
            direction: "row",
            gap: 3,
            itemAlign: "start",
            width: "100%",
            justify: "between",
            children: [
              (0, import_jsx_runtime.jsxs)(
                Flex,
                {
                  direction: "column",
                  itemAlign: "start",
                  justify: "between",
                  className: "oui-h-full",
                  children: [
                    (0, import_jsx_runtime.jsx)(Text2, { className: "oui-text-lg md:oui-text-xl lg:oui-text-2xl xl:oui-text-3xl", children: props.isTrader ? t("affiliate.trader") : t("affiliate.asTrader.title") }),
                    (0, import_jsx_runtime.jsx)(
                      Text2,
                      {
                        className: M(
                          "oui-text-xs oui-text-base-contrast-54 md:oui-text-sm 2xl:oui-text-base",
                          props.isTrader && "oui-hidden"
                        ),
                        children: t("affiliate.asTrader.description")
                      }
                    )
                  ]
                }
              ),
              (0, import_jsx_runtime.jsx)("div", { className: "oui-shrink-0", children: (0, import_jsx_runtime.jsx)(Icon, {}) })
            ]
          }
        ),
        (0, import_jsx_runtime.jsx)(Bottom, { ...props })
      ]
    }
  );
};
var Icon = () => {
  return (0, import_jsx_runtime.jsxs)(
    "svg",
    {
      width: "72",
      height: "72",
      viewBox: "0 0 90 90",
      fill: "none",
      xmlns: "http://www.w3.org/2000/svg",
      className: "xl:oui-size-[90px] ",
      children: [
        (0, import_jsx_runtime.jsx)(
          "path",
          {
            d: "M44.996 7.324c-20.71 0-37.43 16.79-37.5 37.5-.07 20.682 16.806 37.575 37.5 37.617 20.694.04 37.537-17.082 37.5-37.617-.037-20.71-16.79-37.5-37.5-37.5m0 7.5c16.569 0 30 13.433 30 30 0 8.438-3.514 16.043-9.12 21.495-2.762-5.973-8.694-10.245-15.49-10.245h-10.78c-6.791 0-12.706 4.22-15.47 10.197-5.606-5.453-9.14-13.01-9.14-21.447 0-16.567 13.431-30 30-30m0 7.5c-8.284 0-15 6.717-15 15s6.716 15 15 15 15-6.716 15-15-6.716-15-15-15",
            fill: "#fff",
            fillOpacity: ".2"
          }
        ),
        (0, import_jsx_runtime.jsx)(
          "path",
          {
            d: "M90 71.25C90 81.605 81.605 90 71.25 90S52.5 81.605 52.5 71.25 60.895 52.5 71.25 52.5 90 60.895 90 71.25",
            fill: "#005A4F"
          }
        ),
        (0, import_jsx_runtime.jsx)(
          "path",
          {
            d: "M80.62 75.007c0-.24-.08-.49-.263-.675l-3.487-3.456-1.318 1.318 1.845 1.875H63.745a.938.938 0 0 0 0 1.875h13.652l-1.845 1.875 1.318 1.318 3.487-3.456a.95.95 0 0 0 .263-.674m-.937-7.5a.94.94 0 0 0-.938-.938H65.093l1.846-1.875-1.319-1.318-3.486 3.456a.966.966 0 0 0 0 1.349l3.486 3.456 1.319-1.318-1.846-1.875h13.652c.518 0 .938-.42.938-.937",
            fill: "#fff",
            fillOpacity: ".98"
          }
        )
      ]
    }
  );
};
var Bottom = (props) => {
  const { t } = useTranslation();
  const content = () => {
    if (props.isTrader && !props.wrongNetwork) {
      const totalReferrerRebate = props.referralInfo?.referee_info?.total_referee_rebate;
      return (0, import_jsx_runtime.jsxs)(import_jsx_runtime.Fragment, { children: [
        (0, import_jsx_runtime.jsxs)(Flex, { direction: "column", itemAlign: "start", gap: 2, children: [
          (0, import_jsx_runtime.jsxs)(Text2, { className: "oui-text-2xs md:oui-text-xs xl:oui-text-sm", children: [
            t("affiliate.commission"),
            " ",
            (0, import_jsx_runtime.jsx)(Text2, { intensity: 36, children: "(USDC)" })
          ] }),
          (0, import_jsx_runtime.jsxs)(Flex, { direction: "row", gap: 1, children: [
            (0, import_jsx_runtime.jsx)(USDCIcon, {}),
            (0, import_jsx_runtime.jsx)(Text2, { className: "oui-text-base md:oui-text-lg lg:oui-text-xl xl:oui-text-2xl", children: commifyOptional(totalReferrerRebate, {
              fix: 2,
              fallback: "0"
            }) })
          ] })
        ] }),
        (0, import_jsx_runtime.jsxs)(
          Flex,
          {
            direction: "row",
            gap: 1,
            justify: "end",
            itemAlign: "center",
            className: "oui-cursor-pointer",
            onClick: props?.onEnterTraderPage,
            children: [
              (0, import_jsx_runtime.jsx)(Text2, { className: "oui-text-sm md:oui-text-base xl:oui-text-lg", children: t("affiliate.enter") }),
              (0, import_jsx_runtime.jsx)(ArrowRightIcon, { className: "md:oui-size-[18px] lg:oui-size-[20px] xl:oui-size-[24px]" })
            ]
          }
        )
      ] });
    }
    return (0, import_jsx_runtime.jsx)(EntryCode, { ...props });
  };
  return (0, import_jsx_runtime.jsx)(
    Flex,
    {
      direction: "row",
      justify: "between",
      width: "100%",
      itemAlign: "end",
      children: content()
    }
  );
};
var EntryCode = (props) => {
  const { t } = useTranslation();
  return (0, import_jsx_runtime.jsxs)(Dialog, { open: props.open, onOpenChange: props.setOpen, children: [
    (0, import_jsx_runtime.jsx)(DialogTrigger, { children: (0, import_jsx_runtime.jsx)(Button, { variant: "contained", color: "light", children: t("affiliate.asTrader.button") }) }),
    (0, import_jsx_runtime.jsxs)(DialogContent, { className: "oui-w-[320px] oui-font-semibold", children: [
      (0, import_jsx_runtime.jsx)(DialogHeader, { children: (0, import_jsx_runtime.jsx)(DialogTitle, { children: t("affiliate.referralCode.dialog.title") }) }),
      (0, import_jsx_runtime.jsx)(Divider, {}),
      (0, import_jsx_runtime.jsxs)(DialogBody, { children: [
        (0, import_jsx_runtime.jsx)(Text2, { size: "sm", intensity: 54, children: t("affiliate.referralCode.dialog.description") }),
        (0, import_jsx_runtime.jsx)(
          TextField,
          {
            className: "oui-mt-4 oui-w-full",
            placeholder: t("affiliate.referralCode"),
            value: props.code,
            onChange: (e) => {
              const _value = e.target.value.toUpperCase().replace(/[^A-Z0-9]/g, "");
              props.setCode(_value);
            },
            formatters: [
              formatter_exports.createRegexInputFormatter(
                (value) => {
                  return String(value).replace(
                    /[a-z]/g,
                    (char) => char.toUpperCase()
                  );
                }
              ),
              formatter_exports.createRegexInputFormatter(/[^A-Z0-9]/g)
            ],
            onClean: () => {
              props.setCode("");
            },
            label: t("affiliate.referralCode.label"),
            classNames: {
              label: "oui-text-2xs oui-text-base-contrast-54"
            },
            helpText: !props.isExist && !props.isLoading && props.code.length > 0 ? t("affiliate.referralCode.notExist") : void 0,
            color: !props.isExist && !props.isLoading && props.code.length > 0 ? "danger" : void 0
          }
        ),
        (0, import_jsx_runtime.jsxs)(Flex, { direction: "column", gapY: 3, mt: 6, children: [
          props.warningMessage && (0, import_jsx_runtime.jsx)(Text2, { size: "2xs", className: "oui-text-warning-darken", children: props.warningMessage }),
          (0, import_jsx_runtime.jsx)(AuthGuard, { buttonProps: { size: "md", fullWidth: true }, children: (0, import_jsx_runtime.jsx)(
            Button,
            {
              variant: "contained",
              color: "primary",
              size: "md",
              className: "oui-px-[40px]",
              fullWidth: true,
              disabled: props.code.length < 4 || !props.isExist,
              onClick: (e) => {
                e.stopPropagation();
                props.onClickConfirm();
              },
              children: t("common.confirm")
            }
          ) })
        ] })
      ] })
    ] })
  ] });
};
var useAsTraderScript = () => {
  const { t } = useTranslation();
  const {
    isTrader,
    referralInfo,
    setShowHome,
    bindReferralCodeState,
    setTab,
    mutate
  } = useReferralContext();
  const { wrongNetwork, disabledConnect } = useAppContext();
  const { state } = useAccount();
  const onEnterTraderPage = () => {
    setTab(
      "trader"
      /* trader */
    );
    setShowHome(false);
  };
  const [code, setCode] = (0, import_react.useState)("");
  const [open, setOpen] = (0, import_react.useState)(false);
  const {
    isExist,
    error: checkCodeError,
    isLoading
  } = useCheckReferralCode(code);
  const hide = () => {
    setOpen(false);
  };
  const [bindCode, { error, isMutating }] = useMutation(
    "/v1/referral/bind",
    "POST"
  );
  const onClickConfirm = async () => {
    try {
      await bindCode({ referral_code: code });
      Vt.success(t("affiliate.referralCode.bound"));
      mutate();
      if (bindReferralCodeState) {
        bindReferralCodeState(true, null, hide, { tab: 1 });
      } else {
        hide();
      }
    } catch (e) {
      let errorText = `${e}`;
      if ("message" in e) {
        errorText = e.message;
      }
      if ("referral code not exist" === errorText) {
        errorText = t("affiliate.referralCode.notExist");
      }
      if (bindReferralCodeState) {
        Vt.error(errorText);
        bindReferralCodeState(false, e, hide, {});
      } else {
        Vt.error(errorText);
      }
    }
  };
  const { isMobile } = useScreen();
  const authStatus = useAuthStatus();
  const warningMessage = (0, import_react.useMemo)(() => {
    const message = {
      [AuthStatusEnum.ConnectWallet]: t("affiliate.connectWallet.tooltip"),
      [AuthStatusEnum.CreateAccount]: t("affiliate.createAccount.tooltip"),
      [AuthStatusEnum.WrongNetwork]: t("connector.wrongNetwork.tooltip")
    };
    return message[authStatus];
  }, [t, authStatus]);
  return {
    isTrader,
    isLoading,
    referralInfo,
    onEnterTraderPage,
    code,
    setCode,
    open,
    setOpen,
    onClickConfirm,
    isExist,
    wrongNetwork,
    isMobile,
    warningMessage
  };
};
var AsTraderWidget = () => {
  const state = useAsTraderScript();
  return (0, import_jsx_runtime.jsx)(AsTrader, { ...state });
};
var AsAnAffiliate = (props) => {
  const { t } = useTranslation();
  return (0, import_jsx_runtime.jsxs)(
    Flex,
    {
      id: "oui-affiliate-home-asAnAffiliate",
      gradient: "primary",
      r: "2xl",
      p: 6,
      gap: 6,
      direction: "column",
      angle: 180,
      width: "100%",
      justify: "between",
      children: [
        (0, import_jsx_runtime.jsxs)(
          Flex,
          {
            direction: "row",
            gap: 3,
            itemAlign: "start",
            width: "100%",
            justify: "between",
            children: [
              (0, import_jsx_runtime.jsxs)(
                Flex,
                {
                  direction: "column",
                  itemAlign: "start",
                  justify: "between",
                  className: "oui-h-full",
                  children: [
                    (0, import_jsx_runtime.jsx)(Text2, { className: "oui-text-lg md:oui-text-xl lg:oui-text-2xl xl:oui-text-3xl", children: props.isAffiliate ? t("common.affiliate") : t("affiliate.asAffiliate.title") }),
                    (0, import_jsx_runtime.jsx)(
                      Text2,
                      {
                        className: M(
                          "oui-text-xs oui-text-base-contrast-54 md:oui-text-sm 2xl:oui-text-base",
                          props.isAffiliate && "oui-hidden"
                        ),
                        children: t("affiliate.asAffiliate.description")
                      }
                    )
                  ]
                }
              ),
              (0, import_jsx_runtime.jsx)("div", { className: "oui-shrink-0", children: (0, import_jsx_runtime.jsx)(Icon2, {}) })
            ]
          }
        ),
        (0, import_jsx_runtime.jsx)(Bottom2, { ...props })
      ]
    }
  );
};
var Icon2 = () => {
  return (0, import_jsx_runtime.jsxs)(
    "svg",
    {
      width: "72",
      height: "72",
      viewBox: "0 0 72 72",
      fill: "none",
      xmlns: "http://www.w3.org/2000/svg",
      className: "xl:oui-size-[90px]",
      children: [
        (0, import_jsx_runtime.jsx)(
          "path",
          {
            d: "M35.997 5.86c-16.569 0-29.944 13.43-30 30-.056 16.545 13.445 30.06 30 30.093s30.03-13.665 30-30.093c-.03-16.57-13.432-30-30-30m0 6c13.255 0 24 10.746 24 24 0 6.75-2.812 12.834-7.297 17.196-2.209-4.78-6.955-8.196-12.39-8.196h-8.626c-5.432 0-10.164 3.375-12.375 8.157-4.485-4.362-7.312-10.407-7.312-17.157 0-13.254 10.745-24 24-24m0 6c-6.628 0-12 5.373-12 12s5.372 12 12 12c6.627 0 12-5.373 12-12s-5.373-12-12-12",
            fill: "#fff",
            fillOpacity: ".2"
          }
        ),
        (0, import_jsx_runtime.jsx)(
          "path",
          {
            d: "M72 57c0 8.284-6.716 15-15 15s-15-6.716-15-15 6.716-15 15-15 15 6.716 15 15",
            fill: "#1828C3"
          }
        ),
        (0, import_jsx_runtime.jsx)(
          "path",
          {
            d: "M53.624 51.45a4.125 4.125 0 0 0-4.125 4.125c0 2.388 1.36 4.641 3.61 6.703a21 21 0 0 0 2.414 1.875c.287.194.556.37.797.516.147.089.243.154.304.187a.79.79 0 0 0 .75 0c.061-.033.158-.098.305-.187.24-.147.51-.322.797-.516a21 21 0 0 0 2.414-1.875c2.25-2.062 3.61-4.315 3.61-6.703a4.125 4.125 0 0 0-4.126-4.125c-1.294 0-2.557.705-3.351 1.734-.775-1.047-2.034-1.734-3.399-1.734",
            fill: "#fff",
            fillOpacity: ".98"
          }
        )
      ]
    }
  );
};
var Bottom2 = (props) => {
  const { t } = useTranslation();
  const content = () => {
    if (props.isAffiliate && !props.wrongNetwork) {
      const totalReferrerRebate = props.referralInfo?.referrer_info?.total_referrer_rebate;
      return (0, import_jsx_runtime.jsxs)(import_jsx_runtime.Fragment, { children: [
        (0, import_jsx_runtime.jsxs)(Flex, { direction: "column", itemAlign: "start", gap: 2, children: [
          (0, import_jsx_runtime.jsxs)(Text2, { className: "oui-text-2xs md:oui-text-xs xl:oui-text-sm", children: [
            t("affiliate.commission"),
            " ",
            (0, import_jsx_runtime.jsx)(Text2, { intensity: 36, children: "(USDC)" })
          ] }),
          (0, import_jsx_runtime.jsxs)(Flex, { direction: "row", gap: 1, children: [
            (0, import_jsx_runtime.jsx)(USDCIcon, {}),
            (0, import_jsx_runtime.jsx)(Text2, { className: "oui-text-base md:oui-text-lg lg:oui-text-xl xl:oui-text-2xl", children: commifyOptional(totalReferrerRebate, {
              fix: 2,
              fallback: "0"
            }) })
          ] })
        ] }),
        (0, import_jsx_runtime.jsxs)(
          Flex,
          {
            direction: "row",
            gap: 1,
            justify: "end",
            itemAlign: "center",
            className: "oui-cursor-pointer",
            onClick: props.onEnterAffiliatePage,
            children: [
              (0, import_jsx_runtime.jsx)(Text2, { className: "oui-text-sm md:oui-text-base xl:oui-text-lg", children: t("affiliate.enter") }),
              (0, import_jsx_runtime.jsx)(ArrowRightIcon, { className: "md:oui-size-[18px] lg:oui-size-[20px] xl:oui-size-[24px]" })
            ]
          }
        )
      ] });
    }
    return (0, import_jsx_runtime.jsxs)(import_jsx_runtime.Fragment, { children: [
      (0, import_jsx_runtime.jsx)(
        Button,
        {
          variant: "contained",
          color: "light",
          onClick: props.becomeAnAffiliate,
          children: t("affiliate.asAffiliate.button")
        }
      ),
      (0, import_jsx_runtime.jsxs)(
        Flex,
        {
          direction: "column",
          justify: "between",
          className: "oui-h-full",
          itemAlign: "end",
          children: [
            (0, import_jsx_runtime.jsx)(Text2, { className: "oui-text-base md:oui-text-lg lg:oui-text-xl 2xl:oui-text-2xl", children: t("affiliate.upTo") }),
            (0, import_jsx_runtime.jsx)(Text2, { className: "oui-text-2xs oui-text-base-contrast-54 md:oui-text-xs 2xl:oui-text-sm", children: t("affiliate.commission") })
          ]
        }
      )
    ] });
  };
  return (0, import_jsx_runtime.jsx)(
    Flex,
    {
      direction: "row",
      justify: "between",
      width: "100%",
      itemAlign: "end",
      children: content()
    }
  );
};
var useAsAnAffiliateScript = () => {
  const {
    isAffiliate,
    isLoading,
    referralInfo,
    becomeAnAffiliateUrl,
    setShowHome,
    setTab
  } = useReferralContext();
  const { wrongNetwork } = useAppContext();
  const becomeAnAffiliate = () => {
    window.open(becomeAnAffiliateUrl, "_blank");
  };
  const onEnterAffiliatePage = () => {
    setTab(
      "affiliate"
      /* affiliate */
    );
    setShowHome(false);
  };
  const { isMobile } = useScreen();
  return {
    isAffiliate,
    isLoading,
    referralInfo,
    onEnterAffiliatePage,
    becomeAnAffiliate,
    wrongNetwork,
    isMobile
  };
};
var AsAnAffiliateWidget = () => {
  const state = useAsAnAffiliateScript();
  return (0, import_jsx_runtime.jsx)(AsAnAffiliate, { ...state });
};
var Card = (props) => {
  if (typeof props.overwrite === "function") {
    return props.overwrite?.(props.state);
  }
  return (0, import_jsx_runtime.jsxs)(
    Flex,
    {
      id: "oui-affiliate-home-card",
      className: "oui-flex oui-w-full oui-flex-col oui-items-stretch oui-gap-6 xl:oui-flex-row xl:oui-gap-[36px]",
      children: [
        (0, import_jsx_runtime.jsx)(AsAnAffiliateWidget, {}),
        (0, import_jsx_runtime.jsx)(AsTraderWidget, {})
      ]
    }
  );
};
var useCardScript = () => {
  const state = useReferralContext();
  return {
    overwrite: state.overwrite?.ref?.card,
    state
  };
};
var CardWidget = () => {
  const state = useCardScript();
  return (0, import_jsx_runtime.jsx)(Card, { ...state });
};
var HomePage = () => {
  return (0, import_jsx_runtime.jsxs)(
    Flex,
    {
      id: "oui-affiliate-home-page",
      className: M(
        "oui-h-lvw ",
        // padding
        // "oui-p-4 lg:oui-p-6 xl:oui-p-3",
        "oui-font-semibold"
      ),
      direction: "column",
      gap: 4,
      children: [
        (0, import_jsx_runtime.jsx)(TopWidget, {}),
        (0, import_jsx_runtime.jsx)(CardWidget, {}),
        (0, import_jsx_runtime.jsx)(BecomeAffiliateWidget, {})
      ]
    }
  );
};
var Summary3 = (props) => {
  const { t } = useTranslation();
  return (0, import_jsx_runtime.jsxs)(
    Flex,
    {
      id: "oui-affiliate-trader-summary",
      r: "2xl",
      p: 6,
      width: "100%",
      height: "100%",
      gap: 4,
      direction: "column",
      className: "oui-bg-base-9 xl:oui-flex-1",
      children: [
        (0, import_jsx_runtime.jsx)(Title6, { ...props }),
        (0, import_jsx_runtime.jsx)(Rebate, { className: "md:oui-hidden", ...props }),
        (0, import_jsx_runtime.jsxs)(Flex, { direction: "column", gap: 3, width: "100%", className: "md:oui-mt-3", children: [
          (0, import_jsx_runtime.jsx)(SummaryFilter, { ...props }),
          (0, import_jsx_runtime.jsx)(CommissionData2, { ...props })
        ] }),
        (0, import_jsx_runtime.jsx)(Row2, { title: `${t("affiliate.trader.tradingVol")} (USDC)`, ...props })
      ]
    }
  );
};
var Title6 = (props) => {
  const { t } = useTranslation();
  return (0, import_jsx_runtime.jsxs)(Flex, { direction: "row", width: "100%", gap: 3, children: [
    (0, import_jsx_runtime.jsxs)(Flex, { direction: "row", gap: 3, className: "oui-flex-1", children: [
      (0, import_jsx_runtime.jsx)(Text2, { className: "oui-text-lg", children: t("affiliate.trader.yourReferrer") }),
      (0, import_jsx_runtime.jsx)(
        Button,
        {
          color: "secondary",
          size: "sm",
          className: "oui-text-primary-light oui-text-2xs md:oui-text-xs 2xl:oui-text-sm oui-bg-base-6",
          children: props.code
        }
      )
    ] }),
    (0, import_jsx_runtime.jsx)(
      Rebate,
      {
        className: "oui-hidden md:oui-flex md:oui-flex-shrink md:oui-w-fit",
        ...props
      }
    )
  ] });
};
var Rebate = (props) => {
  const { t } = useTranslation();
  return (0, import_jsx_runtime.jsxs)(Flex, { direction: "row", width: "100%", gap: 2, className: props.className, children: [
    (0, import_jsx_runtime.jsx)(
      Text2,
      {
        intensity: 54,
        className: "oui-text-sm md:oui-text-base 2xl:oui-text-lg",
        children: `${t("affiliate.trader.rebate")}:`
      }
    ),
    (0, import_jsx_runtime.jsx)(
      Text2.gradient,
      {
        color: "brand",
        className: "oui-text-lg md:oui-text-xl 2xl:oui-text-2xl",
        children: props.rebateText
      }
    )
  ] });
};
var SummaryFilter = (props) => {
  const { t } = useTranslation();
  return (0, import_jsx_runtime.jsxs)(Flex, { direction: "row", width: "100%", justify: "between", gap: 3, children: [
    (0, import_jsx_runtime.jsx)(Text2, { className: "oui-text-sm", children: t("affiliate.summary") }),
    (0, import_jsx_runtime.jsx)("div", { children: (0, import_jsx_runtime.jsx)(
      Select2.options,
      {
        size: "xs",
        value: props.period,
        onValueChange: props.onPeriodChange,
        options: props.periodTypes
      }
    ) })
  ] });
};
var CommissionData2 = (props) => {
  const { t } = useTranslation();
  return (0, import_jsx_runtime.jsxs)(
    Flex,
    {
      gradient: "success",
      angle: 180,
      r: "xl",
      py: 4,
      px: 6,
      width: "100%",
      direction: "column",
      gap: 3,
      children: [
        (0, import_jsx_runtime.jsx)(Text2, { intensity: 54, className: "oui-text-base 2xl:oui-text-lg", children: `${t("affiliate.trader.rebates")} (USDC)` }),
        (0, import_jsx_runtime.jsxs)(
          Flex,
          {
            direction: "row",
            gap: 3,
            className: "oui-text-xl md:oui-text-2xl xl:oui-text-3xl",
            children: [
              (0, import_jsx_runtime.jsx)(USDCIcon, { className: "md:oui-w-[24px] md:oui-h-[24px] lg:oui-w-[28px] lg:oui-h-[28px] " }),
              (0, import_jsx_runtime.jsx)(Text2, { children: commifyOptional(props.rebates, { fix: 2, fallback: "0" }) })
            ]
          }
        )
      ]
    }
  );
};
var Row2 = (props) => {
  return (0, import_jsx_runtime.jsxs)(Flex, { direction: "row", justify: "between", width: "100%", children: [
    (0, import_jsx_runtime.jsx)(
      Text2,
      {
        intensity: 54,
        className: "oui-text-2xs md:oui-text-xs xl:oui-text-sm",
        children: props.title
      }
    ),
    (0, import_jsx_runtime.jsx)(Text2, { className: "oui-text-xs md:oui-text-sm xl:oui-text-base", children: commifyOptional(props.vol, { fix: 2, fallback: "0" }) })
  ] });
};
var useSummaryScript3 = () => {
  const { t } = useTranslation();
  const [period, setPeriod] = (0, import_react.useState)("All");
  const periodTypes = [
    { label: t("common.all"), value: "All" },
    { label: t("common.select.1d"), value: "1D" },
    { label: t("common.select.7d"), value: "7D" },
    { label: t("common.select.30d"), value: "30D" }
  ];
  const { referralInfo, userVolume } = useReferralContext();
  const rebates = (0, import_react.useMemo)(() => {
    if (!referralInfo) return 0;
    switch (period) {
      case "All":
        return referralInfo.referee_info.total_referee_rebate;
      case "1D":
        return referralInfo.referee_info["1d_referee_rebate"];
      case "7D":
        return referralInfo.referee_info["7d_referee_rebate"];
      case "30D":
        return referralInfo.referee_info["30d_referee_rebate"];
    }
  }, [referralInfo, period]);
  const vol = (0, import_react.useMemo)(() => {
    if (!userVolume) return void 0;
    switch (period) {
      case "All":
        return userVolume.all_volume;
      case "1D":
        return userVolume["1d_volume"];
      case "7D":
        return userVolume["7d_volume"];
      case "30D":
        return userVolume["30d_volume"];
    }
  }, [userVolume, period]);
  const code = referralInfo?.referee_info?.referer_code;
  const rebate = referralInfo?.referee_info?.referee_rebate_rate;
  const rebateText = (0, import_react.useMemo)(() => {
    if (!!rebate) {
      return new decimal_default(rebate).mul(100).toDecimalPlaces(2, decimal_default.ROUND_DOWN).toString() + "%";
    }
    return "-";
  }, [rebate]);
  const onPeriodChange = (item) => {
    setPeriod(item);
  };
  return {
    period,
    periodTypes,
    onPeriodChange,
    rebates,
    vol,
    rebateText,
    code
  };
};
var SummaryWidget3 = () => {
  const state = useSummaryScript3();
  return (0, import_jsx_runtime.jsx)(Summary3, { ...state });
};
var TitleStatistic2 = (props) => {
  return (0, import_jsx_runtime.jsxs)(
    Flex,
    {
      id: "oui-affiliate-trader-titleStatistic",
      r: "2xl",
      p: 6,
      width: "100%",
      gap: 4,
      direction: "column",
      className: "oui-bg-base-9 xl:oui-flex-1",
      children: [
        (0, import_jsx_runtime.jsx)(Title7, { ...props }),
        (0, import_jsx_runtime.jsx)(Flex, { className: "oui-min-h-[170px] oui-h-full oui-w-full oui-flex oui-flex-row oui-items-stretch", children: (0, import_jsx_runtime.jsx)(
          VolBarChart,
          {
            data: props.dataSource,
            colors: { fill: "rgba(0, 180, 158, 1)" },
            className: "oui-w-full oui-flex-1",
            tooltip: {
              rm: decimal_default.ROUND_DOWN,
              dp: props.volType === "volume" ? 2 : 6
            }
          }
        ) })
      ]
    }
  );
};
var Title7 = (props) => {
  const { t } = useTranslation();
  return (0, import_jsx_runtime.jsxs)(Flex, { direction: "row", justify: "between", width: "100%", children: [
    (0, import_jsx_runtime.jsx)(Text2, { className: "oui-text-lg", children: t("affiliate.statistics") }),
    (0, import_jsx_runtime.jsxs)(Flex, { direction: "row", gap: 2, className: "oui-min-w-14", children: [
      (0, import_jsx_runtime.jsx)(
        Select2.options,
        {
          size: "xs",
          value: props.period,
          onValueChange: props.onPeriodChange,
          options: props.periodTypes
        }
      ),
      (0, import_jsx_runtime.jsx)(
        Select2.options,
        {
          size: "xs",
          value: props.volType,
          onValueChange: props.onVolTypeChange,
          options: props.volTypes
        }
      )
    ] })
  ] });
};
var useTitleStatisticScript2 = () => {
  const { t } = useTranslation();
  const [period, setPeriod] = (0, import_react.useState)("7");
  const periodTypes = [
    { label: t("common.select.7d"), value: "7" },
    { label: t("common.select.30d"), value: "30" },
    { label: t("common.select.90d"), value: "90" }
  ];
  const onPeriodChange = (item) => {
    setPeriod(item);
  };
  const [volType, setVolType] = (0, import_react.useState)("rebate");
  const volTypes = [
    { label: t("affiliate.trader.rebate"), value: "rebate" },
    { label: t("common.volume"), value: "volume" }
  ];
  const onVolTypeChange = (item) => {
    setVolType(item);
  };
  const dateRange = (0, import_react.useMemo)(() => {
    if (period === "7") {
      return {
        startDate: subDays(/* @__PURE__ */ new Date(), 7),
        endDate: subDays(/* @__PURE__ */ new Date(), 1)
      };
    } else if (period === "30") {
      return {
        startDate: subDays(/* @__PURE__ */ new Date(), 30),
        endDate: subDays(/* @__PURE__ */ new Date(), 1)
      };
    } else if (period === "90") {
      return {
        startDate: subDays(/* @__PURE__ */ new Date(), 90),
        endDate: subDays(/* @__PURE__ */ new Date(), 1)
      };
    } else {
      return {
        startDate: subDays(/* @__PURE__ */ new Date(), 7),
        endDate: subDays(/* @__PURE__ */ new Date(), 1)
      };
    }
  }, [period]);
  const { data: distributionData, mutate } = useRefereeRebateSummary(dateRange);
  const { dailyVolume, chartConfig } = useReferralContext();
  const dataSource = (0, import_react.useMemo)(() => {
    if (volType === "rebate") {
      const newData = distributionData || [];
      return newData.map((e) => ({
        date: e.date,
        volume: e.referee_rebate
      })).reverse();
    } else if (volType === "volume") {
      return dailyVolume?.filter((e) => {
        return e.date > format(subDays(Date(), Number(period) + 1), "yyyy-MM-dd");
      }).map((e) => ({
        date: e.date,
        volume: e.perp_volume
      })) || [];
    } else {
      return [];
    }
  }, [distributionData, dailyVolume, volType]);
  return {
    period,
    periodTypes,
    onPeriodChange,
    volType,
    volTypes,
    onVolTypeChange,
    dataSource: fillData(Number(period), dataSource)
  };
};
var TitleStatisticWidget2 = () => {
  const state = useTitleStatisticScript2();
  return (0, import_jsx_runtime.jsx)(TitleStatistic2, { ...state });
};
var Rebates = (props) => {
  return (0, import_jsx_runtime.jsxs)(
    Flex,
    {
      id: "oui-affiliate-trader-rebates",
      r: "2xl",
      p: 6,
      width: "100%",
      gap: 4,
      direction: "column",
      className: "oui-bg-base-9 oui-tabular-nums",
      children: [
        (0, import_jsx_runtime.jsx)(Title8, { ...props }),
        (0, import_jsx_runtime.jsx)(
          Flex,
          {
            width: "100%",
            className: "oui-border-y-2 oui-border-line-6",
            height: 49,
            direction: "row",
            itemAlign: "center",
            children: (0, import_jsx_runtime.jsx)("div", { children: (0, import_jsx_runtime.jsx)(
              DatePicker2.range,
              {
                size: "xs",
                value: props.dateRange,
                onChange: (range) => {
                  props.setDateRange(range);
                },
                max: 90,
                disabled: {
                  after: /* @__PURE__ */ new Date()
                }
              }
            ) })
          }
        ),
        (0, import_jsx_runtime.jsx)(List, { ...props })
      ]
    }
  );
};
var Title8 = (props) => {
  const { t } = useTranslation();
  return (0, import_jsx_runtime.jsxs)(Flex, { direction: "row", justify: "between", width: "100%", children: [
    (0, import_jsx_runtime.jsx)(Text2, { className: "oui-text-lg", children: t("affiliate.trader.myRebates") }),
    (0, import_jsx_runtime.jsx)(Text2, { intensity: 36, className: "oui-text-2xs", children: props.displayDate })
  ] });
};
var List = (props) => {
  const { t } = useTranslation();
  const layout767 = useMediaQuery("(max-width: 767px)");
  const columns = [
    {
      title: `${t("affiliate.trader.rebates")} (USDC)`,
      dataIndex: "referee_rebate",
      render: (value) => (0, import_jsx_runtime.jsx)(Text2, { children: commifyOptional(value, {
        fix: 6,
        fallback: "0",
        padEnd: true,
        prefix: "$"
      }) }),
      width: 127
    },
    {
      title: `${t("affiliate.trader.tradingVol")} (USDC)`,
      dataIndex: "vol",
      render: (value) => commifyOptional(value, {
        fix: 2,
        fallback: "0",
        padEnd: true,
        prefix: "$"
      }),
      width: 127
    },
    {
      title: t("common.date"),
      dataIndex: "date",
      render: (value) => (0, import_jsx_runtime.jsx)(Text2.formatted, { formatString: "yyyy-MM-dd", children: value }),
      width: 127
    }
  ];
  if (layout767) {
    return (0, import_jsx_runtime.jsx)(
      ListView,
      {
        dataSource: props.dataSource,
        className: "oui-h-[197px] oui-w-full",
        renderItem: (item, index) => {
          return (0, import_jsx_runtime.jsx)(Cell, { item });
        }
      }
    );
  }
  return (0, import_jsx_runtime.jsx)(
    AuthGuardDataTable,
    {
      bordered: true,
      loading: props.isLoading,
      columns,
      dataSource: props.dataSource,
      ignoreLoadingCheck: true,
      onRow: (record) => {
        return {
          className: "oui-h-[41px]"
        };
      },
      pagination: props.pagination,
      manualPagination: false
    }
  );
};
var Cell = (props) => {
  const { t } = useTranslation();
  return (0, import_jsx_runtime.jsxs)(Flex, { direction: "column", width: "100%", children: [
    (0, import_jsx_runtime.jsxs)(Flex, { width: "100%", gap: 1, children: [
      (0, import_jsx_runtime.jsx)(
        Statistic,
        {
          label: `${t("affiliate.trader.rebates")} (USDC)`,
          className: "oui-text-xs oui-text-base-contrast-54 oui-flex-1 oui-min-w-[105px]",
          children: (0, import_jsx_runtime.jsx)(Text2, { size: "sm", intensity: 80, children: commifyOptional(props.item.referee_rebate, {
            fix: 6,
            fallback: "0",
            padEnd: true,
            prefix: "$"
          }) })
        }
      ),
      (0, import_jsx_runtime.jsx)(
        Statistic,
        {
          label: `${t("affiliate.trader.tradingVol")} (USDC)`,
          className: "oui-text-xs oui-text-base-contrast-54 oui-flex-shrink",
          children: (0, import_jsx_runtime.jsx)(Text2, { size: "sm", intensity: 80, children: commifyOptional(props.item.vol, {
            fix: 2,
            fallback: "0",
            padEnd: true,
            prefix: "$"
          }) })
        }
      ),
      (0, import_jsx_runtime.jsx)(
        Statistic,
        {
          label: t("common.date"),
          className: "oui-text-xs oui-text-base-contrast-54 oui-flex-1",
          align: "end",
          children: (0, import_jsx_runtime.jsx)(Text2.formatted, { size: "sm", intensity: 80, formatString: "yyyy-MM-dd", children: props.item.date })
        }
      )
    ] }),
    (0, import_jsx_runtime.jsx)(Divider, { className: "oui-w-full oui-mt-3" })
  ] });
};
var useRebatesScript = () => {
  const [dateRange, setDateRange] = (0, import_react.useState)({
    from: subDays(/* @__PURE__ */ new Date(), 90),
    to: subDays(/* @__PURE__ */ new Date(), 1)
  });
  const {
    data: distributionData,
    mutate,
    isLoading
  } = useRefereeRebateSummary({
    startDate: dateRange?.from,
    endDate: dateRange?.to
  });
  const { data: dailyVolume, mutate: dailyVolumeMutate } = useDaily({
    startDate: dateRange?.to,
    endDate: dateRange?.from
  });
  const dataSource = (0, import_react.useMemo)(() => {
    if (typeof distributionData === "undefined") return [];
    return distributionData.map((item) => {
      const createdTime = item.date;
      const volume = dailyVolume?.filter((item2) => {
        return compareDate(toDate(createdTime), toDate(item2.date));
      })?.[0];
      if (volume) {
        return { ...item, vol: volume.perp_volume };
      }
      return item;
    });
  }, [distributionData, dailyVolume]);
  let displayDate = void 0;
  if ((dataSource?.length || 0) > 0) {
    displayDate = formatDateTimeToUTC(dataSource?.[0].date);
  }
  const { pagination, setPage } = usePagination();
  (0, import_react.useEffect)(() => {
    setPage(1);
  }, [dateRange]);
  return {
    dateRange,
    setDateRange,
    displayDate,
    dataSource,
    // meta,
    // onPageChange,
    // onPageSizeChange,
    pagination,
    isLoading
  };
};
var RebatesWidget = () => {
  const state = useRebatesScript();
  return (0, import_jsx_runtime.jsx)(Rebates, { ...state });
};
var TraderPage = () => {
  return (0, import_jsx_runtime.jsxs)(
    Flex,
    {
      id: "oui-affiliate-trader-page",
      className: M(
        "oui-h-lvw oui-w-full",
        // padding
        // "oui-p-4 lg:oui-p-6 xl:oui-p-3",
        "oui-font-semibold"
      ),
      direction: "column",
      gap: 4,
      children: [
        (0, import_jsx_runtime.jsxs)(
          Flex,
          {
            width: "100%",
            gap: 4,
            className: "oui-flex oui-flex-col xl:oui-flex-row xl:oui-items-stretch",
            children: [
              (0, import_jsx_runtime.jsx)(SummaryWidget3, {}),
              (0, import_jsx_runtime.jsx)(TitleStatisticWidget2, {})
            ]
          }
        ),
        (0, import_jsx_runtime.jsx)(RebatesWidget, {})
      ]
    }
  );
};
var Tab = (props) => {
  const { t } = useTranslation();
  const extendNode = () => {
    if (props.isAffiliate && !props.isTrader) {
      return (0, import_jsx_runtime.jsxs)(
        Button,
        {
          variant: "contained",
          color: "success",
          size: "sm",
          className: "oui-px-2 oui-flex oui-gap-1",
          style: {
            position: "absolute",
            top: "50%",
            right: "24px",
            transform: "translateY(-50%)"
          },
          onClick: (e) => {
            props.anATrader();
          },
          children: [
            (0, import_jsx_runtime.jsx)(TraderIcon, {}),
            (0, import_jsx_runtime.jsx)(Text2, { children: t("affiliate.asTrader.title") })
          ]
        }
      );
    }
    if (!props.isAffiliate && props.isTrader) {
      return (0, import_jsx_runtime.jsxs)(
        Button,
        {
          variant: "contained",
          color: "primary",
          size: "sm",
          className: "oui-px-2 oui-flex oui-gap-1",
          style: {
            position: "absolute",
            top: "50%",
            right: "24px",
            transform: "translateY(-50%)"
          },
          onClick: (e) => {
            props.anAnAffiliate();
          },
          children: [
            (0, import_jsx_runtime.jsx)(AffiliateIcon, {}),
            (0, import_jsx_runtime.jsx)(Text2, { children: t("affiliate.asAffiliate.title") })
          ]
        }
      );
    }
    return void 0;
  };
  if (props.isLoading) {
    return (0, import_jsx_runtime.jsx)("div", { className: M("oui-max-w-[960px]", props.classNames?.loadding), children: props.splashPage?.() || (0, import_jsx_runtime.jsx)(HomePage, {}) });
  }
  if (!props.isAffiliate && !props.isTrader || props.showHome) {
    return (0, import_jsx_runtime.jsx)("div", { className: M("oui-max-w-[960px]", props?.classNames?.home), children: (0, import_jsx_runtime.jsx)(HomePage, {}) });
  }
  return (0, import_jsx_runtime.jsxs)(
    TabsBase,
    {
      id: "oui-affiliate-dashboard-tab",
      value: props.tab,
      onValueChange: (e) => {
        props.setTab(e);
      },
      className: M("oui-w-full", props.classNames?.dashboard),
      children: [
        (0, import_jsx_runtime.jsxs)(
          TabsList,
          {
            className: M(
              "oui-px-6 oui-flex oui-flex-row oui-justify-start oui-h-[44px] oui-relative oui-items-end",
              "oui-text-base md:oui-text-lg",
              "oui-rounded-xl oui-bg-base-9",
              "oui-border oui-border-line-6",
              props.isAffiliate && props.isTrader && "oui-justify-center"
            ),
            children: [
              props.isAffiliate && (0, import_jsx_runtime.jsx)(
                TabsTrigger,
                {
                  id: "oui-affiliate-dashboard-tab-affiliate",
                  value: "affiliate",
                  children: (0, import_jsx_runtime.jsxs)(Flex, { direction: "row", gap: 1, mb: 2, children: [
                    (0, import_jsx_runtime.jsx)(
                      AffiliateIcon,
                      {
                        fillOpacity: 1,
                        fill: "currentColor",
                        className: props.tab === "affiliate" ? "oui-fill-white/[.98]" : "oui-fill-white/[.36]"
                      }
                    ),
                    (0, import_jsx_runtime.jsx)(Text2, { children: t("common.affiliate") })
                  ] })
                }
              ),
              props.isTrader && (0, import_jsx_runtime.jsx)(
                TabsTrigger,
                {
                  id: "oui-affiliate-dashboard-tab-trader",
                  value: "trader",
                  children: (0, import_jsx_runtime.jsxs)(Flex, { direction: "row", gap: 1, mb: 2, children: [
                    (0, import_jsx_runtime.jsx)(
                      TraderIcon,
                      {
                        fillOpacity: 1,
                        fill: "currentColor",
                        className: props.tab === "trader" ? "oui-fill-white/[.98]" : "oui-fill-white/[.36]"
                      }
                    ),
                    (0, import_jsx_runtime.jsx)(Text2, { children: t("affiliate.trader") })
                  ] })
                }
              ),
              extendNode()
            ]
          }
        ),
        props.isAffiliate && (0, import_jsx_runtime.jsx)(TabsContent, { value: "affiliate", className: "oui-mt-4", children: (0, import_jsx_runtime.jsx)(AffiliatePage, {}) }),
        props.isTrader && (0, import_jsx_runtime.jsx)(TabsContent, { value: "trader", className: "oui-mt-4", children: (0, import_jsx_runtime.jsx)(TraderPage, {}) })
      ]
    }
  );
};
var useTabScript = () => {
  const {
    isAffiliate,
    isTrader,
    isLoading,
    splashPage,
    showHome,
    setShowHome,
    tab,
    setTab
  } = useReferralContext();
  const tableValue = (0, import_react.useMemo)(() => {
    if (isAffiliate && isTrader) {
      return tab;
    } else if (isAffiliate && !isTrader) {
      return "affiliate";
    } else if (!isAffiliate && isTrader) {
      return "trader";
    } else {
      return "affiliate";
    }
  }, [tab, isAffiliate, isTrader]);
  (0, import_react.useEffect)(() => {
    const searchParams = new URLSearchParams(window.location.search);
    const tab2 = searchParams.get("tab");
    if (tab2) {
      setTab(tab2);
    }
  }, [window.location.search]);
  const anAnAffiliate = () => {
    setShowHome(true);
  };
  const anATrader = () => {
    setShowHome(true);
  };
  return {
    setTab,
    tab: tableValue,
    isAffiliate,
    isTrader,
    isLoading,
    anAnAffiliate,
    anATrader,
    splashPage,
    showHome,
    setShowHome
    // tab: TabTypes.affiliate,
    // isAffiliate: false,
    // isTrader: false,
    // isLoading: false,
  };
};
var TabWidget = (props) => {
  const state = useTabScript();
  return (0, import_jsx_runtime.jsx)(Tab, { classNames: props.classNames, ...state });
};
var DashboardPage = (props) => {
  const { root, ...rest } = props.classNames || {};
  const { state } = useAccount();
  const { wrongNetwork, disabledConnect, initialized } = useAppContext();
  const { isMultiLevelEnabled, multiLevelRebateInfo, isAffiliate, isLoading } = useReferralContext();
  const loadingView = (0, import_jsx_runtime.jsx)(Flex, { justify: "center", itemAlign: "center", height: "100vh", children: (0, import_jsx_runtime.jsx)(Spinner, {}) });
  const [delay, setDelay] = (0, import_react.useState)(!initialized);
  (0, import_react.useEffect)(() => {
    const timer = setTimeout(() => setDelay(false), 500);
    return () => clearTimeout(timer);
  }, []);
  if (state.validating || delay) {
    return loadingView;
  }
  if (wrongNetwork || disabledConnect || state.status < AccountStatusEnum.EnableTrading && state.status !== AccountStatusEnum.EnableTradingWithoutConnected) {
    return (0, import_jsx_runtime.jsx)(LandingPage, {});
  }
  if (isLoading) {
    return loadingView;
  }
  if (isMultiLevelEnabled) {
    if (multiLevelRebateInfo?.referral_code || isAffiliate) {
      return (0, import_jsx_runtime.jsx)(MultiLevelAffiliatePage, {});
    }
    return (0, import_jsx_runtime.jsx)(LandingPage, {});
  }
  return (0, import_jsx_runtime.jsx)(
    "div",
    {
      id: "oui-affiliate-dashboard-page",
      className: M("oui-w-full oui-tracking-tight", root),
      children: (0, import_jsx_runtime.jsx)(TabWidget, { classNames: rest })
    }
  );
};
var AffiliateLayout = (props) => {
  const { ...rest } = props;
  return (0, import_jsx_runtime.jsx)(Scaffold, { leftSidebar: (0, import_jsx_runtime.jsx)(LeftSidebar, { ...rest }), children: (0, import_jsx_runtime.jsx)(Box, { mx: 3, my: 6, children: props.children }) });
};
var LeftSidebar = (props) => {
  return (0, import_jsx_runtime.jsx)(Box, { p: 4, m: 3, border: true, borderColor: 8, r: "xl", children: (0, import_jsx_runtime.jsx)(SideBar, { ...props }) });
};
var useLayoutBuilder = () => {
  const [current, setCurrent] = (0, import_react.useState)("/");
  const { t } = useTranslation();
  const items = (0, import_react.useMemo)(() => {
    return [
      {
        name: t("common.trading"),
        href: "/",
        icon: (0, import_jsx_runtime.jsxs)(
          "svg",
          {
            width: "18",
            height: "18",
            viewBox: "0 0 18 18",
            fill: "none",
            xmlns: "http://www.w3.org/2000/svg",
            children: [
              (0, import_jsx_runtime.jsx)(
                "path",
                {
                  d: "M4.883 1.5c-.166.01-.393.076-.515.173a2 2 0 0 0-.18.173L.918 6.553a.86.86 0 0 0-.168.475c0 .339.232.543.232.543l7.374 8.633c.005.006.269.301.644.296s.64-.29.644-.296l7.402-8.663s.204-.269.204-.513-.196-.517-.196-.517L13.82 1.848c-.023-.032-.076-.021-.103-.049-.033-.038-.037-.091-.077-.124-.123-.097-.267-.106-.413-.123-.04-.005-.063-.049-.103-.049h-.078zm1.745 1.582h4.744L9 5.795zm-1.675.542 2.294 2.615H3.122zm8.094 0 1.831 2.615h-4.125zM3.328 7.818h4.847v5.672zm6.497 0h4.847c-1.151 1.345-3.422 4.007-4.847 5.672z",
                  fill: "url(#a)"
                }
              ),
              (0, import_jsx_runtime.jsx)("defs", { children: (0, import_jsx_runtime.jsxs)(
                "linearGradient",
                {
                  id: "a",
                  x1: "17.25",
                  y1: "9",
                  x2: ".75",
                  y2: "9",
                  gradientUnits: "userSpaceOnUse",
                  children: [
                    (0, import_jsx_runtime.jsx)("stop", { stopColor: "rgb(var(--oui-gradient-brand-end))" }),
                    (0, import_jsx_runtime.jsx)(
                      "stop",
                      {
                        offset: "1",
                        stopColor: "rgb(var(--oui-gradient-brand-start))"
                      }
                    )
                  ]
                }
              ) })
            ]
          }
        )
      },
      {
        name: t("common.affiliate"),
        href: "/affiliate",
        icon: (0, import_jsx_runtime.jsx)(
          "svg",
          {
            width: "18",
            height: "18",
            viewBox: "0 0 18 18",
            fill: "none",
            xmlns: "http://www.w3.org/2000/svg",
            children: (0, import_jsx_runtime.jsx)(
              "path",
              {
                fillRule: "evenodd",
                clipRule: "evenodd",
                d: "M5.625 4.95c0-.746.604-1.35 1.35-1.35h4.05c.746 0 1.35.604 1.35 1.35v4.62L9 12.195 5.625 9.57zm8.624 3.163-.524.407v-.96l.074.05c.193.128.346.302.45.502m-4.42 5.148L14.4 9.705v3.345a1.35 1.35 0 0 1-1.35 1.35h-8.1a1.35 1.35 0 0 1-1.35-1.35V9.705l4.571 3.556c.488.379 1.17.379 1.658 0M4.275 8.52l-.524-.407c.104-.2.257-.374.45-.503l.074-.049zm0-2.581V4.95a2.7 2.7 0 0 1 2.7-2.7h4.05a2.7 2.7 0 0 1 2.7 2.7v.989l.823.548a2.7 2.7 0 0 1 1.202 2.247v4.316a2.7 2.7 0 0 1-2.7 2.7h-8.1a2.7 2.7 0 0 1-2.7-2.7V8.734a2.7 2.7 0 0 1 1.202-2.247zM9 6.089l.111-.118.005-.005a1.08 1.08 0 0 1 1.58.004 1.223 1.223 0 0 1 0 1.668l-1.515 1.62-.004.004c-.1.102-.26.1-.358-.004l-1.516-1.62a1.223 1.223 0 0 1 0-1.668l.004-.004a1.08 1.08 0 0 1 1.58.004z",
                fill: "#fff",
                fillOpacity: ".54"
              }
            )
          }
        )
      }
    ];
  }, [t]);
  return {
    items,
    current,
    // open: sideOpen,
    // onOpenChange(open) {
    //   setSideOpen(open);
    // },
    onItemSelect: (item) => {
      setCurrent(item.href);
    }
  };
};
var AffiliateLayoutWidget = (props) => {
  const state = useLayoutBuilder();
  return (0, import_jsx_runtime.jsx)(AffiliateLayout, { ...state, children: props.children });
};
export {
  AffiliateLayoutWidget,
  dashboard_exports as Dashboard,
  LandingPage,
  MultiLevelAffiliatePage,
  ReferralContext,
  ReferralProvider,
  TabTypes,
  useReferralContext
};
//# sourceMappingURL=@orderly__network_affiliate.js.map
