...
async function recoverPublicKey({ hash: hash2, signature }) {
  ...
  const { secp256k1: secp256k13 } = await import("./secp256k1-RKRHVEG3.js");
  const signature_ = (() => {
...
function toBytes2(publicKey, options = {}) {
  return fromHex2(toHex2(publicKey, options));
}
...
function decodeRlpCursor(cursor, to2 = "Hex") {
  if (cursor.bytes.length === 0)
    return to2 === "Hex" ? fromBytes2(cursor.bytes) : cursor.bytes;
  const prefix = cursor.readByte();
  if (prefix < 128)
    cursor.decrementPosition(1);
  if (prefix < 192) {
    ...
    const bytes = cursor.readBytes(length2);
    return to2 === "Hex" ? fromBytes2(bytes) : bytes;
  }
...
function expand_message_xmd(msg, DST, lenInBytes, H) {
  ...
  anum(lenInBytes);
  if (DST.length > 255)
    DST = H(concatBytes3(utf8ToBytes("H2C-OVERSIZE-DST-"), DST));
...
function verify(options) {
  ...
  return secp256k12.verify(signature, from(payload), toBytes2(publicKey), ...hash2 ? [{ prehash: true, lowS: true }] : []);
}
...
*/
//# sourceMappingURL=chunk-UVLSMXWV.js.map
