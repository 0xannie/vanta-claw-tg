...
function walletConnect(options) {
  ...
  return () => {
    return {
      label: "WalletConnect",
      getIcon: async () => (await import("./icon-FY6GL2VH.js")).default,
      getInterface: async ({ chains, EventEmitter, appMetadata }) => {
        const { ProviderRpcError, ProviderRpcErrorCode } = await import("./dist-FP3HRJVZ.js");
        const { default: EthereumProvider, REQUIRED_METHODS } = await import("./index.es-PXMJF3XB.js");
        const { Subject, fromEvent } = await import("./esm5-GNWOJNSY.js");
        const { takeUntil, take } = await import("./operators-5B3U3QJN.js");
        const getMetaData = () => {
          if (!appMetadata)
            return void 0;
